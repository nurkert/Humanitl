...
| HUM-131 | Die Aufzeichnung kennt ein Alter, aber keine Menge | M | Begrenzt sind heute einzelne Zeilen (`limits.recorder_max_body_bytes` 33 554 432, `recorder.inline_max_bytes` 262 144) und das Alter (`recorder.retention_days` 90, taeglich ueber `purge_daily`), nicht die Summe: keine Obergrenze in Bytes, keine Verdraengung des Aeltesten, keine Untergrenze fuer den freien Platz. Ein Agent, der einen Tag lang grosse Antworten zieht, fuellt bis zu 90 Tage lang, bevor ueberhaupt geloescht wird. Abnahme: `recorder.max_total_bytes` (Vorgabe 2 GiB) verdraengt das Aelteste, `recorder.keep_free_bytes` (Vorgabe 1 GiB) laesst Ruempfe weg statt die Platte zu fuellen und meldet das einmal je Sitzung, beide im Register `effective`, die gemessene Groesse einer Sitzung steht im Commit-Body |

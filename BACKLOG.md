...
| **Terminal-Ausgabe** | Der Mensch liest, was der Agent schreibt | Der Daemon filtert jeden Bytestrom, bevor er einen Client erreicht (`humanitl_core::TerminalFilter`, Erlaubnisliste: am PTY Farbe, Cursor und die OSC-Nummern 4/10/11/104/110/111/133, ohne PTY nur `ESC [ … m`). OSC 52 (Clipboard), OSC 8, Fenstertitel, DCS, APC, PM und SOS gehen damit nicht hinaus; xterm2 registriert zusätzlich keinen OSC-Handler. Banner „Agent-Ausgabe ist nicht vertrauenswürdig" | |
...
| Sandbox-Backends | `trait SandboxBackend { fn launch(profile) -> Handle; fn isolation_check() }` | Incus, Docker, Podman, microVM (Kandidat microsandbox, Abschnitt 9 Punkt 12; austauschbar nach Abschnitt 9 Punkt 1), macOS Seatbelt |
...
| HUM-107 | `buf breaking` und `actionlint` können keinen Pull Request rot machen | S | `ci.yml:244` verliert `continue-on-error: true` (der Kommentar verlangte das nach dem Merge von HUM-003 am 2026-09-03), ein Versuchs-PR mit geänderter Feldnummer wird rot; Job `workflows-lint` mit gepinntem `actionlint` und `scripts/ci/lint-workflows.sh` in `make check` (SKIP ohne Werkzeug, Exit 1 mit `STRICT=1`); `CONTRIBUTING.md` verweist auf die Pflichtliste `ci.yml:13-15`, `docs/PROTOCOL.md` 6 nennt keinen Vorbehalt mehr (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-0.md`) |

> **Bestandsaufnahme 2026-09-05**: 64 Kriterien der 13 Issues gegen den Code geprüft, 54 abgehakt, 7 teilweise, 3 überholt formuliert; nach Kriterien vollständig sind HUM-004, HUM-005, HUM-007, HUM-010, HUM-062, HUM-063. Stand je Kästchen in `backlog/sprint-0.md`.
...
| HUM-108 | `PROXY_007` wird nie ausgegeben | S | `tls_observe` deutet `rustls::Error::NoApplicationProtocol` als `TlsFailure::NoApplicationProtocol` und baut daraus einen Befund `PROXY_007` (Warning, Host, `why`, `OpenUrl`) im Ereignisstrom, entstört je Host wie `TLS_002`; Zeile 13 der Matrix assertet ihn; `scripts/ci/lint-dead-codes.sh` macht jeden Registereintrag ohne Konstruktion rot, Allow-Liste mit Issue je Eintrag (`PROXY_001` HUM-057, `TERM_001` HUM-042, `AUDIT_001` HUM-050, `UI_002` Dart) (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |
| HUM-109 | `HoldQueue::extend` ist von nichts erreichbar | M | Vorher zu entscheiden: verdrahten oder entfernen. Verdrahten: `rpc Extend(ExtendRequest{flow_id, by_secs}) returns (ExtendResponse{deadline})`, `TransitionInput::Extend` mit `Held → Held` und `FlowEvent::Extended`, `PROTO_MINOR` 4, `humanitl flows extend <id> [--by SECS]`, `DaemonClient.extendHold` in drei Clients, Reduzierer setzt `deadline`; nicht gehalten ⇒ `IPC_003`. Entfernen: Funktion, Tests und die Zeile in HUM-016 samt `docs/UX.md:362` (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |
| HUM-110 | WebSocket-Upgrade kommt nie zustande | L | Nach `Decided(Allow)` auf einem Flow mit `Upgrade::WebSocket` behält der Proxy `Connection: Upgrade`, `Upgrade: websocket` und `Sec-WebSocket-*` (ohne `Extensions`), reicht das 101 zurück und kopiert danach beide Richtungen; Bytes Ziel → Client als `ResponseChunk` aufgezeichnet, Client → Ziel gezählt (`FlowDetail.upgrade_client_bytes`); Zeile 11 der Matrix assertet 101 und das Echo, 403/504 vor dem Upgrade unverändert; `docs/SECURITY.md` 6 und 10.3 und CONVENTIONS 4.10 im selben Commit auf das gebaute Verhalten (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |
| HUM-111 | Die Sicherheitsdokumente behaupten, was der Code nicht tut | M | Dreizehn Stellen in `docs/SECURITY.md`, `docs/THREAT-MODEL.md` und `README.md` (Prüfbefehl `find -type s`, argv „per Copy-Paste ausführbar", `/sys/class/net`, `--unshare-all`, `/dev/null`-Maskierung, Shim-Pfad, hudsucker, `PROXY_007`, Audit im Präsens, WebSocket-Frames, DNS-Beweis in ESC-3, Spike) werden korrigiert oder als geplant mit Issue gekennzeichnet; `scripts/ci/lint-docs.sh` hält die drei Garantiesätze an vier Stellen wortgleich (Mutation ⇒ Exit 1) (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |
| HUM-112 | hudsucker ist keine Abhängigkeit mehr, ADR-0001 sagt das Gegenteil | S | ADR-0019 hält den eigenen Accept-Loop auf hyper 1 und `tokio-rustls` samt Spike-Ergebnis fest, ADR-0001 trägt `Superseded by ADR-0019` (die Rust-Entscheidung gilt fort), `docs/adr/check.sh` meldet 19; `README.md`, `AGENTS.md`, `docs/ARCHITECTURE.md`, `docs/SECURITY.md:364`, ADR-0015, BACKLOG 2/3.2 und CONVENTIONS 3.1 nennen hyper, rustls, rcgen und die drei Ports statt hudsucker (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |
| HUM-113 | `escape-launch` ist verwaist | S | `daemon/crates/sandbox/src/bin/escape-launch.rs` (1111 Zeilen, seit HUM-064 ohne Aufrufer, zweite Kopie der Prüf-und-Beenden-Logik) wird gelöscht, seine neun Tests werden am CLI-Weg neu geschrieben oder im Commit-Body als abgedeckt erklärt; `docs/SECURITY.md` 10 (Punkt 6b), CONVENTIONS `:476` und `tests/escape/README.md` nennen nur noch `humanitl sandbox run`; `./tests/escape/run.sh` mit gleicher Bilanz (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-1.md`) |

> **Bestandsaufnahme 2026-09-05**: 42 Kriterien von elf Issues gegen den Code geprüft (HUM-015 nicht Teil der Aufnahme), 24 abgehakt, 14 teilweise, 2 nicht erfüllt, 2 überholt formuliert; nach Kriterien vollständig sind HUM-012 und HUM-014. Stand je Kästchen in `backlog/sprint-1.md`.
...
| HUM-089 | acknowledge_findings wird nie gelesen | S | `DecideRequest.acknowledge_findings` entfernt, Nummer 6 und der Name mit Begründung reserviert, Vertragstabelle in `proto_contract.rs`, `proto_roundtrip.rs:156` und `flows.rs:339` nachgezogen, neuer Test `decide_request_reserves_the_never_read_acknowledge_findings_number`, `proto/descriptor.binpb` und `proto/generated.sha256` neu erzeugt, `docs/PROTOCOL.md` 4 führt die reservierten Nummern als Tabelle, HUM-049 in `backlog/sprint-4.md` zieht auf die freien Nummern 8 und 9 und bekommt eine Pfadliste, deren Dateien existieren; `PROTO_MAJOR`/`PROTO_MINOR` bleiben unverändert (heute 1/3 seit HUM-040) und kein Verhalten ändert sich; vorher zu klären: die Ausnahme in `docs/PROTOCOL.md` 4/5 für ein nie gelesenes Feld (Stand in `backlog/sprint-2.md`) |
| HUM-091 | FlowSummary traegt keine registrierbare Domaene | L | `FlowSummary` bekommt `string apex = 25` (A-Label nach Public Suffix List, leer heißt unbekannt und wird nie geraten), gefüllt aus der Spalte `apex` auf dem History-Pfad und aus der `DomainTable` live; `humanitl flows list --json`, der Dart-`Flow` und beide Fakes tragen ihn, `apex:` filtert überall exakt gleich (`apex:github.io` trifft `a.b.github.io` nirgends), `app/lib/features/intercept/psl.dart` entfällt und Warteschlange wie Regel-Ziel „Domäne" arbeiten ohne zweiten `GetFlow`-Aufruf auf dem gelieferten Feld; `record_to_detail` bekommt dieselbe `DomainTable`, `convert::apex_of` weicht `humanitl_catalog::psl::apex`, und 15 Testdateien mit `heldFlow` plus drei Golden-Suites ziehen nach (Stand in `backlog/sprint-2.md`) |
| HUM-094 | Katalogname und Editor fehlen dem M2-Demolauf | XL | Katalog-Karte und Unbekannt-Karte ersetzen `DomainPanePlaceholder`, Gruppenkopf und Summary nennen den Dienst aus `catalog_id` („npm registry", „Looks like: npm install") statt eines Hosts, `app/assets/catalog/domains.yaml` ist byte-gleich mit `catalog/domains.yaml` (`make catalog-lint`), und `BACKLOG.md:395` verspricht für M2 kein `Edit` mehr (Editor bleibt HUM-047, M4). Der Bildschirm-Lauf selbst gehört HUM-097; die zwei Namens-Behauptungen kommen als Nachtrag dorthin, wenn der Gruppenkopf steht. Vorher zu entscheiden: `FlowSummary.catalog_id`, denn der Kopf hat heute keinen Draht (`Flow` kennt kein `domain`, `flows.dart` verwirft `DomainInfo`); hängt an HUM-091 und HUM-097 (Stand in `backlog/sprint-2.md`) |
| HUM-095 | Sitzungsregel aus dem Stapel traegt keine Herkunft | M | `humanitl flows decide <id> allow\|block --remember <PATTERN> [--remember-method\|-path\|-expires\|-note]` schickt einen `Decide` mit gefülltem `remember`, Aktion aus dem Verdikt, Ablauf `session` als Default und `created_from_flow_id` gleich der entschiedenen Id; ohne das Flag bleibt Aufruf und JSON unverändert; `--note` (an den Agenten) und `--remember-note` (in der Regel) kreuzen sich nicht; scheitert die Regel, wird nichts entschieden; `rules list --json` zeigt die Herkunft und der Regel-Bildschirm das Abzeichen; M2-Lauf legt die Sitzungsregel über die Entscheidung an statt über `rules add`; Flag-Form und JSON-Form kollidieren mit HUM-090, der Projekteigentümer entscheidet vor dem Bau (Stand in `backlog/sprint-2.md`) |
| HUM-097 | Oberflaechen-Haelfte des M2-Demoskripts | L | `app/integration_test/m2_first_decision_test.dart` treibt denselben Lauf ueber den Bildschirm statt ueber die Kommandozeile: drei Gruppen in der Warteschlange, Batch-Allow mit `Remember session x apex`, Block mit `B`, Allow mit Enter, eine Anfrage in die Frist laufen lassen, History mit `decision:block`, `decision:timed_out` und `findings:>0` filtern, Export als HAR nach `HUMANITL_E2E_HAR`; `run.sh` faehrt ihn unter `xvfb-run -a` mit 1600x1000 und prueft danach 17 Eintraege in der Datei, `M2_UI=1` endet 0 statt mit `SKIPPED:`, und der Absatz „nur die Daemon-Haelfte“ verschwindet aus `CONTRIBUTING.md`, `CONVENTIONS.md` 4.22 und HUM-036; vorher: der Bildschirm-Treiber läuft, während gehalten wird (Schritt 10 liegt heute hinter dem Ende des Agenten), ein Exportziel nach `HUMANITL_E2E_HAR` statt des Portals, `flutter pub get` und `flutter build linux` im Job `e2e-xvfb` (Stand in `backlog/sprint-2.md`) |
| HUM-105 | Schalter „Deaktivieren" fuer mitgelieferte Regeln | M | Dart-`Rule` trägt `disabled`, `RuleToDomain` liest es, `DaemonClient`, `GrpcDaemonClient` und `FakeDaemonClient` haben `setRuleDisabled` über `RulesRequest.set_disabled`, der Regel-Bildschirm zeichnet eine abgeschaltete mitgelieferte Regel als abgeschaltet und bietet ihr den Schalter statt des Papierkorbs; eigene Regeln bekommen keinen Schalter (der Daemon antwortet `RULES_010`); `humanitl rules disable ID` und der Bildschirm zeigen denselben Zustand (Oberflächen-Hälfte von HUM-038) |
| HUM-106 | TLS-Karte im Intercept-Bildschirm | M | `diagnosticsProvider` sammelt `FlowEvent.diagnostic`, das aus `flow_diagnostic` die `flowId` trägt; eine Karte über der Warteschlange zeigt `TLS_001..003` mit dem `why`-Satz des Daemons, `FixControl` bietet für `SetEnv` den Kopierbefehl `export KEY=VALUE`, jede Karte ist wegklickbar; das Fake-Szenario `default` zeigt dieselbe Karte nach 5 s; kein Knopf, der in die Konfiguration schreibt (`SetConfig` unimplementiert, HUM-069) (Oberflächen-Hälfte von HUM-045) |
| HUM-114 | `humanitl rules test` ist eine Sackgasse | M | Das Kommando ruft `RulesRequest.Test` (steht seit HUM-027 im Vertrag) statt `CLI_003` zu antworten: `verdict: allow\|block\|ask\|redact`, Regel mit Herkunft und Position, Exit 0/10/11, `--json` mit den Feldern von `RuleTest`; die zwei Sätze „10 and 11 do not occur yet" in `--help` verschwinden; `esc-4-rules.sh` fährt die 15 Fälle der Host-Tabelle über das Kommando gegen den Daemon des Laufs; `humanitl llm test URL` ruft `ProbeLlm` (`docs/PROTOCOL.md` 4.9, CONVENTIONS 4.21) (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |
| HUM-115 | ESC-3 führt den DNS-Beweis nicht | M | `HickoryResolver` hinter `resolver.nameserver` (der Schlüssel wirkt heute nicht; Abhängigkeit trägt der Eigentümer in `daemon/Cargo.toml` ein), ein aufzeichnender Stub-Nameserver `tests/escape/dns-stub.py` in `run.sh`; host-seitige Proben `dns_not_before_decision`, `dns_after_allow_once`, `meta_no_dns_lookup` statt der zwei `skip`; `docs/THREAT-MODEL.md` K-10 wird wahr; HUM-021 Kriterium 3 durch ESC-3 abgelöst (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |
| HUM-116 | Body-Ansichten enden am History-Bildschirm | M | Das History-Detail zeichnet Request- und Response-Rumpf mit der `BodyView` aus HUM-030 (Baum, Formular, Raw, Hex, `Content-Encoding`, Fund-Markierung) statt eigener Textzeilen; `flowBodyProvider` zieht nach `app/lib/core/ipc/` und ist der eine Rumpf-Provider; `historyBodyProvider`, `decodeHistoryBody` und die tote `section_body_raw.dart` verschwinden; Goldens `history_detail_body_json_*`, `history_detail_body_hex_*` (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |
| HUM-117 | Die Notiz einer Entscheidung hat keinen Ort außer der 403-Antwort | M | Vorher zu entscheiden: speichern oder nicht. Speichern: Spalte `flows.decision_note` (V5), `FlowSummary`/`FlowDetail.decision_note`, `humanitl flows show` druckt `note:`, History-Detail zeigt sie, HAR `_humanitl.note`, `/why` liest sie nach einem Neustart aus dem Recorder, ein Secret in der Notiz ergibt `FINDINGS_003` ohne den Block zu ändern; der Recorder-Test „must not leak" fällt. Nicht speichern: HUM-072 Ziel, `docs/ARCHITECTURE.md` 8.2 und HUM-051 korrigieren, Grund nach CONVENTIONS 4 (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |
| HUM-118 | Notification-Hints kommen doppelt verpackt an | S | `dbus_notifications.dart:164-167` reicht `DBusByte(1)` und `DBusString` nackt an `DBusDict.stringVariant`, das selbst verpackt (live: `Unsupported variant type for "urgency": "v"`); Test liest die Hints über die Attrappe; `dbus_live_test.dart` prüft `requestName` und überspringt mit Grund, wenn ein fremder Watcher den Namen hält; `make flutter-test-dbus` fährt ihn unter `dbus-run-session` (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |
| HUM-119 | `GetBody` kennt kein `decoded`-Flag | M | `BodyRef.content_encoding` (vom Daemon gefüllt) und `BodyRef.decoded` (vom Client gesetzt), `BodyChunk.encoding_left`; `get_body` entpackt gzip, deflate und brotli über `humanitl_findings::decode` mit Budget und liefert sonst roh mit Namen; die App fragt immer `decoded`, `body_decode.dart` verliert `dart:io`-gzip, Fund-Spans stimmen für brotli-Rümpfe; `PROTO_MINOR` +1 (Bestandsaufnahme 2026-09-05, Spezifikation in `backlog/sprint-2.md`) |

> **Bestandsaufnahme 2026-09-05**: 56 Kriterien der 16 gemergten Issues gegen den Code geprüft, 36 abgehakt, 13 teilweise, 6 nicht erfüllt, 1 überholt formuliert; nach Kriterien vollständig sind HUM-023, HUM-025 und HUM-035. Stand je Kästchen in `backlog/sprint-2.md`.
...
| HUM-041 | Isolation-Check-Panel und Ring | L | Daemon führt drei Prüfungen in der laufenden Sandbox aus, Panel mit drei grünen Zeilen in Klartext plus amber LLM-Ausnahme, Header-Ring mit drei Segmenten, „Exakte bwrap-Kommandozeile anzeigen", Fehlschlag deaktiviert Start mit konkretem Grund und „Diagnose kopieren", nie „trotzdem starten" (Stand in `backlog/sprint-3.md`) |
| HUM-042 | Terminal | XL | PTY lebt im Daemon, `Terminal`-Bidi-Stream chunked mit Resize, xterm2 in Flutter, OSC 52/8/Titel deaktiviert, Banner „Agent-Ausgabe ist nicht vertrauenswürdig", Zeile im Terminal wenn ein Request gehalten wird (Stand in `backlog/sprint-3.md`) |
| HUM-043 | `/work`-Härtung | XL | Nur Unterpfad, maskierte Pfade per tmpfs, Session-Ende: Diff-Zusammenfassung berührter Dateien, Secret-Scan über Diff, Symlinks nach außerhalb markiert; host-seitige Dateizugriffe mit `O_NOFOLLOW`. Escape-Test 5 (Dateisystem-Teil) grün (Stand in `backlog/sprint-3.md`) |
| HUM-044 | Setup-Flow | XL | Vier-Punkte-Checkliste (Daemon, LLM, Projekt, Sandbox-Check) statt leerer Queue, systemd-Unit-Installation per Klick, Coach-Mark am ersten gehaltenen Request („Angehalten, weil keine Regel passt") (Stand in `backlog/sprint-3.md`) |
...
| HUM-067 | `humanitl run` | XL | Startet Daemon-Session im aktuellen Verzeichnis (`/work` = cwd), Profil per Flag oder `.humanitl/`, Agent aus Profil (Default OpenCode), Terminal ist das Terminal des Nutzers (PTY durchgereicht), `--ask terminal` mit Prompt-Format aus ADR-013, `--ask none` blockt alles ohne Regel; `llm-only` liefert die reine Inferenz-Instanz; UI kann sich an die laufende Session hängen und übernimmt die Queue (Stand in `backlog/sprint-3.md`) |
...
| HUM-068 | Geführte Diagnostics im Sandbox-Screen | XL | Jede Prüfung und jeder Startfehler zeigt Grund und Fix inline; LLM-Server unerreichbar ⇒ Diagnostic mit Ping-Ergebnis, Port, Vorschlag; Projektordner ohne Schreibrecht ⇒ Fix „als ro mounten"; bwrap zu alt ⇒ Paketbefehl kopierbar. Snapshot-Tests für alle Diagnostics-Codes (Stand in `backlog/sprint-3.md`) |
| HUM-046 | Demo-Skript M3 | L | CI mit Ollama-Mock: OpenCode startet, erster Prompt, models.dev geblockt per Default-Regel, `webfetch` auf eine URL wird gehalten, Allow, Antwort im Terminal sichtbar; zweiter `webfetch` wird mit Notiz geblockt, Agent gibt die Notiz im Terminal wieder (Stand in `backlog/sprint-3.md`) |
| HUM-087 | resolver.test_ca wirkt nicht | M | `humanitld --allow-test-ca` lädt die Wurzel aus `resolver.test_ca` als PEM und reicht sie an den Upstream-Stapel und die Endpunkt-Probe weiter; ohne Flag bleibt die Wurzelliste leer und der gesetzte Schlüssel erzeugt `CONFIG_008` statt einer nackten Log-Zeile, mit Flag und unbrauchbarer Datei startet der Daemon nicht (`CONFIG_007`, kein Socket); Paar-Test im Proxy zeigt mit `trust(root)` 200 und ohne 502 mit `reason: upstream_tls`; der M2-Lauf fährt über `https://`, Schritt 7 misst 200 statt 502, der Stolperdraht auf `humanitld --help` dreht sich um und das Ziel bedient 17 Anfragen statt 16; `docs/CONFIG.md`, `docs/DIAGNOSTICS.md` und die Schema-Fixture kommen aus den Generatorläufen, M1 bleibt grün |
| HUM-088 | experimental.upstream_port_map wirkt nicht | S | Der Schlüssel steht im Schema, wird validiert und hat im Proxy keinen Leser, während `docs/CONFIG.md` eine Portumlenkung zusagt; er wird entfernt statt eingebaut (Feld, Prüfung, Schema-Fixture, `docs/CONFIG.md`, alle Nennungen in `backlog/` und `tests/`). Abnahme: kein Leser und kein Schema-Eintrag mehr, eine `config.toml` mit dem Schlüssel meldet `CONFIG_005` als Warnung samt Schlüsselname, Issue und Grund und der Daemon startet weiter (Eintrag in `alias::RETIRED`, CONVENTIONS 4.25), der Freiform-Tabellen-Fall bleibt bei `a_free_table_is_replaced_as_a_whole` an `resolver.overrides` und die Zeile in `GROUP_OVERRIDES` bei der Gruppe `experimental`, `cargo test -p humanitl-config` und `tests/e2e/m2_first_decision/run.sh` grün, CONVENTIONS 4.22 hält Grund und Rückkehrbedingung fest |
| HUM-101 | Konfigurationsschluessel ohne Leser | L | Mindestens sieben Schluessel versprechen im Dokument eine Wirkung, die es nicht gibt (`limits.idle_timeout_secs`, `limits.body_timeout_secs`, `experimental.ws_hold`, `ui.sound`, `ui.notifications`, dazu die zwei `pseudonyms`-Schluessel aus HUM-079). Erste Entscheidung: `idle_timeout_secs` und `header_timeout_secs` beschreiben auf Keep-Alive dieselbe Spanne, einer von beiden verschwindet oder speist die vorhandene Uhr. Die Rumpfgrenze sitzt am gestreamten Antwort-Body (`body.rs`), die Leerlaufgrenze im Wrapper vor `serve_connection` (`core.rs:134-141`) mit einem Zaehler gehaltener Fluesse im `ConnectionContext`, der die CONNECT-Rekursion ueberlebt — ohne ihn toetet die Vorgabe von 90 s jeden Hold von 300 s. `reason: idle_timeout` entfaellt (im Leerlauf ist keine Anfrage in Flug), der IPC-Stapel bleibt ausgenommen. Statt eines Shell-Gates ueber Feldnamen (`env` trifft 244-mal, `profile` 464-mal) ein Register im Repo, eine Zeile je Schema-Pfad mit `effective` oder `pending(HUM-xxx)`, geprueft in `daemon/crates/config/tests/` gegen `schema::leaf_paths()` und gelesen von `config_docs.rs` |
| HUM-102 | Abgelehnte private Adresse hat keinen Diagnostic | M | Die Ablehnung aus ADR-006 erzeugt in `record_failure` genau einen Befund (`PROXY_008`) mit `why` und einer anwendbaren `FixAction::AddRule` — nicht Text zum Abtippen, den Typ gibt es und er ist bis zum Klick in der Oberflaeche durchgezogen. Die vorgeschlagene Regel traegt `action: ask` mit `allow_private: true`, `HostPattern::Exact` bzw. `Ip` beim Literal, Port, Schema, Methode und den Pfad durch `strip_query`; dafuer muss `flow.allow_private |= …` vor das `match action` in `pipeline.rs:349` — heute wirkt `allow_private` nur mit `action: allow`, sodass der einzige wirksame Vorschlag eine dauerhafte, ungehaltene Freigabe waere und das Issue netto unsicherer machte. Adresse und Vorschlag erreichen den Agenten nie (die Sandbox hat keinen Resolver, die Zuordnung Name auf private Adresse ist fuer ihn neu); Rundlauf-Test durch `parse_rules`, und der Test „nach Anwendung wird weiter gehalten" schlaegt heute fehl |
| HUM-103 | Meta-Fluesse fehlen in der Historie | M | Eine Anfrage an `humanitl.internal` erscheint in der Historie als eigener Eintrag mit `decision=meta` und ist mit `meta:true` filterbar: neue Variante in `Decision`/`DecisionKind` samt Proto, Recorder-Migration und Spalte, Term `meta:` im Filter-Parser (`recorder/src/filter.rs` und der Dart-Fake, Zeile in `tests/fixtures/filter-language.json` aus HUM-099), Zustandsautomat bekommt den Weg nach `Recorded` ohne Sperre; `/ask` bleibt zusaetzlich auditiert, `/` und `/why` nicht; Abnahme: ein Lauf mit je einem `/`, `/why` und `/ask` zeigt drei Eintraege, `meta:true` liefert genau sie, `meta:false` keinen davon, und HUM-073 Akzeptanzkriterium 4 wird dort abgehakt |
| HUM-104 | Durchreiche steht hinter den Regeln des Nutzers | M | `State::all` haengt die mitgelieferten Regeln samt Durchreiche **hinter** Sitzung und Nutzer, waehrend `main.rs:630` und `profiles/llm-only.toml:8-10` das Gegenteil behaupten und `RuleSet::prepend_bundled` ausserhalb von Tests keinen Aufrufer hat; Folge: `humanitl run --profile llm-only` blockt mit seiner eigenen Regel `block host "**"` das Modell, und eine weite `allow`-Regel des Nutzers macht aus der erklaerten Durchreiche ein stilles gewoehnliches Allow ohne `DecisionSource::Passthrough` und ohne `LLM_005`. Erste Entscheidung: vier Gruppen (Durchreiche, Sitzung, Nutzer, mitgeliefert, empfohlen) oder mitgeliefert ganz nach vorn (dann braucht `immutable_bundled` `Expiry::Session`); `prepend_bundled` wird verdrahtet oder entfernt, `CONVENTIONS.md` 4.5 haelt die eine Reihenfolge fest, und ein Systemtest ueber den echten Ladeweg belegt sie — nicht `prepend_bundled` von Hand |
| HUM-120 | Drei unbewachte Spannen der Verbindung | M | `limits.body_timeout_secs` (Vorgabe 300) wird nur geprueft und von niemandem gelesen, und drei Spannen laufen ohne Uhr: der **Anfrage-Rumpf** (`handler.rs:597` puffert ueber `body::buffer` ohne Frist, hypers Kopf-Uhr ist da schon geloescht — `Content-Length: 1000`, zehn Bytes, dann Schweigen haelt die Verbindung unbegrenzt; es greift nur die Byte-Grenze `hold_body_cap_bytes`), der **gestreamte Antwort-Rumpf** (`body.rs`, bis zu den Antwort-Kopfzeilen deckt der `handshake_timeout` alles ab, danach nichts) und der **TLS-Handschlag nach `CONNECT`** (`handler.rs:337`, ein Tunnel ohne `ClientHello` haelt den Task fuer immer). `limits.body_timeout_secs` speist die beiden Rumpf-Spannen, `limits.header_timeout_secs` den Handschlag. Die Grenze begrenzt die **Stille zwischen zwei Stuecken**, nicht die Gesamtdauer — das ist eine Umdeutung des heutigen Doku-Kommentars und gehoert samt `docs/CONFIG.md` und CONVENTIONS 4.4 in denselben Commit; vorher zu entscheiden, ob die zwei Namen aus Sprint 5 (`client_body_timeout_secs`, `response_idle_timeout_secs`) vorgezogen werden, dann mit `alias::RETIRED`-Eintrag fuer den alten Pfad. Anfrage-Rumpf antwortet `408`, Antwort-Rumpf endet als gekuerzter Strom, Handschlag still. Zeit ueber eine einspeisbare Uhr; die drei Faelle aus HUM-101 (`daemon/crates/proxy/tests/timeouts.rs`) bleiben gruen. Danach steht der Schluessel im Register auf `effective` und `docs/SECURITY.md` verliert seinen Vorbehalt |
| HUM-121 | Zwei Schluessel ohne Leser: ui.sound und experimental.ws_hold | S | Beide stehen im Schema, werden geprueft und haben ausserhalb von `#[cfg(test)]` keinen Leser (HUM-101 hat sie im Register auf `pending(HUM-121)` gesetzt). Je Schluessel eine begruendete Entscheidung, Einbau oder Streichung nach dem Muster von HUM-088: `ui.sound` spielt keinen Ton — entweder der Melder bekommt einen, oder der Schluessel faellt weg, statt im Schema eine Zusage zu tragen; `experimental.ws_hold` haelt kein WebSocket-Upgrade an, obwohl ADR-0007 den Schalter nennt, und `Experimental` kuendigt seinen eigenen Abbau an (Reihenfolge: nach HUM-110, das den Upgrade-Pfad ueberhaupt erst baut). Abnahme: kein Eintrag `pending(HUM-121)` bleibt im Register, `docs/CONFIG.md` und die Schema-Fixture kommen aus den Generatorlaeufen, und was gestrichen wird, ist danach ein harter `CONFIG_002` mit dem Schluesselnamen |
...
| HUM-157 | `audit.retention_days` hat keinen Leser | M | Entweder löscht der Daemon Records der Kette älter als die Frist und hinterlässt einen Record, den `verify` als dokumentierte Lücke anerkennt, oder der Schlüssel wird gestrichen; das Register der Leser führt ihn danach nicht mehr als `pending`; `docs/SECURITY.md` sagt, was die Kette danach noch beweist (Bestandsaufnahme 2026-09-18, Spezifikation in `backlog/sprint-4.md`) |
...
| HUM-163 | Jede Seite der Audit-Tabelle liest die ganze Kette | M | Die Dauer einer Seite über 100 000 Records ist gemessen (vorher rund 400 ms). Neu gefasst am 2026-09-24: Eine Seite prüft jede Zeile nur noch Byte für Byte auf ihre kanonische Form statt sie als JSON zu lesen und liegt über 100 000 Records bei rund 150 ms; die Kosten bleiben linear in der Länge der Kette, die 200 ms erreicht sie bei etwa 130 000 Records. Lesen nur vom Ende ist HUM-226 (aus HUM-156, Spezifikation in `backlog/sprint-4.md`) |
| HUM-226 | Eine Seite der Audit-Tabelle liest von hinten, solange die Kette heil ist | M | Eine Seite ohne Filter kostet unabhängig von der Länge der Kette, gemessen über 100 000 und 1 000 000 Records; bei einem Bruch mitten in der Kette dasselbe Ergebnis wie das Lesen jeder Zeile (aus HUM-163, Spezifikation in `backlog/sprint-4.md`) |
...
| HUM-098 | Ereignis-Reduzierer steht zweimal in der Anwendung | M | `applyFlowEvent` in `app/lib/core/domain/flow_reducer.dart` ist der einzige Ort, an dem ein `FlowEvent` einen vorhandenen `Flow` fortschreibt; von den sechs heutigen Unterschieden zwischen Warteschlange und Historie bleiben die drei beim Aufrufer (`_ready`, `_arrived`, `_resync` gegen `reload`) und die drei Uebergaenge werden zugunsten der Historie aufgeloest: `FlowEventDecided` und `FlowEventTimedOut` loeschen die Frist, `FlowEventResponseChunk` schreibt `responseSize` auch in der Warteschlange fort; damit steht eine ausscheidende Zeile in `visibleQueueFlows` hinter jeder wartenden, und der Countdown-Text in `request_card.dart:132` bekommt dieselbe `isHeld`-Bedingung wie der Ring; erschoepfende `switch` macht eine neue Variante rot, Mutationsprobe (ein Provider zurueck auf seine eigene Kette) macht einen Test rot |
| HUM-099 | Filtersprache wird an drei Stellen verschieden verstanden | M | `tests/fixtures/filter-language.json` ist die einzige Erwartungstabelle fuer `recorder/src/filter.rs` (volle Sprache), `ipc/src/convert.rs::matches_filter` (bewusst kleinere Lesart, die Tabelle sagt welche Terme sie kennt) und `FakeFlowFilter` im Dart-Fake, mit Bezugspunkt fuer `since:` in Millisekunden seit der Epoche; die belegte Divergenz `upgrade:none` wird auf beiden Seiten repariert (Recorder baut heute `upgrade = 'none'` gegen eine Spalte aus NULL oder `websocket` und trifft nie, der Dart-Fake laesst alles durch), kuenftig `upgrade IS NULL` und im Fake eine Pruefung am Fluss; `fake_daemon_client.dart` wird auf `app/lib/core/ipc/fake/` mit einer Rolle je Datei und hoechstens 500 Zeilen aufgeteilt, ohne dass einer der 30 Importeure angefasst wird |
| HUM-100 | `sandbox/profile.rs` traegt Modell und Pfadaufloesung zugleich | M | `profile/{mod,model,mounts,build}.rs` statt einer Datei mit 2190 Zeilen (Modell samt `SessionContext` 230-714, Einhaenge-Politik 718-1234, Laden und Argumentbau 1236-1795, Tests ab 1797); `mounts.rs` traegt allein die Entscheidung, welcher Pfad des Wirts in der Sandbox sichtbar wird (Symlinks, Schreibweisen, Socket-Suche) und sagt das in seinem Kopfkommentar; keine Verhaltensaenderung, keine umbenannten oeffentlichen Typen, keine Datei ueber 800 Zeilen, `git diff --stat` zeigt ausserhalb von `daemon/crates/sandbox/src/profile*` keine Rust-Aenderung, gleiche Zahl Testfaelle wie vorher (im Commit-Text genannt) und eine Mutationsprobe in `mounts.rs` macht einen Test rot |
| HUM-122 | Regel fuer einen Befehl aus fremdem Wert steht fuenfmal | L | Am 2026-09-06 gegen den Code gemessen: die Regel steht fuenfmal (`summary::copy_command`, `doctor::shell_command`, `proxy::ca::shell_quote`, `sandbox::bwrap_args::shell_quote`, `config::validate::shell_word`) in drei Lesarten -- die ersten beiden lehnen `'a'\''b'` ab, die letzten drei stellen es her, lassen `sanitize_note` aus und gehen ueber `to_string_lossy`, also wird ein Pfad ohne gueltiges UTF-8 ein anderer Pfad. Von 17 Stellen, die einen Befehl mit `format!` bauen, setzen zehn einen rohen Wert ein, darunter `sudo install -m 0755 {binary} /usr/local/bin/opencode` (`agent/opencode.rs:586`) und `chmod 700 {dir}` (`humanitld/main.rs:1110`, dieselbe Zeile, die HUM-075 anderswo behoben hat). Danach: eine beweisende Stelle, jede der zehn geht durch sie oder hat einen Typ, der die geschlossene Menge zusichert; die Anzeigefunktion von `bwrap_args` bleibt und heisst nach ihrer Aufgabe; kein Weg in einen `CopyCommand` mehr ueber `to_string_lossy`; die Mutationsprobe an der woertlichen Pruefung macht auf jeder behobenen Seite einen Test rot |
| HUM-123 | `IPC_005` heisst nach dem Regel-RPC und wird ueberall benutzt | S | Titel im Register wird „Anfrage ungueltig"; der Doc-Kommentar nennt alle neun Faelle mit Datei und Zeile (Operation, Regel, Regel-Id, kein Regelspeicher, Kennung, Pruefsumme, `BodyRef`, `order_by`, Cursor); `docs/DIAGNOSTICS.md` erzeugt mit unveraendertem Anker `#ipc_005`; `rulesRequestInvalid` heisst in Dart `requestInvalid`; ein Test prueft fuer jeden Code, dass sein Titel keinen RPC-Namen traegt, der nicht in seinem Doc-Kommentar steht |
| HUM-124 | Ein Lauf laesst eine Sandbox stehen, und Wartepunkte haben keine Frist | S | **Ursache am 2026-09-06 lokal gefunden:** `Inner::stop` meldet erst `Stopping` und toetet die Sandbox danach; `with_session` nahm vom Stop-Strom nur das erste Ereignis und liess ihn fallen, also scheiterte das naechste `tx.send` und `stop` kehrte vor dem Toeten zurueck. Die Sandboxen von `osc52_does_not_reach_host` und `osc8_and_title_are_inert` laufen mit `while :; do sleep 0.05; done` weiter, und die Testbinaerdatei blockierte beim Beenden im `waitpid` -- jeder Test gruen, danach ein Prozess, der nicht endet. Genau die Signatur der vier CI-Laeufe mit 1 628, 2 144, 2 757 und 13 784 Sekunden gegen 190 Sekunden im gruenen Fall; weil `ci.yml` `cancel-in-progress` setzt, enden sie als `cancelled` statt `failure` und sind in der Laufliste unsichtbar. Abnahme: gleiche Zahl `bwrap` vor und nach einem Lauf, dreimal wiederholt; jeder Stop-Strom bis zum Ende gelesen; kein Wartepunkt ohne Frist; eine abgelaufene Frist nennt Label, letzten Zustand und Ereigniszahl. `cargo-nextest` mit `terminate-after` steht als der allgemeine Weg im Issue, weil Fristen in einer Datei nur eine Datei binden |
| HUM-125 | Hinweiszeile erreicht nur ein Terminal, und erst nach der Pruefung | M | `attend` abonniert `HoldQueue` vor `check_isolation_or_kill` und leert erst nach dem Oeffnen des Terminals, also erzeugt auch ein frueh gehaltener Fluss seine Zeile; die Zeile geht zusaetzlich als `LogLine` in den `Sandbox`-Strom, damit `humanitl run` sie zeigt; `ui.terminal_notices` schaltet nur noch den Rahmen `TerminalOutput.notice` (seit dem 2026-09-07 steht die Zeile nicht mehr im Bytestrom, HUM-042) und `docs/CONFIG.md` sagt das; der leere Zustand des Log-Reiters nennt die Hinweiszeilen; `two_sessions_leave_nothing_behind` bleibt gruen |
| HUM-126 | M1 und M2 lesen ihre Bereitschaft aus einer Fifo ohne Frist | S | `e2e_wait_ready` und `e2e_ready_failed` stehen in `tests/e2e/lib.sh` und werden von M1, M2 und M3 gerufen; `m3_wait_ready` entfaellt; `grep -rn 'mkfifo' tests/e2e/` findet keinen Treffer; ein Server, der nicht hochkommt, beendet M1 und M2 innerhalb von 25 Sekunden mit seinem Protokoll im Text statt nach der Gesamtfrist der CI; ein Selbsttest deckt alle drei Rueckgabewerte und laeuft in `make check` |
| HUM-127 | Der Tray-Befehl gilt nur fuer apt, und dort aendert sich der Paketname | S | `TRAY_INSTALL_COMMAND` (`sandbox/src/doctor/checks.rs:45`) bietet jedem `sudo apt install libayatana-appindicator3-1` an; der Doc-Kommentar daneben sagt selbst, dass die Zeile fuer Debian und Ubuntu gilt. Anders als bei bubblewrap (HUM-044) unterscheidet sich nicht nur der Verwalter, sondern der **Paketname** je Distribution, also reicht die Abbildung aus `os_release.rs` nicht: sie bekommt eine Tabelle (Verwalter, Paketname). Jeder Name wird gegen den Paketindex der Distribution geprueft und die Quelle im Test genannt -- ein falscher Paketname ist schlechter als kein Befehl, weil er eine Fehlersuche in die Irre schickt. Wo kein Name belegt ist, entfaellt der `CopyCommand` und `DOCTOR_009` traegt nur den Satz plus `docs`. Abnahme: `grep -rn 'apt install' daemon/crates/sandbox/src` findet keinen fest verdrahteten Verwalter mehr, und ein Test zeigt fuer jede der vier Distributionsfamilien den erwarteten Befehl oder dessen begruendetes Fehlen |
| HUM-128 | Ein Stopp, den der Client nicht zu Ende hoert, toetet die Sandbox nicht | M | `Inner::stop` meldet erst `Stopping` und ruft `handle.terminate` danach; laesst der Client den Strom fallen, scheitert das naechste `tx.send` und `stop` kehrt vor dem Toeten zurueck. Der Mensch drueckt Stopp, schliesst das Fenster, und der Agent laeuft weiter, bis der Daemon endet -- `--die-with-parent` begrenzt den Schaden, nichts sonst. Aus HUM-124 gemessen: drei Testprozesse standen 45, 40 und 29 Minuten mit je zwei lebenden `bwrap`-Kindern. Danach: die Arbeit des Stopps gehoert dem Daemon, nicht dem RPC-Strom; ein verlorener Empfaenger beendet die Meldungen, nicht das Toeten. Abnahme: ein Stopp, dessen Strom nach dem ersten Ereignis fallen gelassen wird, beendet die Sandbox trotzdem, und ein Test zaehlt dabei die `bwrap`-Prozesse |
| HUM-129 | Der Daemon fragt den Agent-Adapter nie, ob der Start gelingen kann | M | `AgentAdapter::preflight` liefert `AGENT_001` und `AGENT_004`; der CLI-Weg ruft es (`cmd/sandbox.rs:663`), der Daemon-Weg nicht (`ipc/src/sandbox.rs:1944-1946` ruft nur `command`, `env`, `files`). Das trifft den Normalfall: `AGENT_001` schlaegt den Installationsbefehl von OpenCode vor, der nach `$HOME` legt, und das mitgelieferte Profil haengt `$HOME` nicht ein. Am 2026-09-06 mit echtem Daemon gemessen: `humanitl run` endet mit `exit 127, humanitl-shim: exec failed`, `grep -c AGENT_004` findet null -- waehrend `humanitl sandbox argv` fuer denselben Zustand `blocking[AGENT_004]` samt Vorschlag zeigt. Abnahme: der Aufruf steht an genau einer Stelle, die beide Wege erreichen; `Sandbox(Plan)` und `Sandbox(Start)` liefern dieselben Befunde; die Mutationsprobe macht den Test rot |
| HUM-130 | Ein ignoriertes SIGINT des Starters erreicht den Agenten | M | `execve` setzt einen Handler zurueck, behaelt aber `SIG_IGN`: Ein ignoriertes `SIGINT` wandert vom Starter (Hintergrundjob ohne Job-Control, `nohup`, Dienst) durch `bwrap` und Shim bis zum Agenten, der es nach POSIX nicht mehr trappen kann; aus der Bitte, selbst aufzuhoeren, wird nach fuenf Sekunden der Schlag mit 130. Am 2026-09-06 gemessen: 300 ms nach dem Signal stehen `bwrap`, Shim, `sh` und `sleep` unveraendert in `ps`, `sigint_reaches_the_agent_and_keeps_its_exit_code` war aus diesem Kontext acht von acht Laeufen rot und allein im Terminal in 50 ms gruen; ohne unseren Code mit blossem `bwrap` nachgestellt. Zweites Loch derselben Stelle: die `SIGINT`-Quelle der Kommandozeile wurde erst nach dem Start der Sandbox angemeldet, ein Signal in diesem Fenster verschwand spurlos. Abnahme: der Shim setzt im Kind jede Disposition auf `SIG_DFL`, die Kommandozeile meldet ihre Quelle vor dem Start an, ein Test startet ueber `trap '' INT` und verlangt Exit 42 samt freier Ignoriermaske, Mutationsprobe fuer beide Schichten zugleich |
| HUM-131 | Die Aufzeichnung kennt ein Alter, aber keine Menge | M | Begrenzt sind heute einzelne Zeilen (`limits.recorder_max_body_bytes` 33 554 432, `recorder.inline_max_bytes` 262 144) und das Alter (`recorder.retention_days` 180 seit HUM-051, taeglich ueber `purge_daily`), nicht die Summe: keine Obergrenze in Bytes, keine Verdraengung des Aeltesten, keine Untergrenze fuer den freien Platz. Ein Agent, der einen Tag lang grosse Antworten zieht, fuellt bis zu 180 Tage lang, bevor ueberhaupt geloescht wird. Abnahme: `recorder.max_total_bytes` (Vorgabe 2 GiB) verdraengt das Aelteste, `recorder.keep_free_bytes` (Vorgabe 1 GiB) laesst Ruempfe weg statt die Platte zu fuellen und meldet das einmal je Sitzung, beide im Register `effective`, die gemessene Groesse einer Sitzung steht im Commit-Body |
| HUM-132 | Der Entwicklungsbaum schreibt die Platte voll | S | `daemon/target` lag am 2026-09-06 bei 103 GiB, und `cargo clean` gab 107,9 GiB frei; Cargo raeumt alte Artefakte nie von selbst weg. Gemessen auf demselben Stand, jeweils nach `cargo clean` und `cargo build --workspace --all-targets`: 13 GiB mit `debug = 2` (Vorgabe), 6,1 GiB mit `debug = "line-tables-only"`. Abnahme: der Schluessel steht im Workspace mit beiden Zahlen als Begruendung, `make check` mit `STRICT=1` bleibt gruen, ein Backtrace zeigt weiter Datei und Zeile, und `CONTRIBUTING.md` nennt Groessenordnung und Aufraeumbefehl |
| HUM-133 | Ein roter Testlauf in CI nennt seinen Test nicht | S | Drei rote `rust-test`-Laeufe am 2026-09-06 (`3f293f2`, `5ee92ba`, `9bc3717`) tragen als einzige Annotation `Process completed with exit code 2`; die Job-Logs verlangen Schreibrechte (403), und lokal ist der Fehlschlag nicht zu erzeugen (dreimal `cargo test --workspace` auf zwei Kernen, einmal mit den Clients der Konformitaets-Matrix, alles gruen). Abnahme: ein roter Lauf traegt je gefallenem Test eine `::error::`-Annotation mit dem Namen, die volle Ausgabe liegt als Artefakt am Lauf, und `scripts/ci/test-report.sh --self-test` haelt die Auswertung fest |

---

## 9. Nach dem MVP (priorisiert)

1. **Austauschbares Sandbox-Backend, direkt nach Sprint 5.** Wunsch des Nutzers vom 2026-09-23: Die Sandbox unter Humanitl soll sich nach bester Software-Engineering-Art austauschen lassen, nicht nur bwrap, sondern auch Incus (System-Container und VMs über die REST-API von `incusd`), microsandbox (libkrun, Punkt 12) und künftige Backends wie Docker/Podman (Punkt 2) oder Seatbelt (Punkt 11). Ziel ist ein Port, hinter dem jedes Backend ein Adapter ist, ohne dass Daemon, IPC, App oder CLI wissen, welches läuft. Umfang:
   - `SandboxBackend` und `LaunchPlan` werden backend-neutral: Der Plan beschreibt, was gelten soll (Mounts mit Modus, Umgebung, Netz nur über die Tür, seccomp-Politik, Ressourcengrenzen, Terminal, Bericht), nicht wie bwrap es ausdrückt. Die lesbare Policy im Isolation-Panel entsteht aus diesem Plan statt aus der bwrap-Kommandozeile.
   - Die Tür zum Proxy wird ein Begriff mit Varianten (Unix-Socket-Datei, vsock-Port, Container-Netz mit einziger Route), und der Shim beziehungsweise sein Gegenstück im Gast bleibt der einzige Weg nach draußen.
   - Die drei Sandbox-Garantien werden je Backend formuliert und von denselben ESC-Tests geprüft; ein Backend ist erst zulässig, wenn `tests/escape/` gegen es grün ist. `humanitl doctor` und `isolation_check` fragen das gewählte Backend.
   - Auswahl über `sandbox.backend` in `config.toml` und im Profil, Vorgabe bleibt bwrap; ein Backend, dessen Voraussetzungen fehlen (`/dev/kvm`, `incusd`), meldet das als `Diagnostic` mit `why` und `fix`.
   - ADR mit mindestens zwei konkreten Adaptern (bwrap und einer von Incus oder microsandbox), Nachträge zu ADR-0002, ADR-0010 und ADR-0015, `docs/SECURITY.md` und `docs/THREAT-MODEL.md` je Backend. Keine Abstraktion über Fremdbibliotheken hinaus, nur der eine Port.
   - Bis dahin gilt HUM-145: Die Kopplung an bwrap (`tools/coupling-baseline.toml`) darf nur sinken, und neuer Code spricht die Sandbox nur über `SandboxBackend` an. Erster Schritt nach Sprint 5 ist ein Spike, der Incus und microsandbox gegen die drei Garantien misst und die Schnittstelle des Ports festlegt.
2. **M6 Docker-Backend + HTTP/2-Upstream.** Zweite `SandboxBackend`-Impl (`--network none` + UDS + eigenes seccomp-JSON), Image-Layering `humanitl/base → agent-opencode → packs`, Cache-Volumes pro Projekt, UID-Mapping. h2 zum Upstream aus `[experimental]` heben.
3. **M7 Browser.** Issues: HUM-080 Sandbox-Profil `browser` (Chromium-Flags, SPKI-Hash der CA, `AF_UNIX` erlaubt, `humanitl_browser.py` im Pack, Briefing-Zusatz); HUM-081 Reverse-Bridge im Shim und `cdp.sock`; HUM-082 CDP-Client im Daemon (`chromiumoxide`, Screencast, Input, Tab-Liste); HUM-083 gRPC `Browser`-Bidi-Stream (Frame JPEG + Metadaten, Input-Events, Takeover-Flag); HUM-084 Browser-Tab im UI (Frame als `Image.memory`, Klick- und Tastatur-Weiterleitung, Übernahme-Modus mit sichtbarem Rahmen, Tab-Wechsel, „zurück an den Agenten"); HUM-085 Screenshot-Vorschau für das Domain-Panel über denselben Browser (ersetzt M10-Teil). Fallstricke: Chromium braucht `--no-sandbox` in bwrap; Screencast-Frames drosseln (max 10 fps, JPEG 60); Tastatur-Layouts in `dispatchKeyEvent`; nodriver-Versionen pinnen.
4. **M8 Plugin-API.** Externe Detektoren und Aktionen über gRPC, WASM-Detektoren, Nutzer-Katalog, Panel-Slots. Erst wenn drei echte Bedarfe dokumentiert sind.
5. **M9 Response-Moderation.** Rücktausch auch in gestreamten Antworten (Token-Grenzen über Chunk-Puffer), optional Responses halten (`ask` auf Response-Ebene). Dazu der einfache Rücktausch für nicht-gestreamte Text-Antworten (HUM-079), am 2026-09-18 aus dem MVP hierher verschoben.
6. **M10 Domain-Vorschau Live.** Nutzer-getriggerter Favicon/og:title-Fetch host-seitig mit Limits; Screenshot via Playwright in eigener netzwerkbeschränkter Sandbox, als Bild ins UI.
7. **M11 Credential-Injection.** Echte Tokens (GitHub, Registries) nur im Proxy, Sandbox sieht Platzhalter; Push nur auf konfigurierten Branch.
8. **OpenCode-Permission-Bridge.** `opencode serve` SSE + `POST /session/:id/permissions/:id`, OpenCodes eigene Tool-Prompts nativ in Flutter rendern. Weitere Agent-Adapter: Aider, Codex (`--oss`), Claude Code (via `ANTHROPIC_BASE_URL`).
9. **Einrichtungsassistent und freie Wahl des Agenten.** Beim ersten Start führt ein Assistent durch die Einrichtung: Sandbox-Voraussetzungen, Modell-Endpunkt, und welcher Agent in der Sandbox läuft. Zur Wahl stehen OpenCode, pi (https://pi.dev/), Claude Code und ein ganz eigenes Kommando; OpenCode und pi kommen mit einer kleinen Auswahl sinnvoller, vorinstallierter Erweiterungen. Wer seine Einstellungen schon mitbringt, überspringt den Assistenten: eine Konfigurationsdatei, die in den eigenen Dotfiles liegt, beantwortet alle Fragen im Voraus, und der Assistent erscheint dann gar nicht. Baut auf dem `AgentAdapter` aus HUM-037 auf (jeder Agent ein Adapter mit Profil, Standardregeln und Env-Kit) und auf dem generierten Settings-Screen (HUM-069); die Dotfile-Konfiguration ist dieselbe `config.toml` wie heute, nur vollständig genug, dass nichts mehr gefragt werden muss. Wunsch des Nutzers vom 2026-09-18.
10. **WebSocket-Frame-Hold.** Frames anhalten statt nur aufzeichnen.
11. **M12 macOS.** Seatbelt-Backend (Muster sandbox-runtime), gleicher Daemon, gleiche Proto.
12. **M13 microVM.** Firecracker/Kata oder Docker Sandboxes mit vsock-only, für stärkeres Threat Model. Vorgemerkter Kandidat seit 2026-09-11: microsandbox (https://github.com/superradcompany/microsandbox, libkrun, Apache-2.0) als zweites, zuschaltbares Backend neben bwrap, nicht als Ersatz. Stand der Prüfung am 2026-09-11 (v0.6.18): noch Beta, braucht `/dev/kvm` ohne Ausweichweg, die Tür wird ein vsock-Port statt einer Socket-Datei, und der Konsolenkanal zu `agentd` bleibt immer zusätzlich offen, so dass Garantie 2 neu formuliert werden müsste. Geschätzt 45 bis 70 Personentage, davon 9 bis 14 für einen neutralen Port. Voraussetzung ist, dass `SandboxBackend` und `LaunchPlan` backend-neutral werden (die lesbare Policy kommt dann aus einer Policy-Tabelle statt aus der bwrap-Kommandozeile), mit Nachträgen zu ADR-0002, ADR-0010 und ADR-0015; bis dahin hält HUM-145 fest, dass die Kopplung an bwrap nicht wächst.
13. **Upstream-Proxy und Tor.** `Egress`-Adapter `HttpProxy` und `Socks5h` (`tokio-socks`), Config `egress.via`, Regel-Feld `via`, Tor-Check im Isolation-Panel, Leak-Test: kein lokaler DNS-Lookup im Tor-Modus. Nice-to-have.
14. **Flatpak.** UI im Flatpak, Daemon außerhalb.
15. **OpenTelemetry** hinter Cargo-Feature, Team-Kataloge, Regel-Profile teilen.
16. **Pseudonym-Mapping dauerhaft (HUM-048).** Verschlüsselte Zuordnungstabelle im Daemon, Schlüssel im System-Keyring, Mapping-Panel in Editor und History, verschlüsselter Export, dazu das Popover am Diff-Glow mit Aktionen je Fund und „Immer ignorieren" samt Allowlist aus HUM-049. Am 2026-09-18 aus dem MVP hierher verschoben; im MVP lebt die Zuordnung je Sitzung im Speicher.
17. **Settings-Screen vollständig (HUM-069).** Formular aus dem JSON-Schema für alle Felder mit Gruppen, Detailstufen, Suche, Herkunftsanzeige und Zurücksetzen, Live-Neuladen mit `ConfigChanged`, Dateiüberwachung von `config.toml`, Audit-Eintrag je Änderung, Ladebefunde im Formular. Am 2026-09-18 aus dem MVP hierher verschoben; im MVP gibt es ein handgebautes Formular mit neun Einstellungen, wirksam nach Daemon-Neustart.
18. **Englisch als Projektsprache.** Wunsch des Nutzers vom 2026-09-24: Das Projekt wird später vollständig auf Englisch umgestellt, also Dokumente (`docs/`, `BACKLOG.md`, Sprint-Files, ADRs), Kommentare und Doc-Kommentare, `CLAUDE.md`, `AGENTS.md` und `CONVENTIONS.md`. Die Regel „Prosa Deutsch, Bezeichner Englisch" in `CLAUDE.md` gilt bis dahin weiter. Englisch sind schon heute Bezeichner, Commit-Texte, die `en`-Quelle der ARB-Dateien und ab HUM-225 die Release-Notes. Umfang und Zeitpunkt legt ein eigenes Issue fest; mechanisch übersetzen allein reicht nicht, weil Querverweise wie „Abschnitt 4.13" und die Kästchen der Sprint-Files ihren Sinn behalten müssen.

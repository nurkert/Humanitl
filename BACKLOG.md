...
| HUM-124 | Terminal-Tests warten ohne Frist auf den Start | S | `running_with` und `with_session` in `daemon/crates/ipc/tests/terminal.rs` warten ueber einen Helfer `within` mit Frist statt unbegrenzt; eine abgelaufene Frist nennt Label, letzten Zustand und Ereigniszahl; ein `Failed`-Start ueberspringt weiterhin mit `SKIP_MARKER`. Gemessen 2026-09-06: der CI-Schritt `rust-test` laeuft gruen in 184 bis 194 Sekunden und stand an zwei Tagen 2 144 beziehungsweise 13 784 Sekunden, einmal ueber eine Stunde auf einem reinen Backlog-Commit bei zehn gruenen Nachbarjobs. Weil `ci.yml` `cancel-in-progress: true` setzt, endet so ein Lauf als `cancelled` und nicht als `failure` -- er ist in der Laufliste unsichtbar, wenn man nicht die Dauer danebenlegt. Lokal reproduziert er nicht (`cargo test --workspace`: 90 Suiten gruen, `terminal.rs` sechs Tests in 1,35 s ohne `ESC5-SKIP`), also ist die Enge der CI der Ausloeser. Die Frist behebt den Haenger nicht, sie macht ihn sichtbar: aus drei Stunden Stillstand wird ein roter Test, der sagt, worauf er gewartet hat |

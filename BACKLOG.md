...
| HUM-122 | Regel fuer einen Befehl aus fremdem Wert steht zweimal | S | `daemon/crates/sandbox/src/shell.rs` traegt die vier Bedingungen (nicht leer, `sanitize_note` aendert nichts, Zitierung woertlich, `shlex::split` ergibt dieselben Woerter) genau einmal; `summary::copy_command` und `doctor::shell_command` bestehen aus hoechstens drei Zeilen und rufen `shell::command_line`; `grep -rn 'fn is_literal_word' daemon/crates` findet einen Treffer, `grep -rn 'try_quote' daemon/crates` ausserhalb von `shell.rs` keinen; ein Gleichheitstest ueber dreissig Pfade zeigt dasselbe Ergebnis wie vorher, und die Mutationsprobe an `is_literal_word` macht auf beiden Seiten je einen Test rot |

...
//! ergibt: ein Argument je Zeile. Wer die Reihenfolge ändert, sieht es hier und
//! muss die Änderung bewusst übernehmen:
//!
//! ```text
...
use humanitl_sandbox::{Namespace, SandboxProfile, SessionContext};
...
fn bwrap_args_snapshot_default() {
    let args = strings(&profile("default").to_bwrap_args(&context(WorkMode::Rw)));
    for arg in &args {
        assert!(
            !arg.contains('\n'),
...
fn work_ro_uses_ro_bind() {
    let args = strings(&profile("default").to_bwrap_args(&context(WorkMode::Ro)));
    let at = index_of(&args, "/home/u/proj");
...
fn the_hard_floor_is_in_every_shipped_profile() {
    for name in ["default", "test"] {
        let args = strings(&profile(name).to_bwrap_args(&context(WorkMode::Rw)));
        for flag in [
            "--unshare-user",
            "--unshare-pid",
            "--unshare-net",
...
fn user_extras_come_before_everything_the_session_mounts() {
    let args = strings(&profile("test").to_bwrap_args(&context(WorkMode::Rw)));
...
fn extra_rw_renders_as_bind_with_the_same_path_twice() {
    ...
    let args = strings(&profile.to_bwrap_args(&context(WorkMode::Ro)));
...
fn mandatory_masks_survive_an_empty_masked_files() {
    ...
    for masked in humanitl_sandbox::MANDATORY_MASKED_FILES {
        let at = window_at(&args, &["--ro-bind", "/dev/null", masked])
            .unwrap_or_else(|| panic!("{masked} is not masked: {args:?}"));
...
fn argv_line_is_shell_parsable() {
    ...
    let line = profile.argv_line(&ctx);
    ...
    assert_eq!(split, strings(&profile.to_bwrap_args(&ctx)));
}

...
- [x] `cargo build --workspace` grün: `daemon/bin/humanitl` und `daemon/crates/ipc` übersetzen ohne das Feld. **Gemessen am 2026-09-11** mit `cargo clippy --workspace --all-targets -- -D warnings`, das alles übersetzt. Dafür waren mehr Stellen nötig, als die Spezifikation kannte: drei weitere Setzer in `daemon/bin/humanitl/src/cmd/moderate.rs` (Übersetzungsfehler `E0560`) und vier Literale in `rules_rpc.rs` und `fake_parity.rs`, deren `..v1::DecideRequest::default()` nach dem Wegfall nichts mehr bewirkte (clippy `needless_update`).
...
- [x] `git diff --stat` berührt keine Datei unter `daemon/crates/proxy/src/`, `daemon/crates/recorder/src/`, `daemon/crates/config/src/`, `app/lib/features/` oder `app/l10n/`. **Gemessen am 2026-09-11**: elf Dateien, keine davon in diesen Bereichen.
- [x] `grep -n "acknowledged_findings = 8" backlog/sprint-4.md` und `grep -n "ignore_always = 9" backlog/sprint-4.md` treffen je einmal; im `DecideRequest`-Entwurf von HUM-049 kommt Nummer 6 nicht mehr vor. **Gemessen am 2026-09-11.**

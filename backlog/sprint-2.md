...
| 17 | HUM-089 | acknowledge_findings wird nie gelesen | S | Proto/IPC |
| 18 | HUM-091 | FlowSummary traegt keine registrierbare Domaene | L | Daemon + UI |
...
3. Sonst `ips = resolver.resolve(host).await`; leer oder Fehler → `Decided(Block{NoRoute})` (Flow war bereits `Decided(Allow)`; der Übergang `Decided(Allow) → Decided(Block)` ist verboten, daher: Zustand bleibt `Decided(Allow)`, es wird `Responded{status: 502}` mit synthetischer Antwort und Diagnostic PROXY_004 (`why` = „DNS lookup for `host` failed: …") am Flow gesetzt, dann `Recorded`). **Gebaut ist es anders, am 2026-09-12 am Code nachgesehen:** Der Flow endet in `Failed { error: UpstreamError::Dns }` (`daemon/crates/proxy/src/upstream.rs:246`, `daemon/crates/proxy/src/handler.rs:1444`), der Client bekommt 502 mit `reason: upstream_dns` (`daemon/crates/core-types/src/flow.rs:209`), und ein eigener Befund entsteht auf diesem Weg nicht. Die Nummer `PROXY_004` wurde nie vergeben: `daemon/crates/core-types/src/diagnostics/codes.rs` führt `PROXY_001` bis `PROXY_003`, `PROXY_005` und `PROXY_007` bis `PROXY_009`, und `backlog/sprint-3.md` hält 4 und 6 ausdrücklich als frei fest. Gemessen wird der Weg von `a_name_that_does_not_resolve_ends_as_upstream_dns`.
...
- `rebinding_to_loopback_rejected`: Mock antwortet `127.0.0.1` für `evil.example`, Regel allow → 502 mit PROXY_004, keine TCP-Verbindung (Fake-Upstream auf 127.0.0.1 zählt 0 Verbindungen). **Am 2026-09-12 am Code nachgesehen:** Gebaut ist er so in `daemon/crates/proxy/tests/dns_after_allow.rs:340`; die Antwort trägt `reason: upstream_private_address`, der Befund dazu heißt `PROXY_008` (`daemon/crates/proxy/src/handler.rs:1790`), und `PROXY_004` wurde nie vergeben.
...
- [x] Alle Tests oben grün. (Die sechs oben genannten stehen in `dns_after_allow.rs`. Am 2026-09-12 über die Attributzeilen gezählt: 21 Tests, 19 `#[tokio::test(flavor = "multi_thread")]` und 2 `#[test]`, keines `#[ignore]`. Die 15 ist der Stand vor HUM-115, `5c61f96` hat sechs Tests für den hickory-Adapter ergänzt.)
- [x] `esc-3.sh` grün: `dns.log` enthält keinen geblockten Host, genau einen erlaubten. (Seit HUM-115, unter anderen Namen als hier vorgesehen: Der Stub ist `tests/escape/dns-stub.py` statt eines Rust-Binarys `dns-recorder.rs`, die Anfragen stehen in `esc-3-egress.sh`, die Prüfung in `tests/escape/dns-proof.sh`; `HUMANITL_RESOLVER__NAMESERVER` wirkt über `HickoryResolver`. Gemessen 2026-09-11: `dns.log` enthält keinen der Namen, die ESC-3 ohne Freigabe schickt, und genau eine Frage nach `allowed.esc3.test`, nach der Freigabe.)
- [x] Config-Schema enthält die fünf `resolver.*`/`upstream.*`-Schlüssel mit Tier `expert` und Beschreibung. (Gemessen 2026-09-12 an `daemon/crates/config/tests/fixtures/config.schema.json` und `docs/CONFIG.md`: fünf `resolver.*`-Schlüssel — `nameserver`, `overrides`, `cache_ttl_secs`, `prefer`, `test_ca` — je mit `x-tier: expert` und Beschreibung. Der Timeout steht unter seinem kanonischen Namen `limits.connect_timeout_secs` (Tier `advanced`); `upstream.connect_timeout_secs` bleibt als Alias gültig (`daemon/crates/config/src/alias.rs`, CONVENTIONS 4.4), die Spezifikation schrieb ihn vor der Umbenennung. Die Start-Warnung für `resolver.overrides`, die CONVENTIONS 4.22 verlangt, ist gebaut: `CONFIG_016` auf Stufe `WARN`, mit Zahl und Namen der Tabelle, gemessen von `a_table_of_fixed_names_is_announced_at_the_start` und `without_fixed_names_the_start_stays_quiet_about_them` in `daemon/bin/humanitld/tests/daemon_end_to_end.rs` sowie zwei Unit-Tests über `announce_overrides`. Mutationen: Aufruf im Start gelöscht, Namen aus der Zeile gestrichen, Leerprüfung entfernt — je rot.)
...
- [x] `list_flows` mit 100k Zeilen und `host:`-Filter antwortet in < 50 ms (Test mit generierten Daten, Zahl in PR). (Die einzige aufgezeichnete Messung steht im Kopfkommentar von `daemon/crates/recorder/tests/list_flows_scale.rs`, gemessen am 2026-09-03 im Release-Profil: 3,5 ms für `host:github.com`, 0,33 ms für einen seltenen Host; im Debug-Profil 45,3 ms, wo die Schranke 200 ms ist, weil `libsqlite3-sys` dort mit `-O0` übersetzt wird. Die früher hier genannten 8,4 ms haben keine Quelle; am 2026-09-12 in Quelltext, Backlog und Doku nicht auffindbar. Der Test ist `#[ignore]`, und kein CI-Schritt übergibt `--ignored` oder `--release`; dauerhaft abgesichert sind nur die Abfragepläne, über das nicht ignorierte `daemon/crates/recorder/tests/query_plan.rs`.)
- [x] Kein `unwrap()` im Writer-Thread; Fehler landen als Diagnostic RECORDER_003 im `diagnosticsProvider`-Stream, der Thread stirbt nicht. **Beide Hälften am 2026-09-12 am Code nachgesehen.** Daemon: kein `unwrap` außerhalb von Tests, `RECORDER_003` wird in `daemon/crates/recorder/src/error.rs:129` gebaut und von `Writer::report` (`daemon/crates/recorder/src/writer.rs:427`) verschickt, die Schleife in `writer.rs:294` lebt weiter; `report_diagnostics` reicht `recorder.diagnostics()` als `FlowEvent::Diagnostic` in den Ereignisstrom (`daemon/bin/humanitld/src/main.rs:557-560`). Oberfläche: seit HUM-106 (`929265f`) faltet `app/lib/features/intercept/providers/diagnostics.dart:123` die Variante in den `diagnosticsProvider`, den `app/lib/features/intercept/widgets/diagnostic_card.dart:93` anzeigt. `app/lib/features/intercept/providers/flows.dart:175`, `app/lib/features/history/providers/history_page.dart:560` und der Fake lassen sie weiter liegen, mit Absicht: sie ändert keinen Fluss. Die eigene Karte für Aufzeichnungsfehler kommt mit HUM-058. (Der frühere Zeilenverweis `writer.rs:396` und die Aussage, es gebe keinen `diagnosticsProvider`, sind überholt.)
...
### Akzeptanzkriterien
- [x] `humanitl rules test https://evil.example` mit Default-Regeln → `verdict: ask`, Exit 11. **Eingelöst mit HUM-114 am 2026-09-07**: Das Kommando ruft `RulesRequest.Test`, druckt `verdict: ask` mit `rule: none (default ask)` und endet 11; `test_not_yet` und die zwei Sätze in `--help` sind weg.
- [x] `esc-4.sh` nutzt `humanitl rules test` für die Tabelle aus HUM-022 (Auszug: Fälle 1–15) und ist grün. **Eingelöst mit HUM-114 am 2026-09-07**: `rules_cli_01` bis `rules_cli_15` in `esc-4-rules.sh` fahren die ersten fünfzehn Zeilen der Host-Tabelle über die Kommandozeile gegen den Daemon des Laufs, alle grün; die sieben Engine-Fälle und die Body-Cap-Probe bleiben daneben stehen, damit dieselbe Frage zweimal beantwortet wird.
...
### Akzeptanzkriterien
- [x] Alle Tests und Goldens grün (alchemist CI-Modus). (272 Tests in `test/features/intercept`, Stand 2026-09-05; am 2026-09-12 sind es dort 298, gezählt über die Zeilenanfänge `test(`, `testWidgets(` und `testGoldens(`, davon 109 in `test/features/intercept/body/`. Das Kriterium verlangt Greenness, keine feste Zahl. 9 Goldens der Aktionsleiste, `action_bar_*` in `app/test/goldens/goldens/ci/`, am 2026-09-12 nachgezählt; Greenness selbst ist ohne Lauf nicht prüfbar.)
...
- [x] Hit-Targets: jedes klickbare Element ≥ 28 px hoch (Widget-Test prüft `Size`). **Für die Aktionsleiste seit dem 2026-09-07 gemessen** -- der Satz gilt für sie und nicht für den ganzen Bildschirm, und was außerhalb noch ungemessen ist, steht unten. Das Entscheidungspaar steht wie bisher in `release_valve_test.dart` gegen `HSize.hitDecision` (32 mal 120); für das Nebensächliche misst `app/test/features/intercept/hit_targets_test.dart` die Fläche im Baum statt der Zahl in den Constraints: alle **acht** Zellen beider Remember-Gruppen mit den **ausgelieferten** Beschriftungen, der Chevron der Ventilzeile, dieselben acht Zellen ausgegraut (ein Ziel, das unter dem Finger schrumpft, sobald eine Entscheidung läuft, wäre schlimmer als ein zu kleines), ein Raster **ohne Wörter** für die Breite und ein Tippen, das ankommt. Die letzten beiden kommen aus dem Review und schließen zwei Lücken, die die erste Fassung hatte: Mit den echten Beschriftungen ist die schmalste Zelle schon 53 px breit, `minWidth` trug also nichts; und rein geometrische Messungen blieben grün, als `onTap` auf `null` stand. Mutationsproben, jede einzeln gemessen: `minHeight` auf 12 (zwei Tests rot), `minWidth` allein auf 12 (der Test ohne Wörter rot -- vor dem Review blieben alle grün), `valveChevronWidth` auf 12 (Chevron-Test rot), `onTap: null` (der Tipp-Test rot). **Was außerhalb der Aktionsleiste ungemessen bleibt**, ebenfalls aus dem Review: der Augen-Schalter in `section_headers.dart:141` misst 28 mal 19,2 und steht als bekannter Verstoß in `docs/UX.md:487`; dazu der Block-Platz der Zeile (`queue_row.dart:338`), Chevron und Block-Platz der Gruppenzeile (`group_header_row.dart:187`, `:246`) und die Aufklappzeile des JSON-Baums (`json_tree_view.dart:327`, `HSize.rowBody` = 24). Und ein Fund, den dieser Test **nicht** sieht, weil er Rechtecke misst und nicht den Raum dazwischen: Zwischen zwei Segmenten liegt eine 4 px breite tote Zone, weil `FocusRing` seine Ringreserve außerhalb des `GestureDetector` legt; ein Klick dort verschwindet lautlos. Gemessen im Review mit drei Taps, aufgeschrieben als HUM-143.
...
### Akzeptanzkriterien
- [x] Tests und Goldens grün. **Beide Hälften gemessen am 2026-09-12**, nachdem HUM-094 die Oberflächen-Hälfte gebaut hat. Daemon: `cargo test -p humanitl-catalog` meldet 72 bestandene Tests über fünf Binaries (22, 11, 13, 26, 0), keinen Fehlschlag. Oberfläche: `flutter test test/features/intercept/domain_panel_test.dart test/features/intercept/catalog_test.dart test/goldens/domain_panel_golden_test.dart` endet mit 26 bestandenen Tests, darunter `quick_rule_calls_rules_add`; die sechs Goldens `domain_panel_{known,unknown,session_summary}_{light,dark}` liegen unter `app/test/goldens/goldens/ci/`. `DomainPanePlaceholder` ist gelöscht.
...
- [x] Kein Netzwerkzugriff aus `humanitl-catalog` (Crate hat keine Abhängigkeit auf hyper/reqwest/tokio-net; `cargo tree` in CI geprüft). **Gebaut und gemessen am 2026-09-11.** Statt `cargo tree` liest `tools/check_offline.py` den aufgelösten Graphen aus `cargo metadata`, mit allen Features und gefiltert auf die Plattform, auf der gebaut wird, und prüft für jedes Crate aus `tools/offline-crates.toml` zweierlei: Unter seinen normalen Abhängigkeiten, auch transitiv, steht kein Crate der Netzliste (27 Einträge: Laufzeiten und Sockets, HTTP, WebSocket, TLS, DNS, darunter `tokio` und `tokio-net`), gemeldet mit dem kürzesten Pfad; und in seinen eigenen Quellen steht keine Netz-API der Standardbibliothek und kein roher Socket über `libc`, denn dafür braucht es gar kein Crate. Dev- und Build-Abhängigkeiten zählen nicht. Die Prüfung hängt in `tools/check-deps.sh` und damit in `make deps-lint`, das der CI-Job `deps-lint` fährt. Heute: „humanitl-catalog free of 27 network crates and of network APIs in its source". Gegenproben: `tokio` als normale Abhängigkeit von `humanitl-catalog` macht sie rot (`tokio`, `mio`, `socket2` mit Pfad); in `tools/tests/check_offline_test.py` (17 Tests, die Kanten so, wie cargo sie schreibt) werden rot: ohne Unterscheidung der Kanten drei, ohne transitive Suche einer, ohne Quelltextprüfung einer, mit einer Suche in die Tiefe statt in die Breite der Test auf den kürzesten Pfad, mit `all` statt `any` über die Kantenarten die Kante normal und dev zugleich, ohne Kanten mit Plattformbedingung die `cfg(unix)`-Kante, mit dem extern-Namen statt der Paketkennung drei (darunter `hyper-util` als `hyper_util`), mit nur `lib.rs` statt aller Quellen der Socket in einem Untermodul. Ein `IpAddr` bleibt erlaubt, eine Namensauflösung über `to_socket_addrs` nicht. Grundlage im Review: Antigravity (zuerst fehlten `std::net` ohne jedes Crate, der Plattformfilter und `tokio-net` beim Namen) und der Ersatz-Review für Codex (ohne `--all-features` blieb ein optionales Crate hinter einem Feature unsichtbar, und die Tests fingen keine Mutation der Kantenlogik). **Die Vorgeschichte:** Die Sache stimmte vorher schon (`cargo tree` ohne hyper, reqwest, tokio), nur die CI-Prüfung fehlte, und `tools/deps-allow.toml` beschränkt nur interne Crates.
- [x] Lizenzhinweis in `catalog/RANKS-LICENSE` und in der About-Ansicht (ARB `aboutRanks`). Der Palettenbefehl „About Humanitl“ öffnet Flutters `AboutDialog` mit Name, Version, GPL-3.0-only und `aboutRanks`; dessen Knopf führt auf Flutters Lizenzseite, auf der die Rangliste unter „Majestic Million“ im Wortlaut von `catalog/RANKS-LICENSE` steht. Die gebündelte Kopie `app/assets/catalog/RANKS-LICENSE` hält `app/test/features/about/licences_test.dart` Byte für Byte gegen das Original, den Dialog `about_test.dart` daneben. **Gebaut und gemessen am 2026-09-11.** Fünf Mutationen, jede rot: ein angehängtes Byte in der Kopie, eine Umschreibung statt des Wortlauts, ein fehlendes `addLicense`, ein Palettenbefehl, der nichts tut, und ein Dialog ohne `aboutRanks`. Damit ein Dialog und die Lizenzseite überhaupt öffnen können, hat die Anwendung jetzt einen Navigator an der Wurzel (`app/lib/app.dart`); die ganze Suite und die Goldens blieben dabei grün. Grundlage im Review: Antigravity (Thema und Texte wurden vor dem Dialog gelesen und zogen bei einem Wechsel der Helligkeit nicht nach), Codex ohne Befund.
...
- [x] Eine Regel, die im Intercept-Bildschirm über Remember entsteht (`RememberDuration.session`, `RememberTarget.host`), trägt `createdFrom` mit der Id der Anfrage, steht im Tab „Temporary" und zeigt dort das Abzeichen `rulesOriginFlow` mit dem letzten Segment dieser Id; ein Klick darauf führt über `flowHandoffProvider` zurück in die Warteschlange, auf genau diese Anfrage (`navigationProvider` auf `Section.intercept`, `selectedFlowIdProvider` auf die Flow-Id). Nicht `from #n` und nicht das History-Detail-Blatt: Eine Anfrage hat keine laufende Nummer, und die Übergabe geht in die Queue (CONVENTIONS 4.16). (Gebaut und gemessen am 2026-09-12 als `app/test/features/rules/rule_origin_round_trip_test.dart`, der den Rundlauf über die ganze Anwendung fährt, weil nicht das Abzeichen die Übergabe ausführt, sondern die Shell. `flutter test test/features/rules/rule_origin_round_trip_test.dart --concurrency=2`: 1 von 1 grün; in der ganzen Suite `flutter test --concurrency=2` 1031 grün und 4 übersprungen, `app/packages/ui` 129 grün, `flutter analyze` und `dart format --set-exit-if-changed` ohne Befund. Zwei Mutationen, jede rot: Lässt `_Origin` in `rule_row.dart` das Abzeichen auch mit `createdFrom` weg, fehlt der Schlüssel `rule-origin-018f0002-0000-7000-8000-000000000001`; streicht `_takeHandoff` in `shell_screen.dart` das `select(next)`, bleibt `selectedFlowIdProvider` auf der nächsten gehaltenen Anfrage `…000000000002` stehen statt auf `…000000000001`. Das Fenster misst 1600×1000: unter `ruleRowOriginBelow` von 420 px Zeilenbreite weicht das Abzeichen aus der Zeile. `RememberTarget.apex` wäre hier nicht messbar, weil `apexResolverProvider` ohne Antwort des Daemons leer ist und der Umfang mit `RefusalReason.apexUnknown` abgelehnt würde.)
...
- Kein Major-Bump und kein `humanitl.v2`. `PROTO_MAJOR` und `PROTO_MINOR` bleiben unverändert (heute `1` und `3`, `daemon/crates/ipc/src/lib.rs:52`, seit HUM-040). Die Ausnahme in `docs/PROTOCOL.md` 4/5 für ein nachweislich totes Feld ist kein Bruch dieses Nicht-Ziels, sondern seine Voraussetzung (siehe Stand).
- `backlog/sprint-0.md:600` bleibt unangetastet; ein abgeschlossenes Sprint-File hält fest, was damals entworfen wurde.

### Betroffene Pfade
- `proto/humanitl/v1/humanitl.proto` (ändern: `reserved`-Block Zeilen 581-584, Feld Zeilen 598-599)
- `proto/descriptor.binpb`, `proto/generated.sha256` (Regenerat aus `make proto`)
- `daemon/crates/ipc/tests/proto_contract.rs` (ändern: Zeile 726 streichen, Test bei Zeile 329 erweitern oder danebenstellen)
...
- `docs/PROTOCOL.md` (ändern: Abschnitt 4, Tabelle der reservierten Nummern; Abschnitte 4 und 5, Ausnahme für ein nie gelesenes Feld, siehe Stand)
- `backlog/sprint-4.md` (ändern: HUM-049, Entwurf Zeilen 226-232 und „Betroffene Pfade" Zeilen 511-519)
...
| `daemon/crates/recorder/src/findings.rs` | `daemon/crates/recorder/src/writer.rs:680` (Schreiben), `daemon/crates/recorder/src/query.rs:243` (Lesen) |
...
### Akzeptanzkriterien
- [x] `grep -rn acknowledge_findings daemon/ app/ proto/ --exclude-dir=target` trifft nur, was das Feld als entfernt beschreibt oder seine Sperre prüft. **Umformuliert und gemessen am 2026-09-11.** Die frühere Fassung verlangte genau eine Zeile, `reserved "acknowledge_findings";`, und war mit dieser Spezifikation selbst nicht erfüllbar: Ihr Proto-Kommentar nennt den Namen, ihr Reservierungstest prüft ihn, und der erzeugte, nicht eingecheckte `app/lib/core/ipc/generated/humanitl/v1/humanitl.pbjson.dart` führt ihn in der Liste der reservierten Namen. Gemessen stehen genau diese Treffer da: zwei Zeilen im Proto (Kommentar und `reserved`), fünf in `proto_contract.rs` (Name, Kommentar, Prüfung und Meldung des Reservierungstests, ein Verweis darauf) und die eine im erzeugten Descriptor-Abbild. Kein Konstruktor, kein Leser und kein Setzer unter `daemon/` oder `app/`. (Die Nennungen in `backlog/` und `BACKLOG.md` bleiben; sie beschreiben das Feld als entfernt. Das noch frühere Kriterium „genau zwei Zeilen im ganzen Repository" war ebenso nie erfüllbar, siehe Stand.)
- [x] `cargo build --workspace` grün: `daemon/bin/humanitl` und `daemon/crates/ipc` übersetzen ohne das Feld. **Gemessen am 2026-09-11** mit `cargo clippy --workspace --all-targets -- -D warnings`, das alles übersetzt. Dafür waren mehr Stellen nötig, als die Spezifikation kannte: drei weitere Setzer in `daemon/bin/humanitl/src/cmd/moderate.rs` (Übersetzungsfehler `E0560`) und sechs Literale, je drei in `fake_parity.rs` und `rules_rpc.rs`, am 2026-09-12 in `git show 766a4b6` nachgezählt, deren `..v1::DecideRequest::default()` nach dem Wegfall nichts mehr bewirkte (clippy `needless_update`).
- [x] `cargo test -p humanitl-ipc` grün, darunter der neue Reservierungstest und `checked_in_descriptor_matches_the_proto_sources`. **Gemessen am 2026-09-11**, `proto_contract` mit 22 Tests.
- [x] Ein lokaler Versuchspatch, der `bool acknowledge_findings = 6;` wieder einfügt, lässt `cargo test -p humanitl-ipc` fehlschlagen; unter den fehlenden Tests ist der neue Reservierungstest (dazu fallen `check_fields`, weil es beidseitig prüft, und `checked_in_descriptor_matches_the_proto_sources`). Ergebnis im Commit-Body. **Gemessen am 2026-09-11, mit einer Einschränkung an der Form des Patches.** Mit `reserved 6` im Proto übersetzt ein wieder eingefügtes Feld gar nicht erst. Der Versuchspatch setzt deshalb Proto und Descriptor auf den Stand vor dem Issue zurück, wie ein echter Rückbau nach `make proto`: Rot werden `decide_request_reserves_the_never_read_acknowledge_findings_number` und `decide_request_has_every_decision_of_the_oneof`, der Test über die Tabelle mit `check_fields`. Wird nur das Proto zurückgesetzt, fällt allein `checked_in_descriptor_matches_the_proto_sources`.
- [x] Nach dem Commit ist `make proto` gefolgt von `git diff --exit-code proto/` leer (keine weitere Drift), und `proto/descriptor.binpb` sowie `proto/generated.sha256` liegen im selben Commit. `git check-ignore app/lib/core/ipc/generated/` trifft weiterhin, es wird nichts Generiertes eingecheckt. **Gemessen am 2026-09-11** direkt nach `766a4b6`: `make proto`, dann `git diff --exit-code proto/` ohne Ausgabe, `git check-ignore` trifft. Die CI bestätigt es im Schritt „Fail on generated drift" des Laufs 34600250925 über `70c44f3`.
- [x] `scripts/gen-proto.sh`, dann `flutter analyze` und `flutter test` in `app/` grün; `grep -rn acknowledgeFindings app/lib` ohne Treffer. **Gemessen am 2026-09-11** mit `STRICT=1` und `protoc-gen-dart` im `PATH`: `flutter analyze` ohne Befund, `flutter test` 1022 grün und 4 übersprungen, kein Treffer für `acknowledgeFindings`.
- [x] `git diff --exit-code daemon/crates/ipc/src/lib.rs app/lib/core/ipc/proto_version.dart` leer: `PROTO_MAJOR` und `PROTO_MINOR` bleiben unverändert. **Gemessen am 2026-09-11.**
- [x] `git diff --stat` berührt keine Datei unter `daemon/crates/proxy/src/`, `daemon/crates/recorder/src/`, `daemon/crates/config/src/`, `app/lib/features/` oder `app/l10n/`. **Gemessen am 2026-09-11**: elf Dateien, keine davon in diesen Bereichen. **Am 2026-09-12 über `git show --stat 766a4b6` nachgezählt**: zwölf Dateien, keine davon in diesen Bereichen. Die elf stammen aus dem Arbeitsbaum, bevor diese Messung selbst in `backlog/sprint-2.md` stand.
- [x] `grep -n "acknowledged_findings = 8" backlog/sprint-4.md` trifft zweimal: `:242` im `DecideRequest`-Entwurf und `:1751` in der Prosa von `## HUM-090`, die dieselbe Streichung beschreibt; `grep -n "ignore_always = 9" backlog/sprint-4.md` trifft einmal (`:243`). Im `DecideRequest`-Entwurf, der im Abschnitt `## HUM-047` steht (Zeilen 232 bis 245), trägt Nummer 6 kein Feld mehr, sie steht nur noch in `reserved 3, 6;`. **Gemessen am 2026-09-11, am 2026-09-12 nachgezählt**: Beide Treffer kamen mit demselben Commit `766a4b6`, die frühere Zusage „je einmal" war schon beim Schreiben falsch. Der Inhalt hält: die Nummern sind registriert, und 6 ist nicht mehr belegt.
- [x] Jeder Pfad ohne Marke „(neu)" in HUM-049 „Betroffene Pfade" existiert: eine `test -e`-Schleife über die Liste, die Einträge mit „(neu)" und Nicht-Pfade wie `- ARB-Dateien` überspringt, meldet keinen Fehler (vor der Änderung meldet sie vier). **Gemessen am 2026-09-11**: acht Pfade, alle vorhanden; die Einträge nennen jetzt `writer.rs`/`query.rs`, `proxy/src/findings.rs`/`findings/src/registry.rs`, `config/src/model.rs` und `decision.dart`.
- [x] `make check` grün und `tools/verify-commit.sh` auf dem Commit grün. **Gemessen am 2026-09-11**: `make check` über den Stand des Commits grün, `tools/verify-commit.sh 70c44f3` grün, und der CI-Lauf 34600250925 über `70c44f3` in allen elf Jobs grün.

### Stand (2026-09-04): baubar, zwei Kriterien und eine Protokollregel vorher richten

Geprüft am Code (Audit 2026-09-04, Zeilen gegen den heutigen Baum gezogen). Die Prämisse hält: `acknowledge_findings` liest niemand (`validate.rs:67`, `:83`; `server.rs:863`; `fake/mod.rs:246`; `convert.dart:500-521`), gesetzt wird es nur in `flows.rs:339` und `proto_roundtrip.rs:156`, beide Struct-Literale ohne `..Default::default()`. Die Vertragszeile steht in `proto_contract.rs:726`. Die Kollision mit HUM-049 ist echt (`backlog/sprint-4.md:231-232`); `ignore_always = 7` kollidiert zusätzlich mit dem lebenden `allow_edited = 7` (`humanitl.proto:591`). Die vier nicht existierenden HUM-049-Pfade und ihre vier Ersatzpfade stimmen. Heute stehen 17 Treffer für `acknowledge_findings` in 8 Dateien im Repository.

**Blockierend, vor dem Bau zu richten** (im Text oben bereits korrigiert):

- Das Akzeptanzkriterium „genau zwei Zeilen" war nie erfüllbar: nach der Änderung bleiben 14 Treffer (Proto, `sprint-0.md:600`, zehn in dieser Spezifikation, `sprint-4.md`, `BACKLOG.md`). Das Kriterium zählt jetzt nur `daemon/`, `app/`, `proto/`.
- `PROTO_MINOR` ist `3`, nicht `2` (`daemon/crates/ipc/src/lib.rs:52`, seit HUM-040; `app/lib/core/ipc/proto_version.dart:20` und `docs/PROTOCOL.md:65` sagen dasselbe). Das Kriterium heißt jetzt „bleibt unverändert".
- `docs/PROTOCOL.md` widerspricht dem Vorgehen: Regel 1 (`:54`) sagt „Nur additiv", Abschnitt 5 (`:68`) „Bruch ⇒ Major steigt und das Paket heißt `humanitl.v2`". Regel 2 (`:55`) erlaubt nur das Reservieren einer entfernten Nummer und hebt beides nicht auf; `proto/buf.yaml` (`breaking: use: [FILE]`) meldet die Löschung als `FIELD_NO_DELETE`. Das Nicht-Ziel „kein Major-Bump" verbot sich bisher die einzige Lösung. Sie gehört in denselben Commit: eine Ausnahme in `docs/PROTOCOL.md` 4/5 für ein Feld, das nachweislich kein Handler liest und kein Client setzt, mit `reserved` als Sperre. **Ob diese Ausnahme gilt, entscheidet der Projekteigentümer; ohne sie steht die Löschung gegen das eigene Vertragsdokument und das Issue ist nicht abschließbar.**

**Weitere Befunde, im Text korrigiert oder hier vermerkt:** Proto-Zeilen sind `:581-584` (`reserved`) und `:598-599` (Feld). HUM-049 „Betroffene Pfade" steht in `sprint-4.md:511-519`; die Liste endet mit `- ARB-Dateien` (kein Pfad) und trägt eine Marke `(neu)` (`:513`), die `test -e`-Schleife muss beides überspringen. `write_findings` beginnt in `writer.rs:680`, das harte `NULL` steht `:684`. HUM-049s `DecideRequest`-Entwurf (`sprint-4.md:226-229`) weicht in vier weiteren Feldern vom gelieferten Vertrag ab: `string flow_id = 1` gegen `repeated string flow_ids = 1` (`humanitl.proto:579`), `DecisionKind decision = 2` gegen `oneof decision` mit `allow = 2`, `HttpRequestProto edited = 3` gegen `reserved 3`, `replacements = 4` gegen `Block block = 4`. Wer nur 6 und 7 auf 8 und 9 zieht, lässt den Entwurf auf der gesperrten Nummer 3 stehen, und das Kriterium „Nummer 6 kommt nicht mehr vor" wird trotzdem grün; Schritt 6 zieht deshalb den ganzen Entwurf nach. Die Gegenprobe (Feld wieder einfügen) lässt drei Tests fallen, nicht einen: `check_fields` prüft beidseitig (`proto_contract.rs:595-633`), dazu `decide_request_has_every_decision_of_the_oneof` (`:747`) und `checked_in_descriptor_matches_the_proto_sources` (`:58`).

**Voraussetzungen auf dieser Maschine:** `protoc-gen-dart` liegt unter `~/.pub-cache/bin` und steht nicht im `PATH`; ohne `export PATH="$PATH:$HOME/.pub-cache/bin"` überspringt `scripts/gen-proto.sh` (`fail_or_skip`, `:39`, `:57`) die Dart-Erzeugung mit Exit 0, erneuert nur `proto/descriptor.binpb` und lässt `proto/generated.sha256` veraltet; der CI-Schritt „Fail on generated drift" (`.github/workflows/ci.yml:249`) wird dann auf einem Commit rot, der lokal grün aussah. `protoc` 27.5 ist da (`~/.local/bin/protoc`), `protoc_plugin 25.0.0` ist aktiv; `AGENTS.md:81` behauptet, es gebe kein `protoc`, und ist veraltet. `proto/` ist im Arbeitsbaum schmutzig (HUM-040, siehe HUM-091 Stand); vor dem Proto-Schritt neu einlesen. Der CI-Schritt „buf breaking against main" steht noch auf `continue-on-error: true` (`ci.yml:240-245`), obwohl der Kommentar das Entfernen nach dem Merge von HUM-003 verlangt; solange die Flag steht, geht die Löschung durch CI, danach greift `FIELD_NO_DELETE`.

**Reihenfolge:** vor HUM-095. Beide schreiben dasselbe Literal `daemon/bin/humanitl/src/cmd/flows.rs:336-340` (dieses Issue streicht `:339`, HUM-095 ersetzt `:338`). Nicht parallel.
...
Sprint: 2 · Größe: L · Abhängigkeiten: HUM-026, HUM-029, HUM-031, Merge von HUM-040 (Proto im Arbeitsbaum, siehe Stand) · Blockiert: HUM-094 (Gruppenkopf auf dem Apex des Daemons)
...
- `daemon/crates/ipc/src/convert.rs`: `recorded_summary_to_proto:1377`, `record_to_summary:725`, `record_to_detail:801` (ruft `record_to_summary` und `domain_of`, siehe Stand), `received_summary:1108`, `matches_filter:1159`, `apex_of:631` (weicht `humanitl_catalog::psl::apex`)
- `daemon/crates/ipc/src/server.rs`: Aufrufer `:297`, `:356`, `:962` (`get_flow`-Rückfall auf `record_to_detail`); `DomainTable` hängt schon an `:108`
- `daemon/crates/ipc/src/fake/state.rs`: `summary():122`, `domain():192-194` (heute `apex_of`, künftig `humanitl_catalog::psl::apex`)
...
- `app/lib/core/domain/flow.dart:85` (neben `authority`) plus freezed-Regeneration
- `app/lib/core/ipc/convert.dart:196`
- `app/lib/core/ipc/fake_daemon_client.dart:1825-1828` (`case 'apex'` von `case 'host'` trennen; `_suffixMatch:2026`), geteilte Datei
- `app/lib/features/intercept/psl.dart` (gelöscht); seine Tests stehen in `app/test/features/intercept/helpers_test.dart` (Import Zeile 9, Fälle Zeilen 83-104) und fallen im selben Commit; ein `psl_test.dart` gibt es nicht
- `app/lib/features/intercept/providers/held_groups.dart:184`, `providers/decision.dart:975-995`, `widgets/batch_modal.dart:97`
- `app/test/features/intercept/fixtures.dart:25` (`heldFlow` bekommt `apex`), `held_groups_test.dart:17` (`flowOf` ebenso), die 15 Testdateien, die `heldFlow` nutzen, darunter `app/test/goldens/intercept_golden_test.dart`, `intercept_group_golden_test.dart`, `action_bar_golden_test.dart` (Goldens neu abnehmen); `batch_test.dart:173`, `:186`, `:313` (Prämisse „`psl.dart` rät")
- Kommentare mit derselben Prämisse: `decision.dart:519`, `:974`, `agent_ask_card.dart:174`, `held_groups.dart:56`, `held_groups_test.dart:65`
...
- `backlog/CONVENTIONS.md`: 4.3 um `FlowSummary.apex` ergänzen, 4.15 (`psl.dart`-Absatz bei `:650`) neu fassen, 4.16 (`:1808`, behauptet ebenfalls, `psl.dart` rate) nachziehen
...
| `humanitld --fake` (`fake/state.rs::summary`) | `humanitl_catalog::psl::apex`, dieselbe Funktion wie im Daemon und wie `domain()` derselben Nachricht (`convert::apex_of` entfällt, siehe Stand) | IP-Literal, unbekanntes Suffix |
...
Alle Aufrufer übergeben `self.domains.as_deref()`: `server.rs:297` (Passthrough-Prüfung, liest nur `.passthrough`), `:356` (Nachlieferung), `:962` (`get_flow` über `record_to_detail`) und `flow_event_to_proto` (trägt `domains` schon). Eine Zeile, zwei Formen wäre die nächste Abweichung.
...
3. `record_to_summary`, `received_summary` und `record_to_detail` um `domains` erweitern, alle Aufrufer nachziehen, `apex_of` durch `humanitl_catalog::psl::apex` ersetzen, `fake/state.rs::summary` füllen; `cargo test -p humanitl-ipc` grün.
...
7. `psl.dart` löschen und `helpers_test.dart:9,83-104` streichen; `heldFlow` und `flowOf` um `apex` erweitern, die 15 Testdateien nachziehen, drei Golden-Suites neu abnehmen; `held_groups.dart`, `decision.dart`, `batch_modal.dart` auf das Feld umstellen; `flutter test test/features/intercept/ test/goldens/` grün.
8. `daemon_end_to_end.rs` um die Zusicherung beider Wege erweitern (der Neustart-Test `the_recording_outlives_the_daemon`, `:328`, hat den Harness schon), `backlog/CONVENTIONS.md` 4.3, 4.15 und 4.16 nachziehen, `make check`, danach `tools/verify-commit.sh`.
...
- `convert::tests::detail_summary_and_domain_agree_on_the_apex`: `record_to_detail(&record, domains)` liefert `summary.apex == domain.apex`, für `a.b.github.io` beide `b.github.io`, für `192.168.1.50` beide leer.
...
- [x] `make check` grün, `proto/descriptor.binpb` im selben Commit erneuert, `tools/verify-commit.sh` vor dem Push grün. (Gemessen am 2026-09-12 über den fertigen Commit: `tools/verify-commit.sh e723bcb` grün — Format und Lints, Bau und Tests, rustdoc, Abhängigkeiten, Dokumente, Lizenzen, Flutter-Analyse und -Tests, Escape-Suite 124 von 124 —, `proto/descriptor.binpb` und `proto/generated.sha256` liegen im selben Commit wie die `.proto`, und der CI-Lauf über denselben Commit ist mit allen elf Jobs grün, darunter `proto-lint-and-gen`, das den Descriptor neu erzeugt und gegen den committeten vergleicht.)

### Stand (2026-09-04): Größe L, ein fünfter Aufrufer, die Fakes raten weiter

Geprüft am Code (Audit 2026-09-04, Zeilen gegen den heutigen Baum gezogen). Der Kern hält: die Spalte `apex` existiert und ist gefüllt (`daemon/crates/recorder/migrations/V1__init.sql:35`, `query.rs:39`, `:407`, `types.rs:327`, geschrieben über `ipc/src/domains.rs:113`), der Filter übersetzt `apex` zu `apex = ?` (`recorder/src/filter.rs:214`, Schlüssel in `KEYS` `:32`), das Proto führt Felder 1..24 ohne Apex (`humanitl.proto:312-350`), und `recorded_summary_to_proto` (`convert.rs:1377`) wirft `row.apex` weg. Die Drei-Wege-Divergenz ist belegt: Daemon exakt, Dart-Fake Suffix (`fake_daemon_client.dart:1825-1828` teilt den Fall mit `host`, `_suffixMatch:2026`), Rust-Fake kennt den Schlüssel nicht (`matches_filter`, `convert.rs:1159`). Keine neue Abhängigkeit nötig: `psl = "=2.1.228"` steht in `daemon/Cargo.toml:67`, `humanitl_catalog::psl::apex` (`catalog/src/psl.rs:27-29`, `::psl::domain_str`) ist `pub`, und `humanitl-ipc` hängt schon an `humanitl-catalog` (`ipc/Cargo.toml:13`, erlaubt in `tools/deps-allow.toml:22`).

**Blockierend, im Text oben korrigiert:**

- **Ein fünfter Aufrufer.** `record_to_detail` (`convert.rs:801`) ruft `record_to_summary` ohne `domains` und setzt daneben `domain: Some(domain_of(...))`, also den Rater `apex_of`. Der echte Dienst erreicht ihn: `server.rs:962` (`get_flow`) fällt darauf zurück, sobald `self.recorder` `None` ist (`server.rs:107`, Default `:173`) oder keine Zeile hat. Ergebnis wäre ein `FlowDetail` mit `summary.apex = ""` neben `domain.apex = <Rat>`, genau das, was der eigene Fallstrick verbietet. `record_to_detail` bekommt denselben Parameter.
- **`apex_of` rät anders als der Daemon.** Die Quellen-Tabelle schrieb dem Rust-Fake `convert::apex_of` (`:631-643`) vor: letzte zwei Label, `a.b.github.io` wird `github.io`, ein IP-Literal wird zur Adresse, wo `catalog::psl::apex` `None` sagt. Mit dem exakten Vergleich fände `apex:b.github.io` im Fake nichts und `apex:github.io` die Zeile, umgekehrt zum Daemon und zum eigenen Test `filter_apex_is_exact`. Das Ziel „beide Fakes exakt wie der Daemon" ist nur zu halten, wenn `apex_of` durch `humanitl_catalog::psl::apex` ersetzt wird (eine Zeile, erreichbar); Tabelle, Fallstrick und Akzeptanzkriterium sind darauf umgestellt. Das alte Kriterium „`rg apex_of` trifft nur `fake/` und `domain_of`" war heute schon grün (Treffer: `convert.rs:631`, `:684`, `:691`, `fake/state.rs:22`, `:194`, `:791`, `:939-955`) und bewies nichts, weil `domain_of` über `record_to_detail` aus einem echten Pfad läuft.
- **Die Tests, die kippen.** `psl_test.dart` gibt es nicht; die Tests von `psl.dart` stehen in `app/test/features/intercept/helpers_test.dart` (Import `:9`, Fälle `:83-104`) und fallen im selben Commit, sonst übersetzt die Datei nicht. `apexOfHost` auf `flow.apex` stellt jeden Gruppentest um: keine Fixture setzt `apex`; `heldFlow` (`app/test/features/intercept/fixtures.dart:25`) hat den Parameter nicht, `flowOf` (`held_groups_test.dart:17`) auch nicht, und `heldFlow` wird von 15 Testdateien benutzt, darunter drei Golden-Suites (`app/test/goldens/intercept_golden_test.dart`, `intercept_group_golden_test.dart`, `action_bar_golden_test.dart`). Ohne den Parameter zerfällt die Warteschlange in den Tests in eine Gruppe je Host (`held_groups_test.dart:60-63` erwartet `['npmjs.org', 'github.com']`, `:93-101` erwartet `pypi.org` und `files.pypi.org` in einer Gruppe). Dazu drei Fälle in `batch_test.dart` (`:173`, `:186`, `:313`), deren Prämisse „`psl.dart` rät" ist. Das ist der Grund für L statt M.

**Kleineres, im Text korrigiert:** `Blockiert: HUM-036` war falsch; HUM-036 ist gemerged (`0bfe967`, `617cf77`), und der M2-Lauf filtert über `apex:` (`run.sh:409`, `:460`, `:469`, `:471`, `:478`, `:484`), ohne je `FlowSummary.apex` zu lesen. Gebraucht wird das Feld von HUM-094 (Gruppenkopf). Zeilenanker waren 20 bis 40 Zeilen verschoben (`received_summary` `:1108`, `matches_filter` `:1159`, `server.rs` `:108`/`:297`/`:356`/`:962`, `fake/state.rs` `:122`/`:192-194`, `flows.rs` `:494`/`:548`, `flow.dart:85`). `backlog/CONVENTIONS.md`: der `psl.dart`-Absatz steht in 4.15 bei `:650`, nicht `:628-637`, und 4.16 (`:1808`) behauptet ebenfalls, `psl.dart` rate die registrierbare Domäne; beide Stellen fallen. Kommentare mit derselben Behauptung stehen in der Pfadliste.

**Voraussetzungen:** `proto/humanitl/v1/humanitl.proto`, `proto/descriptor.binpb` und `proto/generated.sha256` sind im Arbeitsbaum schmutzig (HUM-040: `SandboxRequest.Plan`, `Mount`, `EnvVar`, laut `docs/PROTOCOL.md:65`). Wer jetzt regeneriert, backt fremde Nachrichten in Descriptor und Hash ein; `daemon/crates/ipc/tests/proto_contract.rs` vergleicht Byte für Byte, und `tools/verify-commit.sh` wird rot. Proto-Schritt erst nach dem Merge von HUM-040, Datei unmittelbar davor neu einlesen. `protoc-gen-dart` liegt außerhalb des `PATH` (siehe HUM-089 Stand); ohne `export PATH="$PATH:$HOME/.pub-cache/bin"` existiert `pb.FlowSummary.apex` nicht und `app/lib/core/ipc/convert.dart` übersetzt nicht.
...
- `record_to_summary` bekommt einen zweiten Parameter; jeder Aufrufer übergibt die Tabelle. Ein Aufrufer mit `None` erzeugt still eine zweite Form derselben Zeile, und die fällt erst auf, wenn ein Client zwei Antworten auf dieselbe Frage sieht. Der Aufrufer, der in der ersten Fassung fehlte, ist `record_to_detail` (`convert.rs:801`), erreichbar über `server.rs:962`.
...
Sprint: 2 · Größe: XL · Abhängigkeiten: HUM-029, HUM-031 (Daemon-Hälfte gemerged), HUM-036 (Daemon-Hälfte gemerged), HUM-091 (`FlowSummary.apex`, `psl.dart` entfällt), HUM-097 (Bildschirm-Lauf während des Haltens), Entscheidung zu `FlowSummary.catalog_id` (siehe Stand) · Blockiert: Sprint-3-Merges (Sprint-Gate, geerbt von HUM-036)
...
Zwei Dinge werden dabei ausdrücklich nicht behauptet. Erstens steht `M2_EXPECTED_ASSERTIONS=59` in `tests/e2e/m2_first_decision/run.sh:112` nicht wegen dieser Lücken niedriger: der Kommentar `:107-111` definiert die Zahl als „So viele Behauptungen prüft ein vollständiger Lauf ohne die Oberfläche", und die Oberflächen-Hälfte fällt aus genau einem Grund aus, nämlich weil `app/integration_test/m2_first_decision_test.dart` fehlt (`run.sh:673-681`). Der fehlende Katalogname ist eine unter vielen ungeprüften Sachen, nicht die Ursache der Zahl. Zweitens ist der Katalog nicht ungeprüft: `run.sh:406-416` vergleicht die Apex-Spalte, die der Katalog beim Eintreffen füllt, gegen die Zählung nach Host, zwölf gegen zwölf. Ungeprüft ist der Name.

### Ziel
Das rechte Pane des Intercept-Bildschirms zeigt für eine Anfrage an `registry.npmjs.org` die Katalog-Karte mit „npm registry", Kategorie-Chip, Beschreibung, „Typical for: npm install", Rang-Badge und Schnellregeln; für `evil.example` die gestrichelte Unbekannt-Karte; ohne Auswahl die Sitzungs-Zusammenfassung. Der Kopf einer Gruppe, deren gehaltene Anfragen alle dieselbe `catalog_id` vom Daemon tragen, nennt den Dienst statt eines Hosts, und dieselbe Zeile steht in der Auswahl-Karte. Der Bildschirm-Lauf aus HUM-097 prüft zusätzlich, dass auf dem Bildschirm der Dienstname und nicht der Host stand; Schritt 10, `m2_first_decision_test.dart` und die HAR-Prüfung selbst sind HUM-097. `BACKLOG.md:395` verspricht für M2 nur noch, was M2 liefert.
...
- `app/integration_test/m2_first_decision_test.dart` (HUM-097; hier nur der Nachtrag der zwei Namens-Behauptungen)
- `app/test/features/intercept/catalog_test.dart`, `domain_panel_test.dart` (neu); Goldens unter `app/test/goldens/`
- `app/lib/core/domain/flow.dart`, `app/lib/features/intercept/providers/flows.dart:54-57` (ändern: `DomainInfo` je Warteschlangenzeile behalten statt verwerfen, siehe Stand)
- `app/lib/core/ipc/fake_daemon_client.dart` (ändern, geteilte Datei: `Received.domain` in den Szenarien füllen; heute null Treffer für `domain` in der Datei)
- `proto/humanitl/v1/humanitl.proto` (nur falls die Entscheidung `FlowSummary.catalog_id` lautet; dann zusammen mit HUM-091)
- `app/lib/features/intercept/intercept_screen.dart` (ändern: Import Zeile 37, Verwendung Zeile 478)
...
- `app/lib/features/intercept/widgets/group_header_row.dart` (ändern: `groupTitle`, Zeilen 149-152)
- `app/lib/features/intercept/widgets/selection_card.dart` (ändern: Zeile 51)
- `app/lib/features/intercept/psl.dart` (entfällt mit HUM-091; hier nicht mehr berührt)
...
- `tests/e2e/m2_first_decision/run.sh` (ändern: nur der Nachtrag der zwei Namens-Behauptungen in Schritt 10, Zeilen 671-716, und `M2_EXPECTED_ASSERTIONS` in Zeile 112; der Umbau des Schritts ist HUM-097)
- `backlog/CONVENTIONS.md` (ändern: 4.15, Satz bei Zeile 695; 4.22, Absatz zur Oberflächen-Hälfte bei Zeile 1420, zusammen mit HUM-097)
...
abstract class CatalogEntry with _$CatalogEntry {
  const factory CatalogEntry({
    ...
    @Default(<String, String>{}) Map<String, String> riskNote, // en/de wie description
  }) = _CatalogEntry;
...
Gruppierung. Der Apex kommt aus `FlowSummary.apex` (HUM-091): `apexOfHost(Flow flow) => flow.apex.isEmpty ? flow.host : flow.apex;`. `Flow` hat kein Feld `domain` (`app/lib/core/domain/flow.dart:76-110`, `DomainInfo` hängt an `FlowDetail:174`); das frühere Beispiel `flow.domain.apex` kompilierte nicht. Die `catalog_id` je Zeile braucht einen eigenen Draht, siehe Stand.
...
`run.sh` Schritt 10 gehört HUM-097 (Treiber-Schalter, Bildschirm läuft während des Haltens). Dieses Issue trägt nur den Nachtrag ein, sobald der Gruppenkopf steht; der Zweig für die fehlende Datei wird dann zum harten Fehler:
...
Schritt 10 zählt damit zwei Behauptungen mehr (Name, Katalogzeile); HAR geschrieben und HAR-Einträge zählt HUM-097. `M2_EXPECTED_ASSERTIONS` (`run.sh:112`, heute 59 ohne Oberfläche) trägt die Zahl, die der Bildschirm-Zweig nach HUM-097 meldet; HUM-097 führt sie je Zweig.

`backlog/CONVENTIONS.md`: In 4.15 fällt der letzte Satz des Absatzes zu `Edit + Allow` weg (Zeile 695, „… bis HUM-031 den Katalog liefert."); der Satz zu `Edit + Allow` selbst bleibt, bis HUM-047 ihn einlöst. Den `psl.dart`-Absatz (Zeile 650) schreibt HUM-091 um; das Modal für Entscheidungen über mehrere Hosts bleibt mit der Begründung von dort. In 4.22 fällt der Absatz weg, der die Oberflächen-Hälfte als ausstehend führt (Zeile 1420); das tut HUM-097.
...
5. Nachtrag zu HUM-097s `m2_first_decision_test.dart`: die Summary-Zeile der npm-Gruppe nach `$HUMANITL_E2E_GROUP_SUMMARY` schreiben. Prüfbar: `xvfb-run -a flutter test integration_test/m2_first_decision_test.dart -d linux` gegen einen von Hand gestarteten Lauf grün.
6. `run.sh` Schritt 10: die zwei Namens-Behauptungen und `M2_EXPECTED_ASSERTIONS` (Zeile 112 bzw. der Wert des Bildschirm-Zweigs). Prüfbar: `M2_UI=1 tests/e2e/m2_first_decision/run.sh` endet 0, ohne `SKIPPED:`-Zeile und ohne `raise M2_EXPECTED_ASSERTIONS`.
...
- [x] (nach HUM-097) `M2_UI=1 tests/e2e/m2_first_decision/run.sh` endet mit 0. Die Ausgabe enthält keine Zeile mit `SKIPPED:` und keine mit `raise M2_EXPECTED_ASSERTIONS`. **Gemessen am 2026-09-12 auf dem fertigen Stand** (`2feb41a`): `E2E_ONLY=m2 ./tests/e2e/run.sh` — der Bildschirm ist die Vorgabe — meldet „M2 demo: OK", und `grep -cE "SKIPPED:|raise M2_EXPECT"` über das Protokoll des Laufs ergibt 0.
- [x] Der Bildschirm-Zweig meldet zwei Behauptungen mehr als vor diesem Issue, und `M2_EXPECTED_ASSERTIONS` trägt genau die gemeldete Zahl. **Gemessen am 2026-09-12:** Der Lauf meldet „e2e: 77 assertions checked", und `M2_EXPECTED_ASSERTIONS_SCREEN` steht auf 77, vorher 75. Die Selbstprüfung am Ende des Laufs vergleicht beide und hätte bei einer anderen Zahl gemeckert; der Zweig ohne Oberfläche bleibt bei 71 (HUM-095) und ist im selben Lauf mit `M2_UI=0` gemessen.
- [x] Zwei dieser Behauptungen betreffen den Namen: die Datei aus `$HUMANITL_E2E_GROUP_SUMMARY` enthält `npm registry` und `Looks like: npm install`. **Gemessen am 2026-09-12:** Beide `e2e_check`-Aufrufe in Schritt 10 laufen und sind grün („the screen names the service, not the host" und „and says what the group looks like"); der Bildschirm-Treiber schreibt die Zeile, weil `run.sh` ihm jetzt `HUMANITL_E2E_GROUP_SUMMARY` mitgibt. Die Gegenprobe zur Namensgebung selbst liegt im Widget-Test: `groupTitle` auf `group.display` zurückgesetzt liefert `registry.npmjs.org` statt `npm registry`, und der Finder auf den gezeichneten Kopf findet nichts. Diese Mutation ist nicht über den ganzen e2e-Lauf gefahren worden, und das hat einen zweiten Grund, der hier stehen soll: Der Treiber prüft den Namen selbst, bevor er die Zeile schreibt. Unter der Mutation stirbt also schon der Treiber, `m2_wait_screen` meldet den Fehlschlag, und der Lauf endet, bevor Schritt 10 erreicht ist. Die beiden `e2e_check`-Aufrufe bewachen damit, dass die Zeile geschrieben wurde und was in ihr steht — sie sind keine zweite, unabhängige Probe auf die Namensgebung.
- [x] (HUM-097) `jq -r '.log.entries | length' "$M2_HAR"` liefert 17, die Datei ist nicht leer. **Gemessen am 2026-09-12:** Die Behauptung „the export holds every request of the run" des Laufs vergleicht genau das und ist grün, zusammen mit der Prüfung, dass die Datei existiert und nicht leer ist.
- [x] (HUM-097) Ein Lauf ohne `app/integration_test/m2_first_decision_test.dart` (Datei versetzen, danach zurück) endet mit 1 und der Meldung, dass sie fehlt, nicht mehr mit 0. **Gemessen am 2026-09-12:** Datei beiseitegelegt, `E2E_ONLY=m2 ./tests/e2e/run.sh` endet mit Exit-Code 1 — direkt abgegriffen, nicht durch eine Pipe — und meldet „the integration test of the screen is missing: …. Restore it, or run with M2_UI=0 and know that nothing about the screen and nothing about the HAR export is verified." Danach zurückgelegt, Arbeitsbaum wieder sauber.
...
- [x] `make check` grün. **Gemessen am 2026-09-12 auf dem fertigen Stand** (`2feb41a`): `make check` endet mit Exit-Code 0. Die eine Stelle, die das vorher verhinderte, ist beim Zusammenführen behoben — `daemon/bin/humanitl/src/cmd/flows.rs` baute ein `v1::FlowSummary` als vollständiges Struct-Literal ohne `..Default::default()` und kannte `catalog_id` nicht, was den Workspace mit `error[E0063]` brach. Dieselbe Datei nennt das Feld jetzt auch in `summary_json`, damit `flows list --json` und `flows show --json` denselben Dienst nennen wie der Bildschirm (ADR-018).

### Stand (2026-09-04): Größe XL, die e2e-Hälfte gehört HUM-097, der Gruppenkopf hat keinen Draht

Geprüft am Code (Audit 2026-09-04, Zeilen gegen den heutigen Baum gezogen). Die Bestandsaufnahme hält: `app/assets/` fehlt, `Makefile` hat weder `catalog-assets` noch `catalog-lint`, `app/pubspec.yaml` keinen `assets:`-Block und keine `yaml`-Abhängigkeit, `interceptGroupLooksLike` null Treffer, `DomainPanePlaceholder` steht (`intercept_screen.dart:37`, `:478`), `catalog/domains.yaml` trägt 34 Einträge mit `name`, `typical` und `description` als `en`/`de`-Abbildung, die Katalog-Crate ist vollständig, und der Daemon liefert `DomainInfo` live (`convert.rs:964-978`) und in `GetFlow` (`server.rs:469`, `:484-491`). Baubar ohne jede Vertragsänderung ist die Katalog-Karte für den ausgewählten Flow: `flowDetailProvider(id).value?.domain` trägt `apex`, `catalogId`, `trancoRank`, `firstSeen`, `seenCount`, vom Daemon gefüllt.

**Blockierend:**

- **Der Gruppenkopf aus `catalog_id` hat keinen Draht.** `Flow` (`app/lib/core/domain/flow.dart:76-110`) hat kein Feld `domain`; `DomainInfo` hängt an `FlowDetail` (`:174`) und an `Received`. `FlowSummary` auf der Leitung trägt keine `DomainInfo` (`humanitl.proto:312-350`, Felder 1..24). Die einzige Stelle, an der sie je Warteschlangenzeile ankommt, verwirft sie: `flows.dart:56-57` zerlegt `FlowEventReceived` nur nach `flow`. Die Nachlieferung schickt `domain: None` (`server.rs:338`), die App abonniert ohne `since` (`flow_events.dart`), und der Lücken-Reload geht über `ListFlows`, also über `FlowSummary` ohne Domain (`flows.dart:167-190`). Das frühere Code-Beispiel `flow.domain.apex` kompilierte nicht (oben korrigiert). Ein `FlowSummary.catalog_id` stand nicht in „Betroffene Pfade", und HUM-091 verbietet es in seinem Nicht-Ziel. **Entscheidung des Projekteigentümers vor Teil (c):** `FlowSummary.catalog_id` im Proto (zusammen mit HUM-091, dessen Nicht-Ziel dann fällt) oder `DomainInfo` aus `Received` in `flows.dart` mitführen und den Kopf nur benennen, wenn jede gehaltene Anfrage der Gruppe live beobachtet wurde (deckt Nachlieferung und Reload nicht ab; der Kopf zeigt dann ehrlich den Host).
- **Die e2e-Hälfte gehört HUM-097.** `m2_first_decision_test.dart`, Schritt 10, HAR-Prüfung und `M2_UI=1` sind HUM-097 (`BACKLOG.md:470`, `run.sh:63-65`, `:671-716`); HUM-097 nahm umgekehrt den Katalognamen aus seiner Akzeptanz und verwies auf dieses Issue. Beide warteten aufeinander. Ab jetzt: HUM-097 baut den Bildschirm-Lauf, die zwei Namens-Behauptungen kommen als Nachtrag hinein, wenn der Gruppenkopf steht (Pfadliste, Schritte und Kriterien oben entsprechend gefasst).
- **Im beschriebenen Lauf gibt es zum Zeitpunkt der Prüfung keine npm-Gruppe.** `run.sh` entscheidet alle 17 Flüsse über die Kommandozeile in den Schritten 2 bis 4 (`:425-513`) und startet den Bildschirm erst in Schritt 10 (`:671`), nach `wait "$M2_AGENT_PID"` (`:517`). `heldGroups` leitet aus `heldFlowsProvider` ab (`held_groups.dart:234-235`), `heldFlows` filtert `flow.isHeld` (`flows.dart:195`): nichts hält mehr, keine Gruppe, keine Summary-Zeile, beide Namens-Behauptungen fallen aus. Erst HUM-097s Umbau (Bildschirm-Treiber während des Haltens) macht sie prüfbar.
- **Zahlen veraltet.** `M2_EXPECTED_ASSERTIONS` steht bei `run.sh:112` und trägt 59, nicht 47 bei `:76`; Zeile 76 ist Prosa über `resolver.test_ca`. Nach zwei zusätzlichen Behauptungen wäre die Zahl des Bildschirm-Zweigs plus zwei richtig, nicht 51. Oben korrigiert.
- **HUM-091 schreibt dieselben drei Zeilen anders.** `held_groups.dart:184`, `decision.dart:975-995`: HUM-091 löscht `psl.dart` und setzt `apexOfHost` auf `flow.apex.isEmpty ? flow.host : flow.apex`. Der Fallstrick „`psl.dart` bleibt Rückfall" ist damit hinfällig und korrigiert; HUM-091 steht jetzt in den Abhängigkeiten.

**Major, im Text korrigiert oder hier vermerkt:** `risk_note` ist eine `en`/`de`-Abbildung (`catalog/domains.yaml:34-37`, `entry.rs:157` `Option<Text>`), kein String; dieselbe Falle, vor der die Spezifikation bei `description` selbst warnt. `run.sh` Schritt 10 wird von HUM-097 anders umgebaut als hier beschrieben (Treiber-Schalter; Abschnitte 2 bis 4 entfallen mit Oberfläche); der Zweig ist außerdem nicht stumm: `M2_UI=1` stirbt heute (`:678-679`). Zeilenanker waren teils um 200 Zeilen verschoben (`groupTitle` `group_header_row.dart:149-152`; CONVENTIONS 4.15 läuft `:637-696`, „bis HUM-031 den Katalog liefert" steht `:695`; 4.22 beginnt `:1273`, die Oberflächen-Hälfte steht `:1420`; HAR-Prüfung `run.sh:712-713`, xvfb `:703`, Zähler-Zweig `:748-753`, Apex gegen Host `:406-416`). Richtig waren `BACKLOG.md:395`, `:457`, `:459`, `app_en.arb:801`, `app_de.arb:315`, `action_bar.dart:385-389`, `held_groups.dart:56`, `:168`, `:184`, `selection_card.dart:51`, `ci.yml` 516-569.

**Minor, offen:** `icon` bleibt tot (`make catalog-assets` kopiert nur die YAML, `catalog/icons/` hält 12 SVG, `app/pubspec.yaml` hat keinen SVG-Renderer) und `source` fehlt der Karte, obwohl `entry.rs:150-152` es als „Pflicht, damit die Karte prüfbar bleibt" führt; entweder `catalog/icons/` mitbündeln samt Renderer oder die Zusage „Icon gebündelt" in `BACKLOG.md:459` zurücknehmen. Ein relativer Symlink `app/assets/catalog/domains.yaml -> ../../../catalog/domains.yaml` würde Kopie, `catalog-lint` und Driftgefahr ersetzen (`Makefile:57-58`: Erzeugtes ist keine Quelle und steht nicht im Repository). Beides entscheidet der Projekteigentümer beziehungsweise der Bau, nicht dieser Abschnitt.

**Empfehlung des Audits, nicht entschieden:** nicht als ein Issue bauen. (a) Größe S, sofort, reine Dokumentation: `BACKLOG.md:395` und `CONVENTIONS.md:695` korrigieren; damit ist die unwahre Zusage weg, ohne eine Zeile Code. (b) Größe M, ohne Vertragsänderung: Asset (Kopie oder Symlink), `CatalogEntry`, `catalogProvider`, `DomainPanel` aus `flowDetailProvider(id).value?.domain`, drei Goldens, `quick_rule_calls_rules_add`. (c) Größe M, nach der Entscheidung oben und nach HUM-091: Gruppierung auf dem Apex des Daemons, `HeldGroup.catalogId` und `apexFromDaemon`, Kopf, ARB, Sonderregel des Modals. Danach der Nachtrag in HUM-097s Lauf. Der Zuschnitt ist Sache des Projekteigentümers; XL gilt für das ungeteilte Issue.

### Fallstricke
- Der Kopf darf den Katalognamen nur führen, wenn er vom Daemon kommt. `catalog_id` kommt von dort, der Apex seit HUM-091 aus `FlowSummary.apex`, und `backlog/CONVENTIONS.md` 4.13 und 4.15 verbieten eine geratene Domäne in einer Zeile, die eine Entscheidung bewacht. Tragen zwei gehaltene Anfragen einer Gruppe verschiedene `catalog_id` oder gar keine, bleibt es bei „Host und n weitere".
- `psl.dart` entfällt mit HUM-091; der Rückfall ist der Host (`flow.apex.isEmpty ? flow.host : flow.apex`), nicht ein Rat. Das Modal für Entscheidungen über mehrere Hosts bleibt, mit der Begründung aus HUM-091 (es listet die Hosts auf). Der frühere Fallstrick „wer `psl.dart` löscht, nimmt dem Fake-Modus die Gruppierung" gilt nicht mehr: die Fake-Szenarien setzen `apex`.
...
- `run.sh:712-713` erwartet 17 HAR-Einträge, `backlog/sprint-2.md:1669` schreibt 16. Der Lauf trägt die neuere Zahl; wer 16 einsetzt, macht den Schritt rot.
- Die Auflösung unter xvfb bleibt `1600x1000x24` (`run.sh:703`). Darunter greift das schmale Layout, und das rechte Pane ist gar nicht gezeichnet.
- `M2_EXPECTED_ASSERTIONS` muss von Hand nachgezogen werden. Eine zu niedrige Zahl meldet das Skript nur als Hinweis (`run.sh:751-753`) und bleibt grün; nur eine zu hohe bricht ab (`:748-749`). Genau darum steht sie in den Akzeptanzkriterien.
...
Sprint: 2 · Größe: M · Abhängigkeiten: HUM-027, HUM-036, HUM-089 (läuft davor, gleiches Literal), Entscheidung des Projekteigentümers gegen HUM-090 (siehe Stand) · Blockiert: keine (HUM-078 vergleicht Subkommandos, nicht Flags)
...
### Betroffene Pfade
- `daemon/bin/humanitl/src/cli.rs`: neue Struktur `RememberArgs` neben `RuleArgs` (Zeile 384); `FlowsCmd::Decide` (Zeilen 258-268) bekommt sie als `#[command(flatten)]`
...
- `daemon/bin/humanitl/src/cmd/rules.rs`: `rule_from_args` (Zeile 796) und `rule_json` (Zeile 708) werden `pub(crate)`
- `daemon/bin/humanitl/tests/cli.rs`: neue Fälle (heute kein einziger Treffer für `created_from` in der Datei)
- `tests/e2e/lib.sh`: `flow_decide` (Zeilen 444-451)
- `tests/e2e/m2_first_decision/run.sh`: Schritt 2 (Zeilen 418-472)
- `backlog/CONVENTIONS.md`: Absatz „Die Stapel-Freigabe geht über zwei Aufrufe" in 4.22 (Zeilen 1368-1386) und der Absatz zum Haltebudget (Zeilen 1393-1400), der dieses Issue mit „einem einzigen `Decide` für den Stapel" verwechselt (siehe Stand)
...
### Akzeptanzkriterien
- [x] `humanitl --json flows decide <held-id> allow --remember '**.npmjs.org'` liefert Exit 0, und `jq -r '.created_rule.created_from_flow_id'` der Ausgabe ist genau `<held-id>`, nicht `""` (`decide_remember_carries_origin`). Gemessen: Test grün gegen den Fake, gehaltener Flow `018f0001-0000-7000-8000-000000010000`; Mutation „Zuweisung von `created_from_flow_id` gestrichen" macht ihn rot (`cli.rs:2686`, und in der Regelliste `cli.rs:2799`, je `left: String("")`). Derselbe Test prüft den Klartext: zwei Zeilen, `allow 018f0001` und `rule <id8> allow **.npmjs.org session`; Mutation „zweite Zeile gestrichen" macht ihn rot (`cli.rs:2719`, eine Zeile statt zwei).
- [x] Danach gibt `humanitl --json rules list | jq -r --arg id <created_rule_id> '.rules[] | select(.rule_id == $id) | .created_from_flow_id'` dieselbe Id aus, und `.expires.kind` ist `session`, obwohl kein `--remember-expires` gesetzt war (`decide_remember_defaults_to_session`). Gemessen: Test grün, prüft zusätzlich `--remember-expires never`; Mutation „Vorgabe `session` entfernt" macht ihn rot (`cli.rs:2792`, `left: String("never")`).
- [x] `humanitl --json flows decide <held-id> allow` ohne `--remember` gibt weiterhin genau `{"flow_id","decision","note","applied"}` aus, ohne `created_rule_id`, und `rules list` zählt dieselbe Zahl Regeln wie vor dem Aufruf (`decide_without_remember_creates_no_rule`). Gemessen: Test grün, vier Schlüssel, Regelzahl gleich; Mutation „`created_rule_id`/`created_rule` immer ausgeben" macht ihn rot (`cli.rs:2754`, sechs Schlüssel statt vier).
- [x] `humanitl flows decide <held-id> block --remember '**.evil.example' --note 'use PyPI' --remember-note 'blocked group'` legt eine Regel mit `.action == "block"` und `.note == "blocked group"` an; `use PyPI` steht nicht in `.note` (`decide_remember_note_is_not_the_agent_note`). Gemessen: Test grün, prüft zusätzlich `--remember-method GET` und `--remember-path /packages/**`; Mutation „`--remember-note` verworfen" macht ihn rot (`cli.rs:2858`, `left: String("")`).
- [x] `humanitl flows decide <held-id> allow --remember 'cidr:not-an-address'` liefert Exit 1 mit dem Befund des Daemons, `rules list` hat keine neue Regel, und `flows list --filter 'state:held'` enthält den Flow weiter — die Entscheidung fällt nicht ohne die Regel (`decide_remember_bad_pattern_decides_nothing`). Gemessen: Test grün; der Befund ist `RULES_003` „cidr needs a prefix length", nicht `IPC_005`. Mutation „Host-Muster fest verdrahtet statt aus dem Flag" macht ihn rot (`cli.rs:2886`, Exit 0 statt 1).
- [x] `humanitl flows decide not-a-uuid allow --remember '**.example.com'` liefert Exit 1 mit `IPC_004` und der Zeile über die Flow-Id, nicht mit `IPC_005` über `created_from_flow_id`; `rules list` bleibt unverändert (`decide_remember_bad_flow_id_keeps_ipc_004`). Gemessen: Test grün; Mutation „`FlowId::parse`-Sperre entfernt" macht ihn rot (`cli.rs:2921`, `IPC_005: created_from_flow_id is not a valid id`).
- [x] `cargo test -p humanitl --test cli` grün; `grep -c created_from daemon/bin/humanitl/tests/cli.rs` ist größer als 0 (heute 0). Gemessen: 82 Tests in `tests/cli.rs`, 0 Fehlschläge (davon 7 neue: die sechs des Issues plus `remember_flags_without_a_pattern_are_a_usage_error`); `grep -c created_from` ist 4. Ganzes Paket: 150 + 82 + 4 Tests grün. `remember_flags_without_a_pattern_are_a_usage_error` ist der einzige Test, der das Entfernen der vier `requires = "remember_host"`-Bindungen fängt: Unter dieser Mutation endet er mit Exit 2 (`DAEMON_001`) statt 1, während die sechs anderen grün bleiben. Er ist deshalb nicht überflüssig und darf nicht als Dublette gestrichen werden.
- [x] `tests/e2e/m2_first_decision/run.sh` Exit 0, `grep -c 'rules add' tests/e2e/m2_first_decision/run.sh` ist 0, und Schritt 2 prueft, dass `created_from_flow_id` der einen Sitzungsregel die Id des zuerst freigegebenen Flusses ist. **Gemessen am 2026-09-12 auf dem fertigen Stand** (`2feb41a`): `M2_UI=0 E2E_ONLY=m2 ./tests/e2e/run.sh` endet mit Exit-Code 0 — direkt abgegriffen, nicht durch eine Pipe — und meldet 71 geprufte Behauptungen; `grep -c 'rules add'` ergibt 0; die Behauptung „and it names the request a human released first“ vergleicht `jq -r '.created_from_flow_id'` der Regel mit `$M2_RULE_FROM`, der Id des Flusses, den die Schleife zuerst freigegeben hat.
- [ ] Im Regel-Bildschirm trägt diese Regel das Herkunfts-Abzeichen (`rulesOriginFlow`, letztes UUID-Segment) und der Klick öffnet die Anfrage; ohne den Fix bleibt die Zeile leer (Blick, `app/lib/features/rules/widgets/rule_row.dart`:508-526). **Offen, Stand 2026-09-12:** Das Abzeichen selbst und sein Klick sind als Widget-Test gemessen — `app/test/features/rules/rule_origin_round_trip_test.dart` fährt den Weg von der Regel zurück zur Anfrage, und `rules_screen_test.dart` zeichnet eine Regel mit Herkunft. Was fehlt, ist der Blick auf eine Regel, die `humanitl flows decide --remember` gegen einen echten laufenden Daemon angelegt hat; das ist Sache eines Menschen.
- [x] `backlog/CONVENTIONS.md` 4.22 enthält den Absatz „Die Stapel-Freigabe geht über zwei Aufrufe" nicht mehr, und `backlog/sprint-4.md` Zeile 1629 nennt `--remember` in der Paritätszeile für `Humanitl.Decide`. Gemessen: `grep -c "Die Stapel-Freigabe geht über zwei Aufrufe"` ist 0; die Paritätszeile steht heute auf `backlog/sprint-4.md:1655` und lautet `humanitl flows decide <id> allow|block [--note] [--remember PATTERN]`.
- [x] `make check` grün. **Gemessen am 2026-09-12 auf dem fertigen Stand** (`2feb41a`): `make check` endet mit Exit-Code 0. Vorher schon einzeln gemessen: `cargo fmt --all -- --check`, `cargo clippy -p humanitl --all-targets -- -D warnings`, `RUSTDOCFLAGS="-D warnings" cargo doc -p humanitl --no-deps`, `cargo test -p humanitl` (236 Tests), `tools/check-deps.sh`, `scripts/ci/lint-docs.sh`, `scripts/ci/lint-no-string-errors.sh`, alle ohne Befund. Dazu `tools/verify-commit.sh` über den Commit dieses Issues, grün.

### Stand (2026-09-04): Größe M, Überschneidung mit HUM-090, kein Vorlauf im Code

Geprüft am Code (Audit 2026-09-04, Zeilen gegen den heutigen Baum gezogen). Die Kette steht: `DecideRequest.remember` (`humanitl.proto:597`), `Rule.created_from_flow_id` (`rules.proto:68`), der Dienst legt die Regel vor der Entscheidung an (`server.rs:892-903`), nimmt sie zurück, wenn kein Flow wirkte (`:906-919`), und antwortet mit `created_rule_id` und `created_rule` (`:929-935`); `RulesService::remember` (`rules.rs:150-158`) reicht die Regel des Clients unverändert durch, `rule_from_proto` übernimmt `created_from` nur bei nicht-leerem Feld (`convert.rs:1305-1313`), der Fake macht dasselbe (`fake/mod.rs:520-532`) und lehnt ein kaputtes Host-Muster über `HostPattern::parse` ab. Die CLI schickt `remember: None` (`flows.rs:338`), `rule_from_args` fasst `created_from` nie an (`rules.rs:796-851`). Kein Proto-, kein Daemon-Code nötig.

**Überschneidung mit HUM-090 (blockierend, Entscheidung des Projekteigentümers).** HUM-090 (`backlog/sprint-4.md:1707`, `BACKLOG.md:518`) baut dieselbe Fähigkeit in unvereinbarer Form: `--remember` als Schalter plus `--remember-host PATTERN` statt `--remember <PATTERN>`; `--json` als `results[]` plus `created_rule`, und „die alte Ein-Objekt-Form entfällt", während dieses Issue verlangt, dass ohne `--remember` genau `{flow_id, decision, note, applied}` bleibt (`flows.rs:371-377` heute). Beide setzen `created_from_flow_id`, beide bilden `RememberArgs` auf `RuleArgs` ab, beide heben `rule_from_args` auf `pub(crate)`, beide bauen Tests in `daemon/bin/humanitl/tests/cli.rs`, beide schreiben `run.sh` Schritt 2 um, beide ändern CONVENTIONS 4.22 und `sprint-4.md:1629`. Wer zuerst läuft, zwingt den anderen zum Umbau einer gerade veröffentlichten Flagge. **Entschieden am 2026-09-12 vom Projekteigentümer: Es gilt die Form dieses Issues.** `humanitl flows decide <ID> allow|block --remember <PATTERN>` nimmt das Muster direkt am Flag; ohne `--remember` bleibt `--json` Byte für Byte das heutige Einzelobjekt `{flow_id, decision, note, applied}`, mit `--remember` kommen `created_rule_id` und `created_rule` dazu. HUM-090 übernimmt später dieselbe Flagge und entscheidet dann nur noch die Form für mehrere Ids; `--remember-host` entfällt. Derselbe Absatz steht bei HUM-090.

**Warum M statt S:** `daemon/bin/humanitl/tests/cli.rs` enthält in rund 1500 Zeilen keinen einzigen `flows decide`-Test (`grep -c` ist 0, ebenso für `created_from`); die sechs geforderten Fälle sind die erste CLI-Abdeckung von `decide` überhaupt, jeder gegen `FakeServer` (`cli.rs:129-136`). `run.sh` Schritt 2 (`:418-472`) wird spröder: die Regel hängt danach am ersten `Decide`, und verfällt dieser Flow innerhalb der 10 s (`config.toml:31`), nimmt der Dienst die Regel zurück (`server.rs:906-919`); heute steht sie unabhängig davor (`:433`). Dazu zwei Absätze in CONVENTIONS 4.22 und die Abstimmung mit HUM-090.

**Widersprüche, im Text korrigiert oder hier vermerkt:**

- `Blockiert: HUM-078` war falsch; der eigene Kontext sagt es (`parity-check` vergleicht Subkommandos, nicht Flags, `docs/adr/0018-rpc-parity.md:41-43`), und `sprint-4.md:1629` führt `Humanitl.Decide` schon als abgedeckt. HUM-090 trägt dieselbe falsche Kopfzeile.
- CONVENTIONS `:1393-1400` schreibt, das 10-Sekunden-Haltebudget des M2-Laufs „verschwindet mit HUM-095: Mit einem einzigen `Decide` für den Stapel". Dieses Issue schließt den Stapel aus (Nicht-Ziel „Keine mehreren Ids je Aufruf", Fallstrick „Zwölf Aufrufe mit dem Flag legen zwölf Regeln an"); nach ihm laufen weiter zwölf Prozesse gegen die 10 s. Der Nutzen gehört zu HUM-090s `--also`. Der Absatz steht jetzt in „Betroffene Pfade" und darf HUM-095 keinen Stapel mehr zuschreiben.
- CONVENTIONS `:1385-1386` und HUM-097 behaupteten, das Abzeichen `from #1` sei auch für die Oberflächen-Hälfte unerfüllbar, „weil die Regel neben der Entscheidung entsteht statt in ihr". Das gilt nur für die Kommandozeile: die Oberfläche legt die Regel in der Entscheidung an (`decision.dart:619` `remember: i == 0 ? rule : null`, `rule_sentence.dart:117` `createdFrom: flow.id`, über die Leitung `convert.dart:502-518`, Abzeichen `rule_row.dart:378-395`). Ein Bildschirm-Lauf zeigt `from #1` ohne dieses Issue; HUM-097 ist entsprechend korrigiert, CONVENTIONS 4.22 beim Bau.
- Zeilenanker in Kontext und Pfadliste waren 20 bis 165 Zeilen verschoben; richtig sind `cli.rs:258-268` (`Decide`), `:264-266` (`--note`), `:384` (`RuleArgs`), `rules.rs:796` (`rule_from_args`), `:708` (`rule_json`), `:730`, `convert.rs:179`, `:1305`, `:1339-1345` (`expiry_from_proto`), `rules_store.rs:369` (`add`), `validate.rs:123` (`flow_id`), `server.rs:892-903`, `rules.rs:150-158`, `lib.sh:444-451`, `run.sh:433`, `:463`, Schritt 2 `:418-472`, CONVENTIONS 4.22 `:1368-1400`. Nur die `flows.rs`-Angaben stimmten.

**Reihenfolge:** nach HUM-089; beide schreiben das Literal `daemon/bin/humanitl/src/cmd/flows.rs:336-340` (HUM-089 streicht `:339`, dieses Issue ersetzt `:338`). Nicht parallel.

**Ohne automatisierten Nachweis:** das Sicht-Kriterium zum Herkunfts-Abzeichen braucht einen gebauten Client gegen einen echten Daemon; zwei Kriterien brauchen einen lauffähigen `tests/e2e/m2_first_decision/run.sh` (eigener Nutzer- und Netz-Namensraum, voller Build). Vor dem Commit den Lauf mehrfach fahren, weil die Regel dann am ersten `Decide` hängt.
...
### Akzeptanzkriterien
- [x] Der Test liegt in `app/integration_test/` und läuft unter `make flutter-test-integration` grün, und ein Schritt in CI fährt ihn ebenfalls. **Beide Hälften gemessen am 2026-09-12.** Lokal: `xvfb-run -a --server-args='-screen 0 1600x1000x24' make flutter-test-integration` endet mit 0 in 103 s, davon 25 s für `queue_freeze_test.dart` (das Szenario läuft ohne Zeitraffer) und 6 s für `shell_test.dart`; der M2-Treiber wird laut übersprungen, solange `HUMANITL_E2E_HAR` fehlt. In CI: der Schritt „The app on a screen against a real daemon (HUM-144)" im Job `e2e-xvfb` ist grün, zuerst im Lauf 34702301774 über `a0114d1` in 91 s, danach wieder im Lauf 34707089301 über `2feb41a`.
...
Was fehlt, ist die Oberflächen-Hälfte, und die Naht steht **nicht**: Schritt 10 von `run.sh` liegt hinter dem Ende des Agenten (`:517` `wait "$M2_AGENT_PID"`, `:671` Schritt 10), also zu einem Zeitpunkt, an dem nichts mehr gehalten wird. Der Env-Block ist da, der Platz ist falsch (siehe Stand).
...
### Nicht-Ziel
Keine Änderung an Fake-Upstream, Fake-Agent, `script.json` oder `config.toml`. Kein neuer RPC. Keine Golden-Tests (HUM-054). Nicht das Umstellen des Laufs auf `https://` — das ist HUM-087. Nicht die Herkunft der Sitzungsregel auf der Kommandozeile — das ist HUM-095; für die Oberfläche gilt das nicht, sie legt die Regel in der Entscheidung an (`decision.dart:619`, `rule_sentence.dart:117`), und das Abzeichen `from #1` ist hier prüfbar.

### Betroffene Pfade
- `app/integration_test/m2_first_decision_test.dart` (neu)
- `app/pubspec.yaml` (`integration_test` als Dev-Abhängigkeit, falls sie fehlt)
- `tests/e2e/m2_first_decision/run.sh`: der Bildschirm-Treiber startet im Hintergrund vor Abschnitt 2, Abschnitte 2 bis 4 laufen nur ohne Oberfläche, Abschnitte 6 und 8 bekommen Regel-Id und Stapel-Ids aus dem laufenden Treiber (`:439`, `:459-460`, `:565`, `:626`, `:632`), Schritt 10 behält die HAR-Prüfung; `M2_EXPECTED_ASSERTIONS` (`:112`, `:748`) wird je Zweig geführt, im Bildschirm-Zweig fällt die Zahl
- `.github/workflows/ci.yml`: Job `e2e-xvfb` (Zeilen 516-566) führt `flutter pub get` und `flutter build linux --debug` aus (heute nur der Flutter-Job, `:317`, `:321`) und lädt Screenshots hoch, falls das Kriterium bleibt
- `app/lib/features/history/export/history_export.dart`, `providers/history_export.dart`: ein zweites `HistoryExportTarget`, das nach `HUMANITL_E2E_HAR` schreibt, gewählt über die Startoptionen (`app/lib/core/ipc/launch_options.dart`)
- `app/lib/features/history/export/har.dart:75-76`: Wire-Namen `timed_out` und `block_reason` (siehe Stand)
- `app/test/features/history/history_export_flow_test.dart` und ein Widget-Test für das neue Ziel
...
**Der Env-Block, den `run.sh` heute stellt** (Schritt 10, `:695-705`; der Block bleibt, der Aufruf wandert vor Abschnitt 2, siehe Stand):
...
- `XDG_RUNTIME_DIR` genügt, um den Daemon zu finden: `DaemonPaths.resolve` in `app/lib/core/ipc/daemon_paths.dart` leitet Socket und Token daraus ab, genau wie `humanitl_config::Paths` auf der Rust-Seite. `HUMANITL_SOCKET` und `HUMANITL_TOKEN` stehen daneben, damit der Test die Pfade nehmen kann, ohne sie herzuleiten. `HUMANITL_TOKEN` liest heute nichts in `app/` (nur `HUMANITL_SOCKET`, `launch_options.dart:59`; der Token-Pfad wird neben dem Socket abgeleitet). `launchOptionsProvider` ist nur in `main()` überschrieben (`main.dart:51`); ein Test, der `HumanitlApp` selbst pumpt, ruft `LaunchOptions.resolve` und überschreibt den Provider, sonst redet er mit dem falschen Socket.
- `HUMANITL_E2E_HAR` ist der Pfad, unter den der Test die Export-Datei schreiben muss. `run.sh` prüft danach: Datei nicht leer, `.log.entries | length == 17`. Nichts in `app/` liest die Variable heute; das Exportziel ist `FilePickerExportTarget` (`providers/history_export.dart:34`), also `FilePicker.saveFile` (`export/history_export.dart:184`) und das XDG-Portal über D-Bus, das es unter xvfb nicht gibt. Die Naht ist ein zweites `HistoryExportTarget`, das aus den Startoptionen gewählt wird, damit der Lauf denselben Kodier- und Schreibweg prüft wie ein Mensch; ein bloßes Override des Providers im Test ließe den Produktivweg ungeprüft (CONVENTIONS 4.13, 4.22).
- Die Auflösung ist Vorgabe und kein Vorschlag: unter 1400×900 greift das schmale Layout, und die Selektoren scheitern.
- Der Daemon läuft bereits, die Sitzung ist offen, der Agent hat seine ersten Anfragen gestellt, und sie werden gehalten. Der Test verbindet sich, er startet nichts. Dafür muss `run.sh` ihn vor Abschnitt 2 starten, nicht in Schritt 10 nach `wait "$M2_AGENT_PID"`.

**Die Schritte auf dem Bildschirm** (aus HUM-036, mit dem Stand der Bildschirme von heute):

1. Warten, bis die Warteschlange drei Gruppen zeigt: `registry.npmjs.org` 12, `api.github.com` 2, `evil.example` 1. Zusicherungen: npm-Gruppe eingeklappt, `0 findings`; github-Gruppe 1 Fund (Mailadresse); evil 1 Fund (AWS-Schlüssel). Keine Zusicherung über die Karte „unbekannte Domäne": `domain_pane_placeholder.dart:79` zeichnet sie für jeden ausgewählten Flow, auch für `registry.npmjs.org`, bis HUM-094 die Katalog-Karte liefert.
...
**Was heute noch nicht geht und deshalb nicht Teil der Akzeptanz ist:** der Katalogname „Looks like: npm install" und `Edit + Allow` (HUM-094, HUM-047). Das Abzeichen `from #1` an der Sitzungsregel geht dagegen heute schon: die Oberfläche legt die Regel in der Entscheidung an (`decision.dart:619`, `rule_sentence.dart:117`), das Abzeichen zeichnet `rule_row.dart:378-395`; Schritt 8 aus HUM-036 wird hier geprüft.

**Wer entscheidet.** `run.sh` treibt die Entscheidungen heute selbst über `humanitl`. Beide Treiber zugleich wären ein Wettlauf. Der Lauf bekommt deshalb einen Schalter: mit Oberfläche entscheidet der Bildschirm, im Hintergrund gestartet, bevor Abschnitt 2 begänne, und die Abschnitte 2 bis 4 des Skripts entfallen; ohne Oberfläche bleibt es beim heutigen Weg. Die Abschnitte 5 bis 9 sind **nicht** treiberunabhängig: `:565` und `:626` prüfen gegen `M2_RULE_ID` (gesetzt `:439`, Abschnitt 2), `:632` liest `M2_BATCH_IDS` (`:459-460`, Abschnitt 2). Der Bildschirm-Zweig muss beide liefern (die Regel-Id über `humanitl --json rules list` nach der Freigabe, die Stapel-Ids über `m2_ids` davor), sonst vergleichen drei Behauptungen in 6 und 8 gegen einen leeren String.

### Schritte
1. `integration_test` steht in `app/pubspec.yaml:61`. Kein Gate führt `app/integration_test/shell_test.dart` aus (`Makefile:77` und `ci.yml:310` prüfen nur `test/`): `xvfb-run -a --server-args='-screen 0 1600x1000x24' flutter test integration_test/shell_test.dart -d linux` muss lokal und in einem Wegwerf-Branch in CI grün sein, mit gemessener Laufzeit, bevor irgendetwas behauptet wird.
2. `flutter pub get` und `flutter build linux --debug` in den Job `e2e-xvfb`, `PUB_CACHE` durch den Env-Block von Schritt 10; Schritt 1 in `unshare -rn` wiederholen (der Lauf hat kein Netz, `lib.sh:222-245`, und ein frisches `HOME`, `run.sh:700`).
3. Exportziel nach `HUMANITL_E2E_HAR` als zweites `HistoryExportTarget`, über die Startoptionen gewählt; Widget-Test. `har.dart:75-76` auf die Wire-Namen, `history_export_flow_test.dart` erweitern.
4. `run.sh` umbauen: Bildschirm-Treiber im Hintergrund vor Abschnitt 2, HAR-Prüfung in Schritt 10, Abschnitte 2 bis 4 bedingt, Regel-Id und Stapel-Ids aus dem laufenden Treiber, `M2_EXPECTED_ASSERTIONS` je Zweig.
5. Einen `pumpUntil`-Helfer mit Frist schreiben; feste `sleep`s nur für den Schritt, der die Zeitüberschreitung abwartet. `LaunchOptions.resolve` und `launchOptionsProvider` überschreiben. Die sieben Schritte oben, jeder mit seiner Zusicherung; die zwölf npm-Flüsse zuerst, denn das 10-Sekunden-Budget (`config.toml:31`) geht an zwölf sequenzielle `Decide`-Aufrufe (`decision.dart:610-624`) plus je 350 ms `AllowArmed`-Rearm (`motion.dart:49`).
6. Screenshot-Weg festlegen (siehe Akzeptanzkriterien), dann erst `CONTRIBUTING.md:51-65`, `backlog/CONVENTIONS.md` 4.22 und den Abschnitt „Stand" in HUM-036 zurücknehmen.
...
### Akzeptanzkriterien
- [x] `tests/e2e/m2_first_decision/run.sh` Exit 0 mit der Oberflächen-Hälfte, lokal und im Job `e2e-xvfb`. (Lokal erfüllt: `E2E_SKIP_BUILD=1 ./tests/e2e/m2_first_decision/run.sh` endet mit 0, 75 Behauptungen, 47 s, 2026-09-12. Offen bleibt die zweite Hälfte des Kriteriums: der Job `e2e-xvfb` hat diesen Stand noch nicht gefahren.) **Im CI gemessen am 2026-09-12:** Lauf 34688379715 über `800d9c9`, Job `e2e-xvfb` grün, Schritt „M2 demo (grouping, batch release, block, timeout, history, export)" 83 s; der Job baut die App vorher selbst („Build the Linux app for the screen half", 30 s), sodass der Treiber nicht in die Haltefrist hinein baut.
...
- [x] Laufzeit unter 4 Minuten in CI. (Lokal 47 s für den ganzen Lauf, davon 20 s für den Bildschirm-Treiber selbst, mit warmem CMake-Baum und `E2E_SKIP_BUILD=1`, 2026-09-12. In CI ungemessen; der Job baut die Anwendung in einem eigenen Schritt davor.) **Im CI gemessen am 2026-09-12:** Der Schritt mit dem Lauf dauert 83 s, der ganze Job `e2e-xvfb` 269 s samt Werkzeugen, Paketen, Codeerzeugung und App-Bau. Das Kriterium meint den Lauf; beide Zahlen stehen hier, damit niemand die eine für die andere hält.
...
- **Schritt 10 läuft, wenn nichts mehr gehalten wird.** Er beginnt bei `run.sh:671`, nach `wait "$M2_AGENT_PID"` (`:517`, Schritt „the agent runs to its end", `:515`). Alle 17 Flüsse sind dann entschieden, geblockt oder verfallen. Ein Bildschirm, der sich dort verbindet, kann keine Entscheidung treiben. Der Treiber muss laufen, **während** gehalten wird, also vor `:425`; das ist ein Umbau von `run.sh` in einen Hintergrund-Lauf der Oberfläche plus späte Export-Prüfung, kein Schalter. Die Sätze „Die Naht steht" und „Schritt 10 unverändert zu übernehmen" waren falsch und sind oben korrigiert.
- **Die Abschnitte 5 bis 9 sind nicht treiberunabhängig.** `run.sh:565` (`e2e_expect "by the session rule, not by a human" "$M2_RULE_ID"`) und `:626` (`m2_count "rule:$M2_RULE_ID"`) prüfen gegen `M2_RULE_ID`, gesetzt in Abschnitt 2 (`:439`); `:632` liest `M2_BATCH_IDS` (`:459-460`, Abschnitt 2). Ohne Abschnitt 2 vergleichen drei Behauptungen in 6 und 8 gegen einen leeren String. Oben korrigiert.
- **Kein Netz, fremdes `HOME`, keine gebaute App.** `run.sh:700` setzt `HOME="$E2E_HOME"` (frisches `mktemp`, `lib.sh:335`), `:699` verbiegt `XDG_CONFIG_HOME`, der ganze Lauf steckt in `unshare -rn` (`lib.sh:222-245`), `app/.dart_tool` ist ignoriert (`app/.gitignore:29`), und der Job `e2e-xvfb` (`ci.yml:516-566`) führt weder `flutter pub get` noch `flutter build linux` aus; das tut nur der Flutter-Job (`ci.yml:317`, `:321`). Das erste `flutter test integration_test/... -d linux` löst also Pakete aus einem leeren `$HOME/.pub-cache` ohne Netz auf und baut die GTK-App von Null. Nötig: `flutter pub get` und `flutter build linux --debug` im Job `e2e-xvfb` und `PUB_CACHE` durch den Env-Block in Schritt 10.

**Major, im Text korrigiert oder hier vermerkt:**

- **HAR-Feldnamen.** `har.dart:75-76` schreibt `'decision': flow.decision?.name` und `'block_reason': flow.blockReason?.name`; der Enum heißt `timedOut` (`flow_state.dart:54`). Die Datei trägt also `"decision":"timedOut"` und den Schlüssel `block_reason`, nicht `timed_out` und `reason`. Kriterium und Pfadliste oben gefasst: `har.dart` bekommt die Wire-Namen, das schließt zugleich die Drift gegen CONVENTIONS `:1410` (der Daemon schreibt `timed_out`).
- **Der Export geht durch ein Portal.** Nichts in `app/` liest `HUMANITL_E2E_HAR` (Treffer nur in `run.sh:703`); das Exportziel ist `FilePickerExportTarget` (`providers/history_export.dart:34-36`) über `FilePicker.saveFile` (`export/history_export.dart:184`), das XDG-Portal über D-Bus. Unter nacktem xvfb gibt es kein Portal; die Spezifikation nannte keinen Mechanismus. Oben ergänzt.
- **`takeScreenshot` hat keinen Treiber.** Kein `app/test_driver`, Schritt 10 ruft `flutter test`, nicht `flutter drive` (`run.sh:705`); `IntegrationTestWidgetsFlutterBinding.takeScreenshot` hat unter `flutter test` niemanden, dem es die Bytes gibt. Kriterium oben gefasst.
- **`M2_EXPECTED_ASSERTIONS` fällt im Bildschirm-Zweig, statt zu steigen.** Abschnitte 2 und 3 tragen rund zehn Behauptungen (`e2e_check`, `e2e_expect`, `m2_expect_count` zwischen `:425` und `:494`), Schritt 10 zwei (`:708`, `:712`); eine Konstante (`:112`, geprüft mit `-lt` bei `:748`) muss je Zweig gelten, sonst stirbt die Selbstprüfung mit „a branch was skipped". Pfadliste oben gefasst.
- **Das Abzeichen `from #1` braucht HUM-095 nicht.** Die Oberfläche legt die Regel in der Entscheidung an (`decision.dart:619`, `rule_sentence.dart:117`); Nicht-Ziel und der Absatz „Was heute noch nicht geht" sind korrigiert. CONVENTIONS `:1385-1386` behauptet dasselbe Falsche und wird beim Bau nachgezogen.
...
## HUM-105 · Schalter „Deaktivieren" für mitgelieferte Regeln
Sprint: 2 · Größe: M · Abhängigkeiten: HUM-033, HUM-038 (Daemon-Hälfte gemerged, `215447b`) · Blockiert: keine

### Kontext
Der Daemon kann eine mitgelieferte Regel abschalten, die Oberfläche nicht. HUM-038 hat `Rule.disabled` (`daemon/crates/core-types/src/rule.rs:527`), die Operation `RulesRequest.set_disabled` (`proto/humanitl/v1/humanitl.proto:650-657`), das Feld `Rule.disabled = 13` (`proto/humanitl/v1/rules.proto:88`), `RulesStore::set_bundled_disabled` (`daemon/crates/proxy/src/rules_store.rs:595`), die Kommandozeile `humanitl rules disable|enable ID` (`daemon/bin/humanitl/src/cli.rs:341-352`, `cmd/rules.rs:153-166`) und die Parität im Rust-Fake (`daemon/crates/ipc/src/fake/mod.rs:687-693`) geliefert; `humanitl rules list --all` zeigt eine abgeschaltete Regel als `bundled (off)` (`cmd/rules.rs:610`).

In `app/` kommt davon nichts an. `app/lib/core/domain/rule.dart:65-80` kennt kein `disabled`, `RuleToDomain` in `app/lib/core/ipc/convert.dart:437-452` liest das Feld nicht aus der Proto und `RuleToProto` (`:455-471`) schreibt es nicht, und `DaemonClient` (`app/lib/core/ipc/daemon_client.dart`) hat keine Operation dafür, also auch `GrpcDaemonClient` und `FakeDaemonClient` nicht. Der Regel-Bildschirm zeichnet eine mitgelieferte Regel mit Schloss und dem Wort `bundled` (`app/lib/features/rules/widgets/rule_row.dart:365-374`) und ohne Papierkorb (`onDelete == null`, `:187-196`); einen Schalter gibt es nicht. Folge: eine Regel, die der Nutzer über die Kommandozeile abgeschaltet hat, steht auf dem Bildschirm wie eine wirksame. Das ist die Behauptung, die `backlog/CONVENTIONS.md` 4.13 ausschließt. HUM-038 führt die Lücke in seinem Stand als offen (`backlog/sprint-3.md:386-395`) und in seinem zweiten Akzeptanzkriterium als nicht erfüllt (`:318`); ein eigenes Issue gab es bis heute nicht. Dieses Issue ist die Oberflächen-Hälfte von HUM-038.

### Ziel
Im Regel-Bildschirm, Tab „Gespeichert", Block der mitgelieferten Regeln: Jede Zeile trägt an der Stelle, an der eine eigene Regel den Papierkorb hat, einen Schalter „Ausschalten" beziehungsweise „Einschalten". Eine abgeschaltete Regel wird wie eine abgelaufene gezeichnet (gedämpft), ihr Herkunftswort lautet „mitgeliefert, ausgeschaltet", und ihr Schalter liest „Einschalten". Der Klick schickt `RulesRequest.set_disabled`; die Liste zeigt danach den Regelsatz aus der Antwort des Daemons, nie einen lokal vorweggenommenen Zustand. `humanitl rules disable ID` und der Bildschirm zeigen zu jeder Zeit denselben Zustand. Der Dart-Fake antwortet wie der Daemon.

### Nicht-Ziel
Keine eigene Regel wird abgeschaltet; der Daemon lehnt das mit `RULES_010` ab (`rules_store.rs:598-603`), und die Oberfläche bietet den Schalter dort nicht an (eine eigene Regel löscht man). Kein Löschen mitgelieferter Regeln. Keine Änderung an Daemon, Proto oder Kommandozeile. Kein Rückgängig-Streifen: der Schalter ist sein eigenes Rückgängig. Keine Änderung der Auswertungsreihenfolge (HUM-104, CONVENTIONS 4.5).

### Betroffene Pfade
- `app/lib/core/domain/rule.dart` (ändern: `@Default(false) bool disabled`), `rule.freezed.dart`, `rule.g.dart` (Regenerat)
- `app/lib/core/ipc/convert.dart` (ändern, geteilte Datei: `RuleToDomain` liest `disabled`, `RuleToProto` schreibt es)
- `app/lib/core/ipc/daemon_client.dart` (ändern: `Future<RuleSet> setRuleDisabled(RuleId id, {required bool disabled})`)
- `app/lib/core/ipc/grpc_daemon_client.dart` (ändern: Umsetzung über `_rules(pb.RulesRequest()..setDisabled = …)`, wie `makeRulePermanent` in `:151-152`)
- `app/lib/core/ipc/fake_daemon_client.dart` (ändern, geteilte Datei: Umsetzung neben `makeRulePermanent`, `:525`)
- `app/lib/features/rules/providers/rules.dart` (ändern: `Future<Diagnostic?> setDisabled(Rule rule, {required bool disabled})` neben `makePermanent`, `:166`)
- `app/lib/features/rules/widgets/rule_row.dart` (ändern: Schalter im `actionSlot` für `rule.bundled`, `:187-196`; Herkunftswort und Dämpfung bei `rule.disabled`, `:147-148`, `:365-374`)
- `app/l10n/app_en.arb`, `app/l10n/app_de.arb` (ändern: neue Schlüssel ans Ende der Datei)
- `app/test/core/ipc/convert_test.dart`, `app/test/features/rules/fake_rules_parity_test.dart`, `app/test/features/rules/rules_screen_test.dart`, `app/test/features/rules/fixtures.dart` (ändern: Parameter `disabled` neben `bundled`, `:39`), `app/test/goldens/rules_golden_test.dart` (ändern: `rules_list_saved_*` mit einer abgeschalteten mitgelieferten Regel)
- `backlog/CONVENTIONS.md` 4.16 (ändern: ein Absatz zum Schalter)

Nicht berührt: `daemon/`, `proto/`, `backlog/sprint-3.md` (HUM-038; sein Stand verweist nachträglich hierher, sobald dort niemand mehr arbeitet).

### Spezifikation

```dart
// app/lib/core/domain/rule.dart, in Rule
...
```dart
// app/lib/core/ipc/daemon_client.dart
/// Switches a bundled rule off or back on (`RulesRequest.set_disabled`).
/// Any other rule is refused by the daemon with `RULES_010`.
Future<RuleSet> setRuleDisabled(RuleId id, {required bool disabled});
```

`GrpcDaemonClient`: `_rules(pb.RulesRequest()..setDisabled = (pb.RulesRequest_SetDisabled()..ruleId = id.value..disabled = disabled))`. `FakeDaemonClient`: unbekannte Id ⇒ `DaemonException(_unknownRule(id))`; Regel nicht `bundled` ⇒ `DaemonException` mit `DiagnosticCodes.ruleBundled` (`RULES_010`) und dem `why` des Daemons („only bundled rules are disabled instead of removed", `rules_store.rs:600-601`); sonst `disabled` setzen, `RulesChanged` senden, Regelsatz zurückgeben.

`RulesNotifier.setDisabled` läuft über `_run` wie `makePermanent` und gibt den `Diagnostic` des Daemons zurück; kein `RuleUndo`.

`RuleRow`: für `rule.bundled` steht im `actionSlot` ein `HButton` (`variant: HButtonVariant.ghost`) mit `key: ValueKey<String>('rule-disable-$index')`, Beschriftung `rulesDisable` beziehungsweise `rulesEnable` je nach `rule.disabled`, `semanticsLabel` mit dem Host der Regel; während der Aufruf läuft, ist der Knopf deaktiviert. `_Origin` schreibt für `rule.disabled` `rulesOriginBundledOff` statt `rulesOriginBundled`. Die Zeile wird bei `rule.disabled` mit derselben Dämpfung gezeichnet wie bei `expired` (`ruleRowState`, `:147-148`).

ARB-Schlüssel, `en` Quelle, `de` Übersetzung, ans Ende beider Dateien angehängt:

- `rulesDisable`: „Switch off" / „Ausschalten"
- `rulesEnable`: „Switch on" / „Einschalten"
- `rulesDisableSemantics`: „Switch off the bundled rule for {host}" / „Die mitgelieferte Regel für {host} ausschalten"
- `rulesEnableSemantics`: „Switch on the bundled rule for {host}" / „Die mitgelieferte Regel für {host} einschalten"
- `rulesOriginBundledOff`: „bundled, switched off" / „mitgeliefert, ausgeschaltet"

### Schritte
1. `Rule.disabled`, `build_runner`, `convert.dart` beidseitig. Prüfbar: `flutter test test/core/ipc/convert_test.dart` grün mit dem neuen Fall.
2. `setRuleDisabled` in `DaemonClient`, `GrpcDaemonClient`, `FakeDaemonClient`. Prüfbar: `flutter analyze` sauber, `fake_rules_parity_test.dart` grün mit den zwei neuen Fällen.
3. `RulesNotifier.setDisabled`, Schalter und Darstellung in `rule_row.dart`, ARB. Prüfbar: `rules_screen_test.dart` grün, Golden neu abgenommen.
4. CONVENTIONS 4.16, `make check`.

### Tests
- `convert_test.dart`: `rule_disabled_survives_the_round_trip`: `pb.Rule` mit `disabled = true` ⇒ `Rule.disabled == true`; `toProto()` schreibt es zurück.
- `fake_rules_parity_test.dart`: `set_disabled_toggles_a_bundled_rule` (aus, wieder an; der zurückgegebene Regelsatz trägt den Zustand); `set_disabled_refuses_an_own_rule_with_rules_010` (Diagnostic-Code `RULES_010`, Regelsatz unverändert).
- `rules_screen_test.dart`: `bundled_switch_calls_set_disabled` (Klick auf `rule-disable-<i>` ruft `setRuleDisabled(id, disabled: true)` genau einmal); `own_rule_has_no_switch` (`rule-disable-<i>` fehlt für eine eigene Regel, `rule-delete-<i>` ist da); `disabled_bundled_row_is_drawn_off` (Herkunftswort „mitgeliefert, ausgeschaltet", Schalter liest „Einschalten").
...
- Der Zustand kommt aus der Antwort des Daemons. Ein Schalter, der vor der Antwort umspringt, zeigt für die Dauer eines Fehlschlags etwas, das nicht stimmt (4.13); deshalb deaktiviert, bis die Antwort da ist, dann die Liste aus der Antwort.
- `RuleToProto` muss `disabled` mitschreiben, sonst verliert jede Regel, die einmal durch `toProto()` läuft (Probelauf, Editor), das Feld; der Daemon lehnt `Update` für mitgelieferte Regeln zwar ab, der Konverter soll trotzdem symmetrisch sein wie bei `bundled`.
- `disabled` und `expired` sind zwei Dinge: eine mitgelieferte Regel läuft nie ab (`Expiry::Never`), eine eigene wird nie abgeschaltet. Dieselbe Dämpfung, zwei Wörter.
- Das Herkunftswort „mitgeliefert, ausgeschaltet" ist ein eigener Schlüssel, keine Verkettung: `de` und `en` setzen die Kommata verschieden.

### Referenzen
`backlog/sprint-3.md` HUM-038 (Spezifikation und Stand), HUM-104; `backlog/CONVENTIONS.md` 4.5, 4.12, 4.13, 4.16; `proto/humanitl/v1/rules.proto` Feld 13, `humanitl.proto` `RulesRequest.SetDisabled`; ADR-018 (dünne Clients).

---

## HUM-106 · TLS-Karte im Intercept-Bildschirm
Sprint: 2 · Größe: M · Abhängigkeiten: HUM-020, HUM-045 (Daemon-Hälfte gemerged, `3e5c49b`), HUM-063 · Blockiert: keine; Überschneidung mit der Oberflächen-Hälfte von HUM-039 (siehe Fallstricke)

### Kontext
Der Daemon erkennt einen abgelehnten Handschlag und sagt, welches Werkzeug welche Variable braucht: `TLS_001` mit `FixAction::SetEnv` je Hinweis (`daemon/crates/proxy/src/tls_observe.rs`, HUM-045). Er hängt den Befund an den Flow des `CONNECT` (`daemon/crates/proxy/src/handler.rs:904-911`, `FlowEvent::Diagnostic { flow_id: Some(..) }`) und schickt ihn als `FlowEvent.flow_diagnostic` (`daemon/crates/ipc/src/convert.rs:1040-1046`, Proto-Feld 16); `TLS_003` (kein SNI) geht ohne Flow als `FlowEvent.diagnostic` (Feld 12).

In der Anwendung kommt beides an und wird verworfen. `app/lib/core/ipc/convert.dart:340-353` übersetzt beide Varianten nach `FlowEvent.diagnostic(at, diagnostic)` und lässt die `flow_id` fallen (der Kommentar dort verweist die Erweiterung um `flowId` auf die Oberflächen-Hälfte von HUM-039). `app/lib/features/intercept/providers/flows.dart:138-145` und `app/lib/features/history/providers/history_page.dart:558-563` behandeln `FlowEventDiagnostic` mit `break`, der Dart-Fake ebenso (`app/lib/core/ipc/fake_daemon_client.dart:1128-1133`). Einen `diagnosticsProvider`, wie ihn `backlog/CONVENTIONS.md` 3.9 als verbindlichen Namen führt und HUM-045 Schritt 5 voraussetzt, gibt es nicht (`grep -rn diagnosticsProvider app/lib` ist leer); die Datei `app/lib/features/intercept/widgets/diagnostic_card.dart`, die HUM-045 als Ort nennt, existiert nicht. Kein Bildschirm zeigt den Befund. Ein Mensch sieht den TLS-Fehler nur in `humanitl flows list --json` (`error = tls_handshake_failed`) und im Terminal des Agenten, und `docs/UX.md` 4.4 sagt: „Ein `Diagnostic` mit `FixAction` und ohne sichtbare Aktion ist ein Defekt." HUM-045 führt das in seinem Stand als offen (`backlog/sprint-3.md:1357-1370`) und in seinem ersten Akzeptanzkriterium als nicht erfüllt (`:1334`); ein eigenes Issue gab es nicht. Dieses Issue ist die Oberflächen-Hälfte von HUM-045, ohne den Schreibweg in die Konfiguration.

Was da ist: `HDiagnosticCard` (`app/lib/core/ui/h_diagnostic_card.dart:14-25`) und `FixControl` (`app/lib/core/ui/fix_control.dart`), zusammen benutzt in `setup_screen.dart:51-58`, `sandbox_screen.dart:228-239` und `attention_notice.dart:42-50`; `FixControl` zeichnet `FixActionSetEnv` heute nur als Abzeichen „Set {key}" (`fix_control.dart:68`), die Kopierzeile gibt es nur für `copyCommand` und `openUrl` (`:54-66`). `AgentAskStrip` über der Warteschlange (`app/lib/features/intercept/widgets/agent_ask_card.dart:52-71`, eingehängt in `queue_pane.dart:361`) mit `agentAsksProvider` (`providers/agent_asks.dart`) ist das Muster für Karten aus einem Ereignis, das kein Flow ist. Das Standard-Szenario des Dart-Fakes sendet nach 5 s genau ein `TLS_001` mit `setEnv(CURL_CA_BUNDLE, /etc/humanitl/ca.crt)` (`fake_daemon_client.dart:1236`, `:1366-1376`), und `fake_daemon_client_test.dart:44` prüft, dass es ankommt.

### Ziel
Ein `Diagnostic` aus dem Ereignisstrom erscheint im Intercept-Bildschirm als Karte über der Warteschlange: Code, Titel, der `why`-Satz des Daemons, für `SetEnv` das Abzeichen plus eine Kopierzeile `export KEY=VALUE`, für `AddRule` und `CopyCommand` das, was `FixControl` heute schon zeichnet; jede Karte ist einzeln wegklickbar. `diagnosticsProvider` hält die Befunde der Sitzung, `FlowEvent.diagnostic` trägt die `flowId` aus `flow_diagnostic`. Mit `--dart-define=HUMANITL_FAKE=demo` (Korrektur 2026-09-11: jeder Name außer `1`, `default` und `true` wählt das Standard-Szenario des Dart-Fakes; `default` startet den gRPC-Client gegen `humanitld --fake`, `app/lib/core/ipc/launch_options.dart:50`) steht die Karte nach 5 s, gegen den echten Daemon nach `curl --cacert /dev/null https://example.com` in der Sandbox.

### Nicht-Ziel
Kein Knopf „Für nächste Session setzen": er bräuchte `SetConfig`, und der RPC antwortet `unimplemented` (HUM-045 Stand, HUM-069). Keine amberfarbene Zeile für `LLM_005` an einer durchgereichten Anfrage und kein Öffnen der Flow-Details aus der Karte: das ist die Oberflächen-Hälfte von HUM-039, die denselben Provider und dieselbe `flowId` nutzt. Keine Karte in der Historie: der Flow steht dort mit `error` (HUM-045). Keine Entstörung in der Anwendung: der Daemon entstört `TLS_001` je Host und Hinweis für 60 s (`tls_observe.rs:85`). Keine Benachrichtigung (HUM-034). Keine Änderung an Daemon oder Proto.

### Betroffene Pfade
- `app/lib/core/domain/flow_event.dart` (ändern: `FlowEvent.diagnostic` bekommt `FlowId? flowId`; der Getter in `:130-141`, der für die Variante heute `null` liefert, liefert ihn), `flow_event.freezed.dart` (Regenerat)
- `app/lib/core/ipc/convert.dart` (ändern, geteilte Datei: `:350-353` setzt `flowId: FlowId(flowDiagnostic.flowId)`; der Kommentar `:344-349` entfällt)
- `app/lib/features/intercept/providers/diagnostics.dart`, `diagnostics.g.dart` (neu): `diagnosticsProvider`
- `app/lib/features/intercept/widgets/diagnostic_card.dart` (neu): `DiagnosticStrip`, `DiagnosticCard`
- `app/lib/features/intercept/widgets/queue_pane.dart` (ändern: `DiagnosticStrip` neben `AgentAskStrip`, `:361`)
- `app/lib/core/ui/fix_control.dart` (ändern: `FixActionSetEnv` zeichnet Abzeichen und Kopierzeile `export KEY=VALUE`)
- `app/lib/core/ui/` (ändern: die privaten Helfer `_severityLabel` und `_severityColor` aus `attention_notice.dart:88` einmal hierher heben statt kopieren)
- `app/l10n/app_en.arb`, `app/l10n/app_de.arb` (ändern: neue Schlüssel ans Ende der Datei)
- `app/test/core/ipc/convert_test.dart`, `app/test/features/intercept/diagnostics_test.dart` (neu), `diagnostic_card_test.dart` (neu), `app/test/core/ui/fix_control_test.dart` (neu), `app/test/goldens/intercept_golden_test.dart` (ändern: ein Golden mit Karte)
- `backlog/CONVENTIONS.md` 4.15 (ändern: ein Absatz zur Karte)

Nicht berührt: `daemon/`, `proto/`, `app/lib/features/history/` (`history_page.dart:558-563` behält sein `break`), `backlog/sprint-3.md` (HUM-045; sein Stand verweist nachträglich hierher, sobald dort niemand mehr arbeitet).

### Spezifikation

```dart
// app/lib/core/domain/flow_event.dart
/// A diagnostic of the session, or of one flow when [flowId] is set
/// (`flow_diagnostic`, field 16): a TLS refusal, a finding on a passthrough.
...
```dart
// app/lib/features/intercept/providers/diagnostics.dart
/// One diagnostic as the strip holds it.
@immutable
class SessionDiagnostic {
  final int id;            // laufende Nummer, für Schlüssel und dismiss
  final DateTime at;
  final FlowId? flowId;
...
class Diagnostics extends _$Diagnostics {
  @override
  List<SessionDiagnostic> build();   // hört auf flowEventsProvider wie agent_asks.dart
  void dismiss(int id);
}
```

Der Provider hängt jede `FlowEventDiagnostic` an, ohne Obergrenze (Begründung wie bei `agentAsksProvider`: der Daemon begrenzt, die Anwendung verschluckt nichts). `DiagnosticStrip` zeichnet nichts, solange die Liste leer ist, sonst je Eintrag eine `DiagnosticCard`: `HDiagnosticCard(code, severityLabel, color, title, why, fix: FixControl(fix: diagnostic.fix), docsUrl)` nach dem Aufruf in `attention_notice.dart:42-50`, daneben ein `HIconButton` mit `key: ValueKey<String>('intercept-diagnostic-dismiss-$id')`. Der Titel ist der Text der Anwendung (`interceptDiagnosticTitle`), der `why`-Slot der Satz des Daemons (`docs/UX.md` 4.4). Ein Eintrag ohne `fix` (`TLS_003`) hat keinen leeren Slot.

`FixControl` für `FixActionSetEnv(key, value)`: das Abzeichen `setupFixSetEnv(key)` bleibt, darunter `_copyRow` mit `label: setupFixCopyExport` und `text: 'export $key=$value'`, so wie `copyCommand` es heute tut (`fix_control.dart:54-59`).

ARB-Schlüssel, ans Ende beider Dateien:

- `interceptDiagnosticTitle`: „The daemon reports" / „Der Daemon meldet"
- `interceptDiagnosticDismiss`: „Dismiss" / „Ausblenden"
- `setupFixCopyExport`: „Copy export command" / „Export-Befehl kopieren"

### Schritte
1. `flowId` an `FlowEvent.diagnostic`, `build_runner`, `convert.dart`. Prüfbar: `flutter test test/core/ipc/convert_test.dart` grün mit `flow_diagnostic_carries_the_flow_id`; `flows.dart`, `history_page.dart`, `fake_daemon_client.dart` übersetzen unverändert.
2. `FixControl` mit Kopierzeile für `SetEnv`. Prüfbar: `fix_control_test.dart` grün.
3. `diagnosticsProvider` und Tests. Prüfbar: `diagnostics_test.dart` grün.
4. `DiagnosticStrip`, `DiagnosticCard`, Einhängen in `queue_pane.dart`, ARB, Golden. Prüfbar: `diagnostic_card_test.dart` grün, Golden abgenommen, `flutter run --dart-define=HUMANITL_FAKE=demo` zeigt die Karte nach 5 s.
5. CONVENTIONS 4.15, `make check`.

### Tests
- `convert_test.dart`: `flow_diagnostic_carries_the_flow_id`: `pb.FlowEvent` mit `flowDiagnostic` ⇒ `FlowEventDiagnostic.flowId` gleich der Id; `diagnostic` (Feld 12) ⇒ `flowId == null`.
- `diagnostics_test.dart`: `collects_every_diagnostic_of_the_stream` (zwei Ereignisse, zwei Einträge, auch bei gleichem Code); `dismiss_removes_only_that_one`; `a_flow_event_that_is_no_diagnostic_changes_nothing`.
- `diagnostic_card_test.dart`: `tls_card_shows_code_why_and_export_command`: `FakeDaemonClient` Standard-Szenario, 5 s vorspulen ⇒ genau eine Karte, Text `TLS_001`, der `why`-Satz des Szenarios, Kopierzeile `export CURL_CA_BUNDLE=/etc/humanitl/ca.crt`; `dismiss_hides_the_card`; `a_diagnostic_without_fix_has_no_fix_row` (`TLS_003`-Beispiel ohne `fix`).
- `fix_control_test.dart`: `set_env_offers_the_export_command`: Tippen auf die Kopierzeile legt `export KEY=VALUE` in die Zwischenablage (`SystemChannels.platform`-Mock), das Label wechselt auf `setupFixCopied`.
- `intercept_golden_test.dart`: `intercept_diagnostic_tls` (Karte über der Warteschlange, hell und dunkel wie die übrigen).

### Akzeptanzkriterien
- [x] `grep -n "FlowId? flowId" app/lib/core/domain/flow_event.dart` trifft in der Fabrik `diagnostic` (Zeile 101); `grep -n "flowDiagnostic.flowId.isEmpty" app/lib/core/ipc/convert.dart` trifft einmal (Zeile 362), und der Zweig dort baut `FlowId(...)` nur für ein nicht leeres Feld 16, ein leeres bleibt `null`; `flow_diagnostic_carries_the_flow_id` grün. **Am 2026-09-12 nachgemessen:** Der früher hier genannte Grep `"flowId: FlowId(flowDiagnostic.flowId)"` trifft nicht und hat nie getroffen; der Ausdruck steht über die Zeilen 359 bis 364 verteilt. Die einzeilige Schreibweise stammt aus „Betroffene Pfade" (Zeile 2894), nicht aus dem Code.
...
- [x] **Gemessen am 2026-09-11** auf Xvfb (`:99`, `GDK_BACKEND=x11`) mit dem Debug-Bundle und `HUMANITL_FAKE=demo`: Das Bildschirmfoto nach 20 s zeigt über der Warteschlange die Karte `TLS_001` mit „Warnung", „Der Daemon meldet", dem Satz des Szenarios, dem Abzeichen „CURL_CA_BUNDLE setzen" und der Zeile `export CURL_CA_BUNDLE=/etc/humanitl/ca.crt`; nach 8 s stand sie noch nicht, weil der Start des Debug-Builds die fünf Sekunden des Szenarios verschiebt. Die Zwischenablage misst `set_env_offers_the_export_command`, grün; auf dem Messrechner fehlen `xdotool` und `xclip` für einen Klick von außen. `cd app && flutter run -d linux --dart-define=HUMANITL_FAKE=demo` (oder die Umgebungsvariable `HUMANITL_FAKE=demo` vor dem gebauten Bundle): nach etwa 5 s steht über der Warteschlange eine Karte `TLS_001` mit dem Satz „curl in the sandbox does not trust the Humanitl CA yet", dem Abzeichen `Set CURL_CA_BUNDLE` und der Kopierzeile; nach dem Kopieren enthält die Zwischenablage `export CURL_CA_BUNDLE=/etc/humanitl/ca.crt` (Blick, dazu `tls_card_shows_code_why_and_export_command` und `set_env_offers_the_export_command`).
- [x] **Gemessen am 2026-09-11, nach HUM-149**, auf Xvfb gegen einen echten Daemon: `curl --cacert /usr/share/ca-certificates/mozilla/ISRG_Root_X1.crt https://example.com` in der Sandbox endet mit Exit 60, über der Warteschlange steht die Karte `TLS_001` mit dem Abzeichen „CURL_CA_BUNDLE setzen" und dem Export-Befehl, und `humanitl --json flows list` zeigt `CONNECT example.com block tls_handshake_failed`. Gemessen mit einer fremden Wurzel statt `/dev/null`, aus dem Grund, der bei HUM-045 steht. **Die Vorgeschichte, wie sie vor HUM-149 stand: Offen, gemessen am 2026-09-11, und die Ursache liegt im Proxy, nicht in der Karte.** Mit dem echten curl 8.22 in der Sandbox entsteht kein `TLS_001`: `--cacert /dev/null` scheitert vor jedem Zertifikat (Exit 77, Flow ja, Befund nein), und mit einer fremden Wurzel schließt curl den Handschlag ab und lehnt erst danach ab (Exit 60, weder Flow noch Befund). `openssl s_client -verify_return_error` erzeugt Flow und Befund. Die Erkennung dafür baut HUM-149; danach wird dieses Kriterium mit einer fremden Wurzel gemessen. Gegen einen laufenden Daemon: `curl --cacert /dev/null https://example.com` in der Sandbox (HUM-045, erstes Akzeptanzkriterium) erzeugt dieselbe Karte mit `CURL_CA_BUNDLE`; `humanitl --json flows list` zeigt den Flow mit `"error": "tls_handshake_failed"` (Blick).
...
- Die Karte darf nichts versprechen, was sie nicht tut: kein Knopf, der in die Konfiguration schreibt (4.13; `SetConfig` ist unimplementiert). Die Kopierzeile ist der eine Fix, den die Anwendung heute ausführen kann; HUM-045 nennt ihn ausdrücklich („Befehl kopieren").
- Der `why`-Satz kommt vom Daemon und wird nicht übersetzt oder umformuliert; der Titel der Anwendung ist nur der Rahmen (`docs/UX.md` 4.4, erster Punkt).
- Keine Obergrenze und kein Zusammenfassen gleicher Codes in der Anwendung: der Daemon entstört bereits und führt den Zähler unterdrückter Versuche in der nächsten Karte mit (`tls_observe.rs`, HUM-045 Stand). Zwei Karten sind zwei Befunde.
- Die Historie bleibt unberührt: der Flow steht dort mit `error`, und eine zweite Karte dort wäre dieselbe Aussage zweimal.
...
Abbildung: `RuleTest.action` → `verdict` (`allow`, `block`, `ask`, `redact`); `matched = false` → `rule: none (default ask)`. Exit: `allow` 0, `block` 10, `ask` 11, `redact` 11 (ein Redact wird heute gehalten, `pipeline.rs:371-379`). `--json`: `{"verdict","matched","rule_id","origin","position","passthrough"}`; `origin` aus der Regel in `RulesResponse.rules` derselben Antwort (`session`, `user`, `bundled`), `position` aus `RuleTest.position`. Das sechste Feld kam mit der Umsetzung dazu und steht hier, damit die Spezifikation nicht hinter dem Kommando zurückbleibt: Die Durchreiche zum Sprachmodell ist eine mitgelieferte Regel mit eigenem Rang (`passthrough_llm` **und** `bundled`, CONVENTIONS 4.5), und HUM-039 verlangt genau diese Unterscheidung von diesem Kommando. `origin` und `passthrough` sind `null`, wenn die Antwort die Regel nicht führt. Eine URL ohne Schema oder mit Fragment ist `CLI_004` vor dem Aufruf. Der Verdikt-Exit läuft nicht über `exit_code(&Diagnostic)`; `run` gibt `Ok(EXIT_BLOCK)`/`Ok(EXIT_ASK)` zurück.
...
### Akzeptanzkriterien
- [x] `humanitl rules test https://evil.example` gegen einen laufenden Daemon mit den mitgelieferten Regeln druckt `verdict: ask` und endet 11; `humanitl rules test https://models.dev/api.json` druckt `verdict: block` mit `rule: <id> (bundled, position n)` und endet 10. **Gemessen am 2026-09-07** gegen `humanitld --fake fixtures/sessions/mixed.jsonl`: die erste Zeile lautet `verdict: ask` mit `rule: none (default ask)` und Exit 11, die zweite `verdict: block` mit `rule: 018f0000-0000-7000-8000-0000000000a1 (bundled, position 1)` und Exit 10. Dieselben zwei Fälle stehen als `rules_test_ask_exit_11_and_block_exit_10` in `daemon/bin/humanitl/tests/cli.rs`, dazu `rules_test_allow_exit_0_and_redact_exit_11` für die beiden anderen Ausgänge (`redact` endet 11, weil der Proxy eine solche Anfrage hält).
- [x] `humanitl --json rules test https://evil.example | jq -r .verdict` druckt `ask`; `grep -n 'do not occur yet' daemon/bin/humanitl/src/cli.rs` ist leer; `grep -n 'test_not_yet' daemon/bin/humanitl/src/cmd/rules.rs` ist leer. **Alle drei am 2026-09-07 ausgeführt**, das erste gegen denselben laufenden Daemon (`ask`), die beiden `grep` mit Exit 1 und ohne Zeile. `rules_test_json_shape` misst dazu die sechs Felder `verdict`, `matched`, `rule_id`, `origin`, `position` und `passthrough` und dass der Exit-Code des JSON-Laufs derselbe ist wie der des lesbaren.
- [x] `grep -c 'rules_cli_' tests/escape/esc-4-rules.sh` ist mindestens 15, und `./tests/escape/run.sh` zählt sie grün; `tests/escape/README.md` nennt die neue Bilanz. **Gemessen am 2026-09-07:** `grep -c` sagt 24 (fünfzehn Fälle, jeder mit Namen in Aufruf und Muster), und der Lauf zählt `rules_cli_01` bis `rules_cli_15` grün — 121 Fälle, 117 grün, 0 rot, 4 übersprungen, gegen 105/101/0/4 unmittelbar davor (die Zahl 97/90/0/7 stand in der README und war der Stand nach HUM-022, also vor HUM-042 und HUM-043; sie ist mitkorrigiert). Der Regelsatz des Laufs wird dafür erst **nach** der Body-Cap-Probe eingespielt (`esc4.yaml` als `rules.yaml` plus `humanitl rules reload`): Bis dahin braucht ESC-3 den Stand, in dem `blocked.example` gehalten und nicht erlaubt wird, und ein früherer Wechsel hätte drei fremde Proben umgefärbt.
- [x] `humanitl llm test http://127.0.0.1:1/` endet 1 mit einem `LLM_`-Befund; gegen einen antwortenden Endpunkt druckt es `flavor` und Modelle (`llm_test_prints_models`). **Gemessen, aber nicht gegen den Fake, und der Unterschied gehört hierher:** Der Fake fasst kein Netz an, also kann er „nicht erreichbar" gar nicht beobachten — er antwortete auf denselben Aufruf mit seiner Form und Exit 0, und ein Test dagegen hätte eine Messung behauptet, die nie stattgefunden hat. Der Fall steht deshalb dort, wo ein echter Daemon läuft: `llm_cli_unreachable` in `tests/escape/esc-4-rules.sh` misst `exit=1 code=LLM_001` mit `127.0.0.1:1/api/tags did not answer: upstream_connect`. Gegen den Fake gemessen ist, was er wirklich weiß: `llm_test_prints_models` (`flavor: ollama`, Latenz, zwei Modelle, jede Zeile höchstens 40 Zeichen wie in der Oberfläche, die Ankündigung auf `stderr` vor dem ersten Paket) und `llm_test_an_endpoint_the_daemon_refuses_is_exit_1` (`LLM_007`, und ohne Daemon Exit 2).
- [x] `cargo test -p humanitl --test cli` grün; `backlog/CONVENTIONS.md` 4.21 führt `humanitl llm test` nicht mehr als offen. **75 Tests grün am 2026-09-07**, darunter die sieben neuen. CONVENTIONS 4.21 und `docs/PROTOCOL.md` 4.9 nennen beide Kommandos jetzt als gebaut statt als offen, und `docs/cli.md` hat für jedes einen Abschnitt mit Ausgabe, Exit-Codes und den Grenzen der Modellliste.
- [x] `make check` grün. **`STRICT=1 make check` rc=0 am 2026-09-07**, mit `cargo fmt`, `clippy`, `check-deps.sh`, der Rust- und der Flutter-Sammlung.
...
### Akzeptanzkriterien
- [x] `grep -n 'skip dns_not_before_decision\|skip meta_no_dns_lookup' tests/escape/esc-3-egress.sh` ist leer; `./tests/escape/run.sh` meldet `dns_not_before_decision`, `dns_after_allow_once` und `meta_no_dns_lookup` als `pass` und 6 übersprungene Fälle. (Gemessen 2026-09-11: 124 Fälle, 122 grün, 0 rot, 2 übersprungen. Die Zahl 6 der Spezifikation stammt aus einem älteren Stand; vor diesem Issue waren es 4, zwei davon die beiden ESC-3-Fälle, übrig bleiben `audit_delete_is_detected` und `audit_truncate_is_detected` aus ESC-5. Die Host-Fälle stehen in `tests/escape/dns-proof.sh`, das `run.sh` einliest.)
- [x] `target/escape/dns.log` existiert nach dem Lauf und enthält genau eine Zeile mit `allowed.esc3.test`, keine mit `held.esc3.test`, keine mit `humanitl.internal`. (Nach dem ganzen Lauf, ESC-4 und ESC-5 eingeschlossen, genau eine Zeile: `<epoch-ms> allowed.esc3.test A`, 22 bis 49 ms nach dem `decide`.)
- [x] `grep -n 'post-MVP' daemon/crates/proxy/src/resolver.rs` ist leer; `docs/CONFIG.md` beschreibt `resolver.nameserver` als wirksam und zeigt in der Spalte „Wirkung" `ja`; das Leser-Register führt ihn als `effective`; `cargo test -p humanitl-proxy -p humanitl-config` grün. (Im Workspace-Lauf von `make check` grün; `dns_after_allow.rs` hat 21 Tests, sechs davon für den Adapter, jeder unter seiner Mutation rot gesehen.)
- [x] `docs/THREAT-MODEL.md` K-10 und `docs/SECURITY.md` Tabelle 9.3 nennen ESC-3 als host-seitigen Beweis, und `tests/escape/README.md` trägt die neue Bilanz. (Beide nennen dazu die Grenze: Beobachtet wird der Adapter hinter `resolver.nameserver`; der Namensdienst des Systems hängt am selben Aufrufpunkt und ist im Daemon belegt.)
- [x] `daemon/Cargo.toml` trägt `hickory-resolver` als Workspace-Abhängigkeit (vom Eigentümer eingetragen), `cargo deny check` in der CI grün. (Eingetragen, `hickory-resolver` 0.26 ohne Standard-Features, nur `tokio`; `cargo deny check` lokal grün: advisories, bans, licenses, sources ok. In der CI gemessen am 2026-09-11: Lauf 34610457206 über `ea28059`, das den Merge `e9b4b31` enthält, Job `rust-check`, Schritt „Licenses, advisories, duplicate crates" (`make rust-deny`) grün.)
- [x] `make check` grün. (Im Arbeitsbaum des Issues am 2026-09-11 grün bis einschließlich `typed-errors-lint`: rustfmt, clippy, Bau, alle Rust-Tests, rustdoc, `deps-lint`, `docs-lint`. Rot wurde erst `flutter-analyze`, mit 255 Befunden, die alle auf den nicht erzeugten Dart-Code unter `app/lib/core/ipc/generated/` zeigen; dem Rechner fehlen `protoc` und `protoc-gen-dart`, `make proto` übersprang die Dart-Hälfte. Das Issue ändert keine Dart-Datei; `flutter-test` und `flutter-build` liefen deshalb nicht. Nachgemessen am 2026-09-11 über den Merge-Stand: `tools/verify-commit.sh ea28059`, das `e9b4b31` enthält, grün in allen Schritten von `make check` außer `flutter-build` (Format und Lints, Bau und Tests, rustdoc, `deps-lint`, `docs-lint`, `typed-errors-lint`, Lizenzen, `flutter-analyze` samt erzeugtem Dart-Code, `flutter-test`), dazu die Escape-Suite mit 124/122/0/2; `flutter build linux --debug` lief im CI-Lauf 34610457206, Job `flutter-analyze-test`, Schritt „Build the Linux bundle", grün.)
...
- [x] `make check` grün. **Gemessen am 2026-09-11** über den fertigen Commit statt über den Arbeitsbaum: `tools/verify-commit.sh f8433ca` grün (dieselben Schritte wie `make check`, ohne `flutter-build`, dazu die Escape-Suite), und der CI-Lauf über denselben Commit ist mit allen elf Jobs grün, darunter `flutter-analyze-test` samt `flutter build linux --debug`.
...
### Ziel
Die Notiz einer Block-Entscheidung liegt in der Aufzeichnung, erscheint in `humanitl flows show`, im History-Detail unter der Entscheidung und im HAR-Export unter `_humanitl.note`, und `/why/<id>` liefert sie innerhalb der laufenden Sitzung auch dann, wenn die Registry den Flow nicht mehr hält. HUM-072s Nebenzusage steht: enthält die Notiz selbst ein Secret, warnt ein Befund, die Entscheidung fällt trotzdem.
...
Der Writer schreibt `decision_note` beim `Decided`-Ereignis mit `Decision::Block { note: Some(_) }`, gesäubert über `sanitize_note` (dieselbe Funktion wie für die 403-Antwort, damit die Aufzeichnung genau das trägt, was der Agent gesehen hat). Proto: `string decision_note = 27` in `FlowSummary` (25 trägt `meta`, 26 nimmt `apex` aus HUM-091) und `string decision_note = 11` in `FlowDetail`, leer wenn keine. `PROTO_MINOR` geht dabei von 9 auf 10 — HUM-119 hat die 9 vorher genommen (korrigiert am 2026-09-12; die Kette in HUM-094 nennt noch die alte Reihenfolge) —, und der Dart-Spiegel `ProtoVersion.minor` zieht mit. `humanitl flows show` druckt `note: …` nach der Entscheidung, `--json` das Feld. History-Detail: eine Zeile unter der Entscheidung, ARB `historyDetailNote` („Note to the agent: {note}"). HAR: `_humanitl.note`. `/why`: `note=` aus dem Registry, sonst aus dem Recorder. **Entschieden am 2026-09-12 vom Projekteigentümer:** Die Notiz überdauert den Neustart, sie liegt dafür in der Aufzeichnung; `/why` liest sie über `Recorder::get_flow`, sobald die Registry den Flow nicht mehr hält, und nur für die eigene Sitzung (CONVENTIONS 4.24). Ein Neustart des Daemons beendet die Sitzung, also antwortet `/why` danach `404`; über `ListFlows` und `GetFlow` steht die Notiz trotzdem. Ins Audit-Log kommt sie nicht — `docs/SECURITY.md` 8 und HUM-050 schließen das aus, und HUM-072 hält diese Entscheidung fest.
...
### Akzeptanzkriterien
- [x] Nach `humanitl flows decide <id> block --note 'use PyPI'` druckt `humanitl --json flows show <id> | jq -r .decision_note` `use PyPI`, auch nach einem Neustart des Daemons. **Ganz gemessen am 2026-09-12, beide Hälften.** Erste Hälfte, die Leitung und die Ausgabe, gegen `humanitld --fake fixtures/sessions/mixed.jsonl`: `humanitl flows decide 018f0001-0000-7000-8000-000000040000 block --note 'use PyPI'` gefolgt von `humanitl --json flows show <id>` liefert `decision_note = 'use PyPI'`, und `humanitl flows show <id>` druckt `note      use PyPI` unmittelbar unter `decision  block`. Zweite Hälfte, die Spalte und der Neustart, gegen einen echten `humanitld` mit Recorder: `the_note_of_a_human_block_outlives_the_daemon` in `daemon/bin/humanitld/tests/daemon_end_to_end.rs` schickt eine Anfrage in den Proxy-Socket, sieht sie als `Held` im gRPC-Strom, blockt sie über `Decide` mit `note: "use PyPI"`, liest beim wartenden Agenten `HTTP/1.1 403` mit `X-Humanitl-Note: use PyPI`, startet denselben Prozess mit `Daemon::restart` im selben XDG-Baum neu und findet `use PyPI` danach in `FlowSummary.decision_note` aus `ListFlows` und in `FlowDetail.decision_note` aus `GetFlow`. Der zweite Prozess hat eine leere Registry und eine neue Sitzung, also kann der Wert nur aus der Spalte `decision_note` kommen; die CLI ist an dieser Stelle ein dünner Client derselben beiden RPCs (ADR-018). Mutationsprobe, zwei Läufe, jedes Mal am Produkt und nicht am Test: (1) `decision_note` in `daemon/crates/recorder/src/writer.rs` gibt immer `None` zurück, die Spalte wird also nie geschrieben — rot in `assert_eq!(before.decision_note, HUMAN_NOTE, "ListFlows: {before:?}")` mit `left: "" right: "use PyPI"`; (2) `list_flows` in `daemon/crates/ipc/src/server.rs` beantwortet die Seite aus der Registry statt aus der Aufzeichnung — vor dem Neustart grün, danach rot mit `the blocked flow reaches ListFlows within ten seconds, after the restart`. Beide Dateien danach byte-genau zurückgestellt und mit `cmp` geprüft. `cargo test -p humanitld --test daemon_end_to_end`: 18 von 18 grün, `rustfmt --check` und `cargo clippy ... -- -D warnings` auf dem Testziel ohne Befund. Nicht in diesem Kasten und Sache des nächsten: `/why/<id>` nach dem Neustart. Derselbe Test misst mit, warum das eine eigene Umgebung braucht — der Rückfall auf die Aufzeichnung beantwortet nur Flows der eigenen Sitzung (CONVENTIONS 4.24), und ein neu gestarteter `humanitld` legt mit `SessionId::new` eine neue an; über den Proxy-Socket antwortet `/why/<flow>` deshalb vor dem Neustart `decision=block reason=user note=use PyPI` und danach `404`. Eine Sandbox ändert daran nichts, und das war bis zu diesem Lauf falsch angenommen: die Sitzung gehört dem Prozess und nicht dem Agenten, `humanitld` legt sie bei jedem Start neu an, und der Shim reicht keine mit. Das `404` nach einem Neustart des Daemons ist damit die Zusage aus CONVENTIONS 4.24 und kein Mangel; siehe das nächste Kästchen.
- [ ] Aus der Sandbox liefert `curl http://humanitl.internal/why/<id>` nach dem Neustart `note=use PyPI` als letztes Feld. **So nicht erreichbar, am 2026-09-12 gegen den Code nachgemessen.** Das Kriterium widerspricht CONVENTIONS 4.24. `/why/<id>` beantwortet nur Flows der eigenen Sitzung, und eine Sitzung gehört dem Prozess: `daemon/bin/humanitld/src/main.rs` legt sie bei jedem Start mit `SessionId::new` neu an, es gibt keinen Weg, eine alte fortzusetzen, und eine Verbindung auf `proxy.sock` erbt die des laufenden Daemons, auch aus einer Sandbox heraus. Nach einem Neustart des Daemons gilt deshalb `archived.session != request.session`, und `why_body` (`daemon/crates/proxy/src/meta.rs`) antwortet `404 no such flow`. Gemessen im selben Lauf wie das Kästchen darüber: über den Proxy-Socket vorher `HTTP/1.1 200` mit `decision=block reason=user note=use PyPI`, nachher `HTTP/1.1 404`. Was der Rückfall auf die Aufzeichnung wirklich trägt, ist ein Flow, den die Registry innerhalb einer laufenden Sitzung nicht mehr hält; genau das misst `why_survives_a_restart` (`daemon/crates/proxy/tests/meta.rs`): ein zweiter Proxy über derselben Datenbank und derselben Sitzung, mit leerer Registry, antwortet auf `GET http://humanitl.internal/why/<flow>` mit genau `decision=block reason=user note=use PyPI\n`. Das Kästchen bleibt leer und wird nicht umformuliert: ob eine Sitzung einen Neustart überdauern soll, ist eine Entscheidung über die Sitzungsgrenze selbst und gehört in ein eigenes Issue mit ADR, nicht in HUM-117.
...
HAR-Export als `_humanitl.note`. `/why/<flow-id>` liest sie aus der
Aufzeichnung, sobald die Registry den Flow nicht mehr hält, und nur für die
eigene Sitzung. Ins Audit-Log kommt sie nicht.
...
### Akzeptanzkriterien
- [x] `grep -n 'decoded\|encoding_left\|content_encoding' proto/humanitl/v1/humanitl.proto` trifft in `BodyRef` und `BodyChunk`; `PROTO_MINOR` um eins erhöht; `proto/descriptor.binpb` im selben Commit. **Gemessen am 2026-09-12:** Der Grep trifft in `BodyRef` (Zeilen 148 und 152: `string content_encoding = 5`, `bool decoded = 6`) und in `BodyChunk` (Zeile 592: `string encoding_left = 4`). `PROTO_MINOR` steht auf `9` (`daemon/crates/ipc/src/lib.rs`), `ProtoVersion.minor` ebenso (`app/lib/core/ipc/proto_version.dart`); `docs/PROTOCOL.md` 5 nennt `1.9`. `make proto` hat `proto/descriptor.binpb` und `proto/generated.sha256` neu geschrieben, beide liegen im selben Commit.
- [x] `cargo test -p humanitl-ipc` grün, darunter die fünf `get_body_*`-Tests; ein brotli-Rumpf kommt mit `decoded = true` als Klartext und `encoding_left == ""`, ein zstd-Rumpf roh mit `encoding_left == "zstd"`. **Gemessen am 2026-09-12:** 279 Tests grün, darunter die zehn in `daemon/crates/ipc/tests/get_body.rs` (`get_body_decoded_inflates_gzip`, `_inflates_brotli`, `_leaves_zstd_alone`, `_leaves_a_chain_alone`, `_stops_at_the_budget`, `_keeps_the_prefix_of_a_truncated_body`, `_on_a_broken_stream_hands_out_the_raw_bytes`, `get_body_raw_is_unchanged`, `detail_carries_the_content_encoding`, `detail_leaves_the_content_encoding_empty_for_identity`) und `the_fake_detail_carries_the_content_encoding_of_each_side` im Rust-Fake. Der brotli-Rumpf kommt als Klartext mit `encoding_left == ""`, der zstd-Rumpf roh mit `encoding_left == "zstd"`, die Kette `gzip, br` roh mit `encoding_left == "gzip, br"`, und die Bombe endet bei `gepackte Länge × 4` mit `limits.max_decompress_ratio = 4`.
- [x] `grep -rn 'GZipCodec\|ZLibCodec\|gzip' app/lib/core/body/body_decode.dart` ist leer; `flutter test test/features/intercept/body` grün, darunter der brotli-Fall mit korrekter Fundstelle. **Gemessen am 2026-09-12:** Der Grep ist leer, ebenso `grep -rn 'GZipCodec\|ZLibCodec' app/lib`; `body_decode.dart` importiert kein `dart:io` mehr. `flutter test test/features/intercept/body`: 115 Tests grün, darunter `brotli_body_shows_its_finding_at_the_right_place` (die Marke steht auf `eyJhbGciOiJIUzI1NiJ9.e30.x`), `the default scenario really carries a brotli body` (der Fake liefert 85 gepackte Bytes und mit `decoded` 127 Klartext-Bytes, der Fund sitzt bei 11..37) und `unsupported_encoding_names_itself`.
- [ ] Gegen einen laufenden Daemon: ein `curl --compressed` aus der Sandbox an ein Ziel, das `br` liefert, zeigt in der Karte den lesbaren Rumpf (Blick). **Offen am 2026-09-12**, weil dafür eine Sandbox-Sitzung mit Netz und ein Mensch vor dem Bildschirm nötig sind. Was zu tun ist: `humanitld` mit Aufzeichnung starten, die Oberfläche verbinden, in der Sandbox `curl --compressed https://<ziel>` gegen ein Ziel laufen lassen, das `content-encoding: br` antwortet, die Anfrage erlauben und im History-Detail den Antwort-Tab öffnen. Zu sehen sein muss: lesbarer Text statt Hex, kein Hinweis „packed with br", und in `FlowDetail.response_body.content_encoding` steht `br` (über `humanitl --json flows show <id>` nachprüfbar). Ohne Menschen gemessen ist derselbe Weg bis zur Karte: `daemon/crates/ipc/tests/get_body.rs::get_body_decoded_inflates_brotli` fährt Aufzeichnung, `GetFlow` und `GetBody` des echten Dienstes, und `app/test/features/intercept/body/parser_test.dart::the default scenario really carries a brotli body` fährt die Client-Hälfte; was allein der Blick belegt, ist das Bild.
- [x] `make check` grün. **Gemessen am 2026-09-12:** Vollständiger Lauf grün bis `flutter build linux --debug` (`✓ Built build/linux/x64/debug/bundle/humanitl`), inklusive `cargo fmt --check`, `cargo clippy -D warnings`, `cargo test --workspace`, `cargo doc -D warnings`, `tools/check-deps.sh`, `tools/check_coupling.py`, `scripts/ci/lint-docs.sh`, `scripts/ci/lint-no-string-errors.sh`, `flutter analyze` und `flutter test` (1043 Tests, 4 übersprungen).
...
### Stand (2026-09-12): gebaut, mit sechs Abweichungen
- **`PROTO_MINOR` geht von 8 auf 9, nicht auf 10.** Die Notiz unter HUM-094 (`:2138`) schreibt die Kette „8 für HUM-091, 9 für HUM-117, 10 für HUM-119, 11 für HUM-094"; HUM-117 ist noch nicht gebaut, dieses Issue war zuerst dran, und die Nebenversion zählt Änderungen, nicht Issue-Nummern. HUM-117 nimmt damit 10, HUM-094 die 11.
- **Der Port bekommt eine zweite Methode statt einer geänderten.** `DaemonClient.getBody` liefert weiter `Stream<Uint8List>` mit den aufgezeichneten Bytes; `getBodyChunks` liefert `Stream<BodyChunk>` samt `encodingLeft` und ist der Weg der Rumpf-Ansicht. Der HAR-Export (HUM-032, `history_export.dart`) bleibt damit unberührt und exportiert weiter, was auf der Leitung war — das Nicht-Ziel „Änderungen an der Aufzeichnung" hätte eine geänderte Signatur sonst gestreift.
- **`BodyProblem.truncatedStream` ist weg.** Den Fall entscheidet jetzt der Daemon: Er liefert entweder entpackte Bytes oder die rohen mit dem Namen der Kodierung. Die ARB-Schlüssel `interceptBodyStreamTruncated` (`en`, `de`) bleiben vorerst verwaist, wie die anderen verwaisten Schlüssel aus der Bestandsaufnahme.
- **`InflateError::BrokenStream` trägt jetzt, was bis zum Abbruch entpackt wurde.** Nur so lässt sich der Fallstrick „`truncated`-Rümpfe liefern, was entpackt wurde" einhalten, ohne einen zweiten Entpacker in den IPC-Pfad zu setzen. Der Scan wirft die Teilausgabe weiterhin weg und meldet `FINDINGS_002`; `GetBody` reicht sie nur aus, wenn `BodyRef.truncated` gesetzt ist. Ein Strom, der vollständig sein müsste und trotzdem bricht, kommt roh zurück.
- **Ein Abbruch am Budget hat kein eigenes Feld auf der Leitung.** Die Spezifikation nennt „`truncated`-Semantik im letzten Chunk", der Proto-Block darunter gibt `BodyChunk` aber nur `encoding_left`. Umgesetzt ist: Der Strom endet mit `last = true` auf dem, was entpackt wurde, und `encoding_left` bleibt leer, weil die Bytes echt entpackt sind. Wer die Grenze sichtbar machen will, braucht ein weiteres Feld; das gehört zu HUM-057 (Ratio-Grenze) und nicht hierher.
- **Die CLI fragt weiter roh.** `humanitl flows show` und `humanitl moderate` holen den Rumpf ohne `decoded` (`daemon/bin/humanitl/src/cmd/flows.rs`, `moderate.rs`); die Pfadliste dieses Issues nennt sie nicht. Für einen gepackten Rumpf zeigt die CLI damit weiter Bytes statt Text. Eigenes, kleines Folge-Issue.

Dazu zwei Dinge, die beim Bauen auffielen und hier gleich mit erledigt sind: Die Stückelung von `GetBody` stand zweimal im Code (`server.rs` und `fake/mod.rs`, samt zwei Konstanten `BODY_CHUNK_BYTES`) und steht jetzt einmal in `daemon/crates/ipc/src/body.rs`, das echter Dienst und Fake gemeinsam benutzen; und `IpcServer::over_the_recording` baut die leere Warteschlange, damit ein Test, der nur aus der Aufzeichnung liest, `humanitl-proxy` nicht kennen muss (die Kopplungs-Grundlinie aus HUM-145 wäre sonst gewachsen).

Jeder neue Test ist durch eine Mutation belegt: `deliver` ohne `decoded` (7 von 10 `get_body`-Tests rot), `recorded_encoding` immer leer (8 rot), `chunks` ohne `encoding_left` (3 rot plus die Unit-Prüfung), der Präfix-Sonderfall ohne `wire.truncated` (1 rot), der Rust-Fake ohne `encoding_of_headers` (1 rot), `aligned` immer wahr (4 Dart-Tests rot), `asking` ohne `decoded` (4 rot), der Cache-Schlüssel ohne `decoded` (1 rot), der Provider ohne `encodingLeft` (1 rot) und der Dart-Fake ohne Beachtung von `decoded` (4 rot, darunter `history_gzip_body_is_readable`). Dazu die fünf aus dem Review: `BodySource.asked` zurück auf den rohen Verweis (1 rot), die alte `short`-Bedingung (2 rot), `short` ohne das fehlende letzte Stück (1 rot), `short` ohne die Ausnahme für entpackte Bytes (1 rot) und der Dart-Fake, der `gzip` nicht mehr auspackt (3 rot, darunter wieder `history_gzip_body_is_readable`).

**Nach dem Review vom 2026-09-12** (Antigravity, fünf bestätigte Befunde, alle übernommen):

- **Eine Antwort, ein Verweis.** `BodySource.asked` ist jetzt der eine Verweis der Quelle; `parsedBodyProvider` und die Ansicht (`body_view.dart`) gehen über ihn. Vorher fragte die Karte zweimal — einmal entpackt für das Modell, einmal roh für die Hex-Ansicht —, und weil der Cache-Schlüssel `decoded` trägt, war das ein zweites `GetBody` über die Leitung je angesehenem Rumpf. `the card asks for the same body once, unpacked` (`view_test.dart`) zählt die Aufrufe.
- **`short` hatte zwei Löcher.** Ein roh abgerufener gepackter Rumpf, der auf der Leitung abbrach, galt als vollständig (der Daemon schickt ohne `decoded` immer `encoding_left = ""`), und ein entpackter Strom, der mittendrin endet, ebenso (an `size` ist ein entpackter Rumpf nicht messbar). `BodyChunk` trägt jetzt `last` bis in den Dart-Klienten, und `short` ist „kein letztes Stück, oder die Bytes sind die der Leitung und es sind weniger als angekündigt". Drei Tests: `a packed body cut short on the wire is short too`, `a stream that never ends is short, even unpacked`, `a body that unpacks smaller than it arrived is not short`.
- **`a recorded prefix is not called incomplete` prüfte nichts mehr.** Die Entscheidung war aus `buildBodyLoad` in `flowBody` gewandert; der Test baute ein `RawBody` mit `short: false` und wäre mit jedem `truncated` grün geblieben. Er heißt jetzt `a recorded prefix is not called short, a cut stream is` und fährt beide Fälle über den Provider.
- **`a body the daemon unpacked is never called short` prüft den Inhalt**, nicht nur die Länge: Rohe Bytes wären ebenfalls länger als nichts.
- **`get_body_raw_is_unchanged` erklärt, warum leer hier richtig ist**, und belegt mit dem `content_encoding` des Verweises, dass die Kodierung trotzdem auf den Bytes liegt. Dass das Feld überhaupt gefüllt wird, hält `get_body_decoded_leaves_zstd_alone` fest.

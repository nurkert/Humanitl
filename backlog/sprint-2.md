...
- [ ] `daemon/Cargo.toml` trägt `hickory-resolver` als Workspace-Abhängigkeit (vom Eigentümer eingetragen), `cargo deny check` in der CI grün. (Eingetragen, `hickory-resolver` 0.26 ohne Standard-Features, nur `tokio`; `cargo deny check` lokal grün: advisories, bans, licenses, sources ok. Die CI ist nicht gemessen.)
- [ ] `make check` grün. (Im Arbeitsbaum des Issues am 2026-09-11 grün bis einschließlich `typed-errors-lint`: rustfmt, clippy, Bau, alle Rust-Tests, rustdoc, `deps-lint`, `docs-lint`. Rot wurde erst `flutter-analyze`, mit 255 Befunden, die alle auf den nicht erzeugten Dart-Code unter `app/lib/core/ipc/generated/` zeigen; dem Rechner fehlen `protoc` und `protoc-gen-dart`, `make proto` übersprang die Dart-Hälfte. Das Issue ändert keine Dart-Datei; `flutter-test` und `flutter-build` liefen deshalb nicht.)

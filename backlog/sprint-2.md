...
- [ ] Config-Schema enthält die fünf `resolver.*`/`upstream.*`-Schlüssel mit Tier `expert` und Beschreibung. Vier `resolver.*`-Schlüssel mit Tier `expert` stehen; `upstream.connect_timeout_secs` ist nur Alias auf `limits.connect_timeout_secs` (Tier `advanced`, CONVENTIONS 4.4), und die verlangte Start-Warnung für `resolver.overrides` fehlt. `resolver.nameserver` hat seit HUM-115 einen Leser; offen bleiben der Alias und die Warnung, beide nicht Teil von HUM-115.

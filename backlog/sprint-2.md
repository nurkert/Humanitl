...
- [ ] `tests/e2e/m2_first_decision/run.sh` Exit 0, `grep -c 'rules add' tests/e2e/m2_first_decision/run.sh` ist 0, und Schritt 2 prüft, dass `created_from_flow_id` der einen Sitzungsregel die Id des zuerst freigegebenen Flows ist. Teilweise gemessen: `grep -c 'rules add'` ist 0, Schritt 2 prüft die Herkunft gegen `M2_RULE_FROM`, `M2_EXPECTED_ASSERTIONS_CLI` steht auf 71 (gezählt: 71 Behauptungen im Kommandozeilen-Zweig, eine mehr als vorher; der Bildschirm-Zweig bleibt bei 75), `sh -n` und `bash -n` grün, und die Argumentbildung von `flow_decide` ist gegen einen Stub geprüft; der Lauf selbst braucht eigenen Nutzer- und Netz-Namensraum und ist hier nicht gefahren.
...
- [ ] `make check` grün. Teilweise gemessen: `cargo fmt --all -- --check`, `cargo clippy -p humanitl --all-targets -- -D warnings`, `RUSTDOCFLAGS="-D warnings" cargo doc -p humanitl --no-deps`, `cargo test -p humanitl`, `tools/check-deps.sh`, `scripts/ci/lint-docs.sh` und `scripts/ci/lint-no-string-errors.sh` je Exit 0; die Flutter-Schritte und der übrige Workspace sind hier nicht gelaufen.

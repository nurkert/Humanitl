...
- [ ] `make check` grün. **Teilweise gemessen am 2026-09-13, und `make check` ist ohnehin nicht das Maß:** `CLAUDE.md` Schritt 5 verlangt `tools/verify-commit.sh` über den fertigen Commit, und den fährt die Hauptsitzung. Gefahren wurden hier die Teile, die dieses Issue erreichen kann: `flutter analyze --fatal-infos` ohne Befund, `dart format --set-exit-if-changed lib test` ohne Änderung, `flutter test` 1141 bestanden bei 4 übersprungenen (Stand 2026-09-18), `make runner-test` 23 von 23, `flutter build linux --debug` gebaut, `scripts/ci/lint-docs.sh` grün. Die Rust-Hälfte hat dieses Issue nicht gefahren: Es fasst keine Zeile in `daemon/` an, und mehrere Agenten bauten zugleich auf derselben Maschine.

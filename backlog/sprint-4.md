...
>
> - **Nach dem MVP verschoben** (BACKLOG.md Abschnitt 9, Punkt 17): das Formular aus dem JSON-Schema für alle Felder, Suche, Detailstufen, Herkunftsanzeige und Zurücksetzen je Feld, Live-Neuladen mit `ConfigChanged`, die Dateiüberwachung von `config.toml`, der Audit-Eintrag je Änderung und die Anzeige der Ladebefunde im Formular.
...
### Nicht-Ziel
Die Allowlist aus HUM-049 (nach dem MVP, BACKLOG.md Abschnitt 9, Punkt 16).
...
### Akzeptanzkriterien
- [x] Mit Schalter und gültiger IBAN: „Senden nicht möglich", `Decide(Allow)` über gRPC endet mit `HOLD_004`, der Flow bleibt gehalten. Gemessen: `allow_with_checksum_secret_refused_over_grpc` (`daemon/crates/ipc/tests/ipc_server.rs`: `FailedPrecondition` mit `HOLD_004`, auch mit bestätigtem Fund, in der Sammelanfrage nur dieser Flow abgelehnt, danach weiter gehalten); `a_checksum_secret_is_held_and_every_allow_is_refused` (`daemon/crates/proxy/tests/hard_block.rs`, echter Proxy: Ansage `HOLD_004` vor `Held`, `Allow` des Menschen, mit Bestätigung und im Namen einer Regel zurückgewiesen, Flow weiter gehalten, Ziel nie erreicht); die Zeile trägt die Sperre (`FlowSummary.send_refusal`): `list_flows_carries_the_hard_block_of_a_held_flow` (mit und ohne Aufzeichnung); Oberfläche `hard_block_hides_send_anyway`, `a client that starts after the announcement sees the lock`, `an allow the daemon refuses locks the valve`, `a group sends what is not locked and names the rest`, `a group of locked requests cannot be sent`, `the refusal speaks German` (`app/test/features/intercept/hard_block_test.dart`); ohne Frist `without_a_deadline_a_checksum_secret_is_blocked_as_a_secret`.
- [x] Pseudonymisiert geht dieselbe Anfrage hinaus. Gemessen: `allow_edited_without_the_secret_goes_out` (echte Detektoren, gültige IBAN: `Allow` zurückgewiesen, `AllowEdited` mit ersetztem Wert `200`, beim Ziel kommt der saubere Rumpf an); über gRPC der letzte Schritt von `allow_with_checksum_secret_refused_over_grpc`.
- [x] Eine Regel `allow` lässt eine solche Anfrage nie hinaus. Gemessen: `rule_allow_still_blocks_a_checksum_secret` (`403`, nie gehalten, nie ein `Decided(Allow)`, `Block { Secret }` durch `System`; ohne Schalter `200` mit derselben Regel), `the_llm_passthrough_never_releases_a_checksum_secret`, `the_passthrough_pipeline_never_releases_a_checksum_secret`, dazu die zweite Linie `the_handler_takes_back_an_allow_that_skipped_the_hard_block`.

...
- [ ] Terminal ist nach jedem Exit-Pfad (normal, Ctrl+], SIGTERM, Panic) wieder im Normalmodus (`stty -a` zeigt `icanon echo`). **Gebaut, nicht gemessen.** `daemon/bin/humanitl/src/tty.rs` gibt das Terminal auf vier Wegen zurück: beim Fallenlassen (also auf jedem Fehlerpfad), über einen Panik-Hook, über Signalhandler für `SIGTERM`, `SIGHUP` und `SIGINT` (Exit `128 + n`) und vor dem Start eines Editors. `humanitl sandbox attach` benutzt dieselbe Fassung, statt eine zweite zu tragen. Was fehlt, ist die Messung: Ein Rohmodus entsteht nur an einem echten Terminal, also braucht dieses Kriterium ein Pseudoterminal um die Kommandozeile herum — dieselbe Vorrichtung, die `daemon/crates/ipc/tests/terminal.rs` für den Daemon hat, die es für `daemon/bin/humanitl/tests/cli.rs` aber noch nicht gibt. Ein `Ctrl+]`-Menü gibt es nicht; `docs/cli.md` sagt das.

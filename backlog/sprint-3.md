...
- [ ] Manuell: OpenCode startet in der Sandbox, `opencode` zeigt den Provider `Humanitl local LLM` in der Modellauswahl. **Die erste Hälfte ist seit dem 2026-09-07 gemessen, die zweite nicht.** Das echte OpenCode startet in der Sandbox und benutzt den Anbieter: Im M3-Lauf steht `> build · mock` im Transkript, also das Modell dieser Sitzung aus `humanitl-local`, und der Verkehr geht über die Durchreiche ans lokale Modell (HUM-046, Schritt 13, `M3 demo: OK` mit 122 Zusicherungen). Was fehlt, ist die **Auswahl**: Ob der Anbieter unter seinem Namen `Humanitl local LLM` in der Modellliste des TUI steht, sieht nur ein Mensch vor dem Bildschirm; der Lauf fährt `opencode run` ohne TUI.

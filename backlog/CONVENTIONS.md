...
deny_syscalls = ["ptrace", "io_uring_setup", "io_uring_enter", "io_uring_register", "process_vm_readv", "process_vm_writev", "pidfd_getfd", "keyctl", "add_key", "request_key"]
[env]
...
Datei `daemon/crates/core-types/src/diagnostics/codes.rs` hält alle Codes als Konstanten mit Doc-Kommentar. Die reservierten Bereiche stehen **dort und nur dort**, in `AREAS`, und `docs/DIAGNOSTICS.md` gibt sie als Tabelle wieder (erzeugt, `UPDATE_DIAG_DOCS=1 cargo test -p humanitl-core --test diag_docs`). Bis HUM-047 stand an dieser Stelle eine Aufzählung; sie nannte zehn von damals achtzehn Bereichen, und jede Ergänzung um einen weiteren hätte eine veraltete Liste vollständig aussehen lassen. Wer einen Bereich braucht, trägt ihn in `AREAS` ein; der Test `codes_stay_inside_their_area` prüft jeden Code dagegen. Eine Einzelheit, die `AREAS` nicht sagt: In `edit` (HUM-047) bleibt **`EDIT_004` mit Absicht frei** — die Spezifikation gab ihm die gesperrte Kopfzeile im Edit, und die wird nicht abgelehnt, sondern still verworfen und als `edit.locked_header_dropped` vermerkt, weil der Daemon diese Werte selbst setzt. Ein Code wird nie wiederverwendet; entfernte Codes bleiben als `#[deprecated]` stehen. CI-Test: jeder im Code verwendete Code ist im Register.
...
- Prozessmodell mit `--as-pid-1` (HUM-203, ersetzt die frühere Fassung „ohne `--as-pid-1`"): PID 1 in der Sandbox ist der Shim-Elternprozess, nicht das Init von bwrap. `to_bwrap_args` setzt das Flag unabschaltbar; es verlangt `--unshare-pid` (Pflicht in jedem Profil) und verbietet `--lock-file`. Der Elternprozess macht sich als erste Anweisung von `parent()` mit `prctl(PR_SET_DUMPABLE, 0)` nicht-dumpable, prüft das mit `PR_GET_DUMPABLE` und öffnet erst danach das Tor zum `exec`; sein Kind, der Agent, trägt den Filter mit `Seccomp: 2`, er selbst den der Brücke. Als PID 1 erntet der Shim jede Waise (`waitpid(-1)`), zählt nur den Status des eigenen Kindes und reicht `SIGTERM` und `SIGHUP` weiter, weil der Kernel einem Namensraum-Init Signale ohne Handler nicht zustellt. `/proc/1/status` zeigt deshalb `Seccomp: 2`, `/proc/1/environ` und `/proc/1/mem` sind für den Agenten nicht lesbar (Besitzer root (vom Host aus gesehen; in der Sandbox die Überlauf-UID 65534), weil nicht dumpable), und `pidfd_getfd` steht im Boden. Vorher war bwraps Init PID 1, ungefiltert und dumpable, und bei `ptrace_scope=0` über `/proc/1/mem` beschreibbar (Sicherheitsdurchlauf 2026-09-23, Befund M1).
...
- Fake-Modus: `humanitld --fake <session.jsonl> [--speed N] [--loop] [--scale-timeouts] [--pause-at T_MS]… [--hold-timeout-secs N] [--event-buffer N] [--socket PATH]`. `--loop` ersetzt in jeder Flow-Id die 48-Bit-Zeit durch die des Durchlaufs, einmal je Durchlauf. `--pause-at` hält vor der Stelle `T_MS` an, bis `SIGUSR1` kommt, ein Signal je Haltepunkt (HUM-185). Sitzungsformat: `body_b64` verbindlich, `body` Klartext für Handgeschriebenes; höchstens 2000 Flows, abgeschlossene werden zuerst entfernt. Alle Pfade aus `humanitl_config::Paths`.
...
- Konfiguration, Nachtrag aus dem Review von HUM-062 (Codex): Das Projekt-Profil `<projekt>/.humanitl/profile.toml` liegt im geklonten Repository und ist damit Angreifer-beeinflusst. Vertrauensrelevante Schlüssel dürfen deshalb nur aus eingebauten Defaults, globaler Config, globalem Profil, Umgebung oder Kommandozeile kommen, nie aus dem Projekt-Profil: `llm.*`, `sandbox.work_dir`, `sandbox.work_mode`, `sandbox.profile`, `sandbox.env`, `agent.adapter`, `agent.command`, `hold.ask_mode`, `hold.hard_block_checksum_secrets` (seit HUM-208: ein Projekt, das die harte Sperre für Geheimnisse mit Prüfsumme abschaltet, lässt IBAN und Kartennummer ungefragt über eine vorhandene Freigabe hinaus), `findings.enabled`, `findings.ignored_hashes`, `findings.email_allow_domains`, `pseudonyms.*`, `resolver.*`, `experimental.*`, `recorder.retention_days`, `limits.recorder_max_body_bytes` (seit HUM-209: der Wert bestimmt, wie viel die Aufzeichnung belegt, und was der Mensch beim Halten sieht; ein Projekt, das ihn auf 1 KiB senkt, zeigt beim Halten nur diesen Rest, während `Allow` den ganzen Body schickt; die Gruppe `limits` bleibt sonst `allowed`), `audit.*` (seit HUM-050: ein Projekt, das den Abstand der Anker oder der Synchronisation vergrößert, vergrößert das Fenster, in dem ein gekürztes Audit-Log unbemerkt bleibt). Jedes Schema-Feld trägt dafür `x-project-scope: allowed | denied`; der Loader lehnt ein Projekt-Profil mit gesperrtem Schlüssel mit `CONFIG_003` ab und nennt Schlüssel und Ebene. Zusätzlich: `llm.endpoint` nur mit Schema `http` oder `https`; `sandbox.work_dir` muss absolut sein, wird kanonisiert und muss ein existierendes Verzeichnis sein; `..`-Segmente sind `CONFIG_003`.
...
### 4.33 Aus der Umsetzung der Pause mit offenen Funden (HUM-049, 2026-09-18)

Verkleinerter Umfang, siehe Kopf von `backlog/sprint-4.md`. Die Abweichungen von der Spezifikation:

- **Die harte Sperre bleibt eine sofortige Sperre.** Seit HUM-023 blockt `FlowHandler::analyze` eine Anfrage mit bestätigtem Geheimnis unter `hold.hard_block_checksum_secrets` als System (`BlockReason::Secret`), bevor jemand gefragt wird; auch eine Regel `allow` und die Durchreiche zum Sprachmodell kommen daran nicht vorbei. Die Spezifikation beschrieb stattdessen eine gehaltene Anfrage, deren `Decide(Allow)` mit `HOLD_004` scheitert. Das ist nicht gebaut: Es hätte die Sperre für Regel-Freigaben neu schließen und `daemon/crates/ipc/src/server.rs` umbauen müssen, den HUM-156 gleichzeitig umbaut. Stattdessen trägt die sofortige Sperre jetzt den Befund `HOLD_004`, und dieselbe Prüfung läuft über die Funde des zweiten Scans einer bearbeiteten Fassung: Wer ein bestätigtes Geheimnis in den Entwurf schreibt oder stehen lässt, dessen Freigabe nimmt das System zurück (`BlockReason::Secret`). Der Weg über den Editor bei gesetztem Schalter ist HUM-159.
- **Eine Prüfung, ein Ort.** `humanitl_proxy::findings::check_allow(findings, hard_block)` und `hard_blocks(finding)` stehen in `daemon/crates/proxy/src/findings.rs`, nicht in `hold.rs`: Die Warteschlange kennt keine Funde (Modulkommentar von `hold.rs`). Gesperrt wird nur `Tier::Checksum` in den Arten `ApiKey`, `Jwt`, `Iban`, `CreditCard`; heute tragen nur IBAN und Kreditkarte diese Stufe. `edit::remaining_findings` liefert dafür die Liste statt der Zahl.
- **Bereich `hold`.** `HOLD` mit 001 bis 009 steht in `AREAS`; vergeben ist nur `HOLD_004`, weil der Name in Spezifikation und Oberfläche so feststand. 001 bis 003 sind frei.
- **Keine Bestätigung auf dem Draht.** `DecideRequest.acknowledged_findings` (Feld 8) und `ignore_always` (Feld 9) sind nicht angelegt; „Trotzdem senden" schickt ein gewöhnliches `Decide(Allow)`. Für die harte Sperre ist das gleichgültig, weil eine Bestätigung sie nie aufhebt; was fehlt, ist die Spur in `findings.resolved`, im `Decided`-Ereignis und im Audit (HUM-160).
- **Die Pause in der Oberfläche.** `openFindingsPauseProvider` (Flow-Id oder null) sagt, über welcher Anfrage sie steht; `FindingsPause` in `app/lib/core/ui/findings_pause.dart` (seit HUM-161; vorher unter `features/intercept/widgets/`) ersetzt die Controls der Aktionsleiste. Ein Klick auf die amberfarbene Freigabe, `Enter` und `A` öffnen sie bei **einer** Anfrage; über eine Gruppe bleibt es beim Halten oder einer Taste (`docs/UX.md` 4.7). Das Halten der Freigabe sendet weiter ohne Pause. `InterceptDecision.allow` heißt dafür `allow({remember, acknowledged})`; `allowMany` behält `confirmed` für Gruppen und reicht `acknowledged` an die einzelne Anfrage weiter. Ob die Pause steht, sagt genau ein Prädikat, `findingsPauseVisibleProvider` in `providers/decision.dart`: geöffnet über dieser Anfrage, sie ist gehalten, sie ist die ganze Auswahl, und sie hat noch einen offenen Fund. Die Leiste zeichnet danach, und `S`, `P`, `Esc` wirken nur danach; sonst wäre eine unsichtbare Pause über die Tastatur noch bedienbar. Den gekürzten Wert zeigt die Pause, wie `display_prefix` ihn bringt (`daemon/crates/findings/src/display.rs`, maskiert und mit eigenem `…`); die Liste scrollt ab 160 px, damit die Knöpfe sichtbar bleiben.
...
- **Pseudonymisieren.** `InspectorEditorBuilder` trägt `{required bool replaceAll}`; `EditorScreen.replaceAllOnOpen` ruft nach dem Laden einmal `DraftNotifier.replaceAllOpen`. Der Editor-Knopf „Editierte Version senden" fragt seit HUM-161 bei verbliebenen Funden: Er heißt dann „Editierte Version senden mit n Funden" (`editorSendWithFindings`), und Klick oder `Ctrl+Enter` öffnen dieselbe Pause an der Stelle der Editor-Leiste, über den offenen Funden des Entwurfs. Dafür nimmt `FindingsPause` `count` und die Liste `findings` statt eines `FindingSet`, dazu `onBlock` (null lässt „Blockieren" weg), `pseudonymizeLabel` (im Editor „Alle durch Pseudonyme ersetzen") und `letterKeys` (im Editor falsch: `S`, `P`, `B` schreiben dort ins Feld). `EditorScreen` bekommt `onBlock` und `sendRefused`; unter der harten Sperre fehlt „Trotzdem senden", solange ein offener Fund die Stufe `checksum` trägt. Ein Fund des Entwurfs trägt seinen Wert als Anker (`FindingView.value`); nach jeder Eingabe und jeder Ersetzung ordnet `reanchorFindings` (`app/lib/features/editor/model/anchoring.dart`) die Funde ihren Werten neu zu: eins zu eins (zwei Funde teilen nie eine Kopie), ersetzt wird nur, wo der Wert noch genau so steht, ein Fund ohne freie Kopie an seinem Ort bleibt offen und ohne Stelle, solange der Wert irgendwo in Rumpf, Kopfzeilen (Name und Wert), Methode, Pfad oder Query steht (in Pfad und Query auch prozentkodiert), `FindingStatus.removed` gilt erst, wenn er nirgends mehr steht, und ein ersetzter oder entfernter Fund wird wieder offen, sobald sein Wert frei dasteht (Zurücktippen, `Ctrl+Z`). `ignored` setzt nur `ignoreFinding`. Ein Fund, dessen Bereich sich nicht auf einen Wert umrechnen lässt oder dessen Kopfzeile unter gleichnamigen nicht eindeutig ist (`FindingView.unplaced`), bleibt offen, zählt für Knopf und Pause und wird nie ersetzt; trägt er einen Wert oder mögliche Werte (`FindingView.alternatives` bei mehrdeutiger Kopfzeile), ist er `removed`, sobald keiner davon mehr hinausginge. Eine Auswahl mit `Ctrl+R` trägt ihren Text als Anker, damit `Ctrl+Z` sie wieder öffnet, gilt aber nur an ihrem Ort: Sie ist eine Entscheidung über diese Stelle, oft ein kurzes Wort, und öffnet sich nicht an zufälligen Kopien anderswo. Pfad und Query werden nachsichtig dekodiert verglichen (nur gültige `%XX`, `+` als Leerzeichen, UTF-8 mit Ersatzzeichen), auch beim Wiederöffnen. „Alle ersetzen“ läuft, bis nichts mehr ersetzt wird. Solange der Editor offen ist, sind die Tasten der Warteschlange aus.
...
| `daemon install` beim Paket: nur `enable --now humanitld.socket` | Erledigt durch HUM-164: seither wie spezifiziert nur `enable --now humanitld.socket` | Bis HUM-164 las ein Client das Token vor dem Verbinden, und ein Socket allein weckte den Dienst nie; deshalb aktivierte `daemon install` auch den Dienst. Seit HUM-164 öffnet ein Client ohne Token den Socket einmal und wartet höchstens zehn Sekunden auf das Token (`humanitl_ipc::client::token_or_wake`). `daemon uninstall` meldet weiter beide Units ab. |
...
### 4.35 Aus der Umsetzung der Paritäts-Tabelle (HUM-078, 2026-09-19)

- **Kein Attribut `#[humanitl(rpc = "…")]`.** `clap` lehnt fremde Attribute an seinen `derive`-Typen ab; ein eigenes Makro nur dafür wäre mehr Code als die Zuordnung selbst. Die Zuordnung steht als `PARITY: &[(&str, &str)]` in `daemon/bin/humanitl/src/parity.rs`, eine Zeile je Paar aus RPC und Unterkommando samt Schaltern, zum Beispiel `("Humanitl.GetBody", "flows show --body")`. Die Tests dort lösen jede Zeile gegen `cli::command()` auf: Die Wörter vor dem ersten Schalter müssen ein Unterkommando ohne weitere Unterkommandos ergeben; danach darf nur ein langer Schalter dieses Unterkommandos stehen (Aliasse und globale Argumente zählen), gefolgt von höchstens einem Wert, wenn er einen nimmt, und dann einem erlaubten. Jedes andere Wort, auch ein kurzer Schalter, ist ein Fehler. Das Modul wird nur für Tests übersetzt, im Binary hat die Tabelle keine Aufgabe. ADR-018 nennt noch das Attribut; gemeint ist dieselbe Pflicht.
- **Die Spalte „CLI" nennt Unterkommando und Schalter, nicht die volle Synopse.** `cargo xtask docs` liest `PARITY` als Text und sieht die `clap`-Struktur nicht; eine handgeschriebene Synopse wäre die gepflegte Tabelle, die ADR-018 ausschließt.
- **Der Ort in der Oberfläche ist ein Pfad.** `app/lib/core/parity.dart` nennt je RPC eine Datei unter `app/lib/` ohne `.dart` (`features/intercept/widgets/action_bar`), und der Generator prüft, dass es sie gibt. Ein verschobenes Widget bricht `parity-check`, bis die Zeile folgt.
- **Streng in beide Richtungen.** Eine RPC ohne CLI-Zeile und ohne Ausnahme bricht den Lauf, ebenso ein Eintrag in `PARITY`, in der UI-Registry oder in `daemon/xtask/parity_exempt.toml`, der eine RPC nennt, die der Vertrag nicht kennt, eine Ausnahme ohne Begründung, eine Ausnahme mit CLI-Zeile und jede Zeile in den beiden Tabellen, die nicht im festen Format steht. Nur eine fehlende UI-Zeile ist eine Warnung.
...
- **Der zweite Scan läuft zweimal.** Die Warteschlange veröffentlicht `Decided`, bevor der Handler die bearbeitete Fassung prüft; die Zahl des zweiten Scans gehört aber in dieses Ereignis. `HoldQueue::counting_edits` hängt deshalb `edit::SecondScan` ein, das mit demselben Scanner und derselben Grenze durch `apply_edit` und `remaining_findings` geht wie der Handler, der danach die harte Sperre prüft. Die Naht ist der Trait `EditCount`, wie `DomainSink` ohne zweiten Adapter und kein Port; die Warteschlange kennt weiter keine Detektoren. Die Alternative, `Decided` einer bearbeiteten Freigabe erst im Handler zu veröffentlichen, hätte den Vertrag von `FlowPipeline::decide` („zurück in `Decided`") gebrochen.
- **`IPC_004` statt `IPC_002`.** Die Spezifikation nennt für einen Index außerhalb der Funde `IPC_002`. Der Code heißt „AllowEdited nur für genau einen Flow" und behauptete hier etwas Falsches; `IPC_004` („Decide-Anfrage ungültig") deckt jede unbrauchbare `Decide`-Anfrage (4.12). Ebenso `IPC_004`: `acknowledged_findings` mit `block` oder `allow_edited` (bei einer bearbeiteten Fassung zeigten die Indizes in die gehaltene, nicht in die, die hinausgeht; der Editor bestätigt erst mit HUM-161) und mit mehr als einer Flow-Id. Die statischen Fälle prüft `validate::decide_plan`, den Index die Warteschlange (`NotHeld::UnknownFinding`); in beiden Fällen wird nichts entschieden, und der Flow wartet weiter.
- **Nur die Pause bestätigt.** „Trotzdem senden" (Knopf und `S`) schickt über `InterceptDecision.sendAnyway` die Indizes aller offenen Funde, wie das Detail sie nennt (`FindingSet.indices`, der Platz in der Liste des Daemons). Das Halten der Freigabe sendet ohne Bestätigung; die History zählt dann jeden Fund als offen. Das ist die Aussage aus dem Kriterium: `1` über das Halten, `0` nach der Pause.
- **Aufzeichnung und Audit.** `flows.unresolved_findings` (`V9__unresolved_findings.sql`) führt die Zahl des letzten `Decided`; nimmt das System eine Freigabe zurück, wird sie wieder `NULL`. Ein bestätigter Fund bekommt `findings.resolved = 'acknowledged'`. `flow.decided` im Audit trägt `unresolved_findings` und `acknowledged` (die Zahl, nie die Indizes); `acknowledged` steht genau dann, wenn `unresolved_findings` steht.
- **Eine Zurücknahme ist eine zweite Entscheidung, auch im Audit.** Nimmt das System eine Freigabe zurück, bevor etwas hinausging, schreibt der Audit-Sink einen zweiten `flow.decided` (`block`, `decided_by = system`, ohne `unresolved_findings`) samt `flow.blocked_reason`, und `session.ended` zählt den Flow unter `blocked` statt unter der Freigabe. Jedes andere zweite `Decided` desselben Flows bleibt ungezählt. Aus der ersten Review-Runde; war kurz als HUM-175 notiert und ist hier erledigt.
- **Bestätigt wird nur, was beschrieben ist.** „Trotzdem senden" und `S` wirken erst, wenn das Detail jeden offenen Fund beschreibt (`FindingSet.complete`); bis dahin kennt die Oberfläche nur die Zahl, und ein Senden ohne Indizes schriebe das Gegenteil dessen, was der Mensch getan hat. Fragt eine Regel „für immer" vorher im Modal, reisen die Indizes im `BatchRequest` mit.
- **Der Fake zählt keine bearbeitete Fassung.** Er hat keine Detektoren; das Feld fehlt dort, wie im echten Daemon ohne `EditCount`.

### 4.38 Aus der Umsetzung der Ein-Klick-Installation (HUM-077, 2026-09-19)

Das meiste stand schon: die Units und `daemon install` aus HUM-044, HUM-070 und HUM-053, der Knopf in der Einrichtung aus HUM-044, die Zeile `daemon` des Doctors aus HUM-075. Gebaut wurde, was fehlte: `daemon uninstall`, die Erneuerung eines `AppImage` beim Start, der Neustart nach einem Wechsel der Binaries und der Befund der Kommandozeile in der Karte der Einrichtung.

| Spezifikation | Gebaut | Grund |
|---|---|---|
| `app/lib/features/setup/widgets/service_card.dart`, eine eigene Karte „Hintergrunddienst aktivieren" | die Zeile „Hintergrunddienst" mit `FixControl` aus HUM-044 | Steht schon so in 4.34: Der Knopf ruft `humanitl daemon install`, und das erkennt Paket, Archiv und `AppImage` selbst. Eine zweite Karte wäre eine zweite Entscheidung in der Oberfläche (ADR-018). |
| Fehler: `DAEMON_005` mit dem genauen `systemctl`-Befehl zum Kopieren | der Befund der Kommandozeile selbst, gelesen aus `humanitl --json daemon install`; die Kopierzeile ist sein Vorschlag | `DAEMON_005` heißt „fremde Unit". Welcher Schritt scheiterte, weiß nur die Kommandozeile (`DAEMON_008` mit `systemctl --user status humanitld.service`, `DAEMON_010` mit `loginctl enable-linger`, `DAEMON_005` mit dem `mv`). Vorher stand in der Karte `DAEMON_001` und als Grund der ganze Text von `stderr`, also die Ankündigung samt Unit. |
| `AppRun` vergleicht `current` mit der eigenen Fassung, kopiert, schreibt die Unit, `daemon-reload`, `restart humanitld.socket` | `AppRun` ruft `humanitl -q daemon install --refresh`; Vergleich, Kopie, Unit, Neustart stehen in Rust | Eine Stelle mit Tests statt einer zweiten in Shell. Neu gestartet wird `humanitld.service`: Ohne Paket gibt es keine Socket-Unit (4.34). |
| Erster Start des `AppImage` richtet die Unit ein | der erste Start zeigt die Einrichtung mit dem Knopf; `--refresh` erneuert nur, was schon eingerichtet ist | Eine Unit, die bei jeder Anmeldung einen Dienst startet, legt ein Programmstart nicht ungefragt an (`docs/INSTALL.md`: „jeder Mensch für sich, mit einem Befehl oder einem Klick"). Und wer `daemon uninstall` gerufen hat, bekäme den Dienst sonst vom nächsten Start still zurück. |
| Doctor-Zeile 6 nutzt `daemon status --json` | der Doctor fragt weiter selbst `GetInfo` über dieselbe Verbindung | `daemon status` tut nichts anderes; ein Kindprozess seiner selbst wäre ein zweiter Weg zur selben Antwort. Geändert hat sich der Vorschlag: Ohne erreichbaren Daemon trägt `DOCTOR_006` jetzt `InstallService` statt `humanitld`. |
| Versionsabgleich mit Karte „Dienst neu starten", Ausgabe als Fortschritt, Karte grün und nach 2 s weg | nicht gebaut | HUM-186. Die Zeile wird heute grün, sobald der Zwei-Sekunden-Takt den Dienst findet (HUM-044). |
| Nightly: frisches Debian, `.deb`, UI unter Xvfb, Klick | nicht gebaut | HUM-187. `packaging/deb/check-install.sh` prüft das Paket ohne systemd. |

**`daemon install` startet neu, wenn sich geändert hat, was `ExecStart` startet.** `enable --now` lässt einen laufenden Dienst in Ruhe. Zeigte `current` vorher auf eine andere Kopie oder wurde eine Unit mit Marke ersetzt, folgt `systemctl --user restart humanitld.service`; unter `--json` steht `restarted`. Scheitert der Neustart, zeigt `current` wieder auf die vorige Kopie, die Unit bekommt ihren alten Text, ein zweiter Neustart holt den alten Daemon zurück, und der Befund ist `DAEMON_008`. Den zweiten Neustart gibt es nur, wenn beides nachweislich zurück ist (`Staged::restore` liefert ein `Result`); sonst startete er die gescheiterte Kopie noch einmal (Review von Codex).

**Alte Kopien gehen erst nach dem Neustart** (Fallstrick der Spezifikation). Vorher ging die vorige Kopie gleich nach `enable --now`, während der Daemon noch aus ihr lief. Jetzt räumt `daemon install` sie und jede ältere Kopie mit einem Namen der Form `<version>.<nanos>-<pid>` erst weg, wenn der Dienst neu gestartet ist oder vorher keine Kopie bestand; ohne Aktivierung (`--no-start`, kein `systemctl`) bleibt sie liegen.

**`daemon install --refresh`.** Tut nur etwas aus einem `AppImage`, wenn die Unit mit Marke `ExecStart=~/.local/lib/humanitl/current/bin/humanitld` nennt (oder `current/humanitld`, wie vor HUM-165; die Unit bekommt dann den neuen Pfad) und `current` auf die Kopie einer anderen Fassung zeigt. Sonst Exit 0 und `action` `not_appimage`, `not_installed` oder `up_to_date`. Verträgt sich nicht mit `--print`, `--no-start` und `--bin-dir`.

**`daemon uninstall [--purge-binaries]`: erst abmelden, dann entfernen.** `systemctl --user disable --now` für `humanitld.service`, beim Paket `humanitld.socket humanitld.service`. Scheitert das, ist nichts entfernt, und `DAEMON_014` nennt genau diesen Aufruf. Danach gehen die Verweise der Aktivierung, die Unit mit Marke, ein Socket, an dem niemand lauscht, und mit `--purge-binaries` `current`, `current.tmp-*` und die eigenen Kopien unter `~/.local/lib/humanitl`. Eine Unit ohne Marke ergibt `DAEMON_005`, und nichts wird angefasst; die Units des Pakets werden abgemeldet und bleiben liegen. Antwortet nach dem Abmelden noch ein Daemon am Socket, entfernt `--purge-binaries` nichts und meldet `DAEMON_014` (Review von Codex). Der Code steht in `daemon/bin/humanitl/src/cmd/daemon/uninstall.rs`, einem Untermodul von `daemon.rs`, weil `daemon.rs` sonst fast 1900 Zeilen hätte.

Neu im Register: `DAEMON_014` (Deinstallation unvollständig).

**Aus dem Ersatz-Review (für Antigravity).** `daemon install` und `daemon uninstall --purge-binaries` sperren `~/.local/lib/humanitl` mit `flock` auf dem Verzeichnis selbst (`lock_lib`), von der Kopie bis zum Aufräumen, und das Aufräumen lässt stehen, worauf `current` in diesem Augenblick zeigt. Scheitert der Neustart auf eine Unit, die der Lauf erst angelegt hat, gehen Unit und die Verweise der Aktivierung, die dieser Lauf angelegt hat (`Enablement` vor dem Start), und statt eines zweiten Neustarts folgen `stop` und `reset-failed`. Der Satz im `DAEMON_008` entsteht aus dem Ergebnis des zweiten Neustarts. Wo die Kommandozeile die Units des Pakets sucht, bestimmt `HUMANITL_SYSTEM_UNIT_DIR` (Vorgabe `/usr/lib/systemd/user`, ein Unterstrich, also kein Konfigurationsschlüssel); die Testumgebung setzt es auf ein leeres Verzeichnis, damit ein installiertes Paket die Tests nicht umlenkt. Zwei `daemon install` nebeneinander sind nicht deterministisch getestet; gemessen ist nur, dass das Aufräumen das Ziel von `current` stehen lässt.

**Die Einrichtung ruft `humanitl --json daemon install`.** `installServiceArguments` trägt `--json`; angezeigt wird weiter `humanitl daemon install`. Steht auf `stdout` ein Befund (`code` und `why`), gibt `commandLineDiagnostic` ihn unverändert weiter; ein Vorschlag, den die Anwendung nicht ausführt, wird zu seinem `command` zum Kopieren. Steht dort keiner, bleibt der eigene Befund `DAEMON_001` mit `humanitl daemon install` zum Kopieren.

### 4.39 Aus der Umsetzung von „Ein Agent, den es nicht gibt" (HUM-137, 2026-09-19)

- **Nie gelaufen heißt: Der Shim sagt es auf seinem Berichtskanal.** Kommt `execvp` zurück, schreibt das Kind des Shims `EXEC fail errno=<n>` auf `HUMANITL_REPORT_FD` (`humanitl_sandbox::parse_exec_line`, `ReportSnapshot::exec_failed`) und endet mit `127`. Fälschen kann der Agent die Zeile nicht: Der Deskriptor trägt `FD_CLOEXEC`, ein gelungenes `exec` schließt ihn, und vor dem `exec` wartet das Kind an einem Tor (eine Pipe, deren Schreibseite nur der Eltern-Shim hält), das erst aufgeht, wenn der Eltern-Shim nach seiner eigenen Einrichtung auch seine Kopie des Berichts geschlossen hat. Ohne das Tor hielte der Eltern-Shim, der im PID-Namensraum des Agenten lebt, einen Schreiber, den der Agent über `/proc/<pid>/fd` öffnen könnte (Review von Codex). Seit HUM-138 behält der Eltern-Shim den Schreiber für die verweigerten Versuche des Agenten, und das Tor geht auf, sobald er nicht mehr „dumpable" ist (4.42). Mit dieser Zeile ist alles im Terminal Sache des Shims, auch eine Zeile, die ein Kommandoname mit Zeilenumbruch in zwei zerlegt. Der Text im Terminal entscheidet nie; bis zum Review von Codex galt eine Zeile, die mit `humanitl-shim: exec failed:` begann, als Zeile des Shims, und ein Agent, der sie nachdruckt, hätte sich einen falschen Befund ausgestellt. `Inner::report_exit` liest den Bericht nach dem Ende bis zum Schluss der Pipe (`SandboxHandle::report_after_exit`, höchstens `STATUS_DRAIN`), denn die Zeile kommt nach den fünf Prüfungen.
- **Ohne Bericht bleibt die Regel der Spezifikation, und jedes Byte zählt.** Exit-Code `127` oder `126` und kein einziges Byte Ausgabe, auch kein Leerraum, ergeben ebenfalls `AGENT_005`. Eine Shell, die an einem fehlenden Programm scheitert, hat ihre Meldung geschrieben; ein Agent mit `--version` seine Zeile. Kommt der Faden, der die Ausgabe liest, nicht zurück, gilt sie als geschrieben (`FirstOutput::unknown`).
- **`AGENT_005` ist `Error`, nicht `Blocking`.** Die Sandbox steht und ihre drei Garantien gelten; sie ist auch ohne Agenten eine gültige Sitzung. Ein blockierender Befund schaltete in der Oberfläche den Start ab (`SandboxStatus.blocking`), und das wäre eine Aussage über die Sandbox, die nicht stimmt. Der Befund geht vor dem Exit-Code als eigenes `SandboxEvent.diagnostic` hinaus und hängt in der Oberfläche an der Momentaufnahme von `running`; der Ring bleibt grün, weil er nur die drei Garantien liest.
...
### 4.42 Aus der Umsetzung der verweigerten Versuche (HUM-138, 2026-09-23)

- **Entscheidung (ADR-Teil des Issues): `SECCOMP_RET_USER_NOTIF` für die Socket-Sperre, zusammengefasst im Shim.** Die Spezifikation stellte zwei Wege gegeneinander: Rückfrage je Aufruf oder ein Zähler im Shim. Der zweite Weg allein geht nicht: Ein Filter, der `EPERM` selbst antwortet, sagt niemandem etwas, und `SECCOMP_RET_LOG` landet im Audit-Log des Kernels, das die Sandbox nicht liest; `SECCOMP_RET_TRACE` bräuchte `ptrace`. Gebaut ist deshalb die Verbindung beider: Nur die beiden Verweigerungen der Socket-Sperre im Präludium (`Gate::Reported`) antworten `USER_NOTIF`, alles andere im Programm bleibt beim Kernel (x32, Sperrliste, Architektur). Der Elternprozess des Shims hält den Zuhörer, zählt in einer `Tally` je Paar aus Familie und Typ und antwortet `EPERM`; er antwortet nie „weiter". Erlaubte Aufrufe berührt der Zuhörer nicht. Gemessen: rund 5 bis 30 µs je verweigertem Aufruf.
- **Der Zuhörer reist über ein `SOCK_SEQPACKET`-Paar aus dem Kind in den Elternprozess** (`SCM_RIGHTS`), das vor dem `fork` entsteht. Auf demselben Paar sagt das Kind nach seinen `families`-Proben „jetzt kommt der Agent" (`X`); erst danach wird gezählt. Die Prüfung liegt hinter dem Empfang jeder Meldung, und der erste Aufruf des Agenten folgt auf das Senden des Zeichens, also kann kein Aufruf des Agenten als Probe durchgehen. Das Kind lädt den Filter mit Zuhörer **ohne** `TSYNC` (vor Linux 5.7 schließen sich beide aus, und das Kind hat nach dem `fork` einen Thread); die Beschreibung des Shims in 3.1 („TSYNC") gilt für den Rückfall und den Brückenfilter weiter.
- **Rückfall statt Abbruch.** Verweigert der Kernel den Zuhörer (`EBUSY`, wenn schon ein Filter weiter oben einen hat, oder eine Struktur, die größer ist als die dieses Binaries), lädt das Kind denselben Filter mit `Gate::Silent` und sagt es über das Paar; der Elternprozess schreibt `REFUSALS off errno<N>`, der Wirt macht daraus `SANDBOX_019` (Warning). Verweigert wird in beiden Fällen dasselbe.
- **Berichtszeilen neben den `CHECK`-Zeilen:** `REFUSALS on`, `REFUSALS off <grund>`, `REFUSED socket <familie> <typ> <family|type|overflow> <anzahl> <erste-ms> <letzte-ms>`. Jede `REFUSED`-Zeile trägt den laufenden Stand; der Wirt (`humanitl_sandbox::refusals`) behält je Paar das Größte, prüft jedes Feld und nimmt höchstens 17 Paare (16 plus Sammelposten `* *`). Der Shim schreibt höchstens alle zwei Sekunden und einmal nach dem Ende des Agenten; deshalb hält der Elternprozess seine Schreibseite des Berichts jetzt, bis der Agent gegangen ist.
- **Der Elternprozess des Shims ist nicht „dumpable"** (`PR_SET_DUMPABLE` 0 nach dem `fork`). Er hält den Zuhörer und die Schreibseite des Berichts; `/proc/<pid>/fd`, `/proc/<pid>/mem` und `pidfd_getfd(2)` bleiben dem Agenten damit verschlossen, auch bei `kernel.yama.ptrace_scope = 0`. `docs/THREAT-MODEL.md` K-04 ist nachgezogen. Das ersetzt das Argument von 4.39 (HUM-137), der Eltern-Shim halte beim Start des Agenten keinen Schreiber mehr: Er hält ihn, und das Tor vor dem `exec` geht auf, sobald er nicht mehr „dumpable" ist und seinen Filter trägt, nicht mehr, wenn er den Bericht schließt. Das `socketpair` entsteht nach dem `close_inherited` in `launch`, unmittelbar vor dem `fork`; das Kind behält sein Ende in seinem eigenen `close_inherited`.
- **Kein eigener Zuhörer für den Agenten** (Review zu HUM-138, blockierend). Beide Gates beantworten `seccomp()` mit `SECCOMP_FILTER_FLAG_NEW_LISTENER` in `arg1` mit `EPERM` (Präludium, Zeile in der Regeltabelle). Ohne diese Zeile hätte der Agent, sobald der Zuhörer des Elternprozesses zu ist, einen neueren Filter mit eigenem Zuhörer laden und seine verweigerten Sockets selbst mit „weiter" beantworten können; `has_duplicate_listener` greift nur, solange ein Zuhörer lebt. Zweite Lage: `refusals::serve` kehrt nie zurück, auch nicht nach Fehlern von `RECV`, die der Agent auslösen kann (Threads in `socket()` parken und beenden, `ENOENT`); nach hundert Fehlern in Folge wartet es 10 ms je Versuch. Belegt von `a_dropped_listener_does_not_free_the_agent_to_take_one` und ESC-1 `seccomp_no_own_listener`.
- **Der Kanal steht in `channel.rs`**, getrennt vom Zählen in `refusals.rs`.
- **Proto 1.14:** `SandboxEvent.refusals` (Feld 9, der ganze Stand je Nachricht) und `SandboxEvent.Status.refusals` (Feld 14, für einen Client, der den Start nicht begleitet hat), mit `RefusalReporting` und `RefusalReason`. Der Dienst reicht den Stand aus einem eigenen Faden (`Inner::forward_refusals`) in den Strom des Starts und wartet vor dem Exit-Code höchstens drei Sekunden auf den letzten.
- **Nicht gemeldet und bewusst so:** ein `connect()`, der an der leeren Routing-Tabelle scheitert (Garantie 1 steht als Zustand daneben), die Aufrufe der Sperrliste und die x32-Aufrufe, und die Versuche eines Agenten, der vor ihnen einen eigenen Filter lädt, der selbst `EPERM` antwortet (der Kernel nimmt die strengere Antwort, der Zuhörer hört nichts). Die Sichtbarkeit ist ein Bericht und trägt keine der drei Garantien.
- **Code:** `SANDBOX_019` ist die dritte der freien Nummern 017 bis 019 (`backlog/sprint-3.md`, Nachtrag vom 2026-09-12); 017 und 018 bleiben für Watchdog und Cache.

### 4.43 Aus der Umsetzung der harten Sperre mit Halten (HUM-159, 2026-09-24)

Löst den ersten Punkt von 4.33 ab („Die harte Sperre bleibt eine sofortige Sperre").

- **Halten statt blocken.** Unter `hold.hard_block_checksum_secrets` setzt `FlowHandler::analyze` den Befund `HOLD_004` aus `findings::check_allow` als `Flow::send_refusal` an den Flow und veröffentlicht ihn als `FlowEvent::Diagnostic` mit Flusskennung, vor dem `Held`. Der Flow geht dann in die Pipeline wie jeder andere.
- **Jeder Weg hinaus prüft den Befund am Flow.** `RulesPipeline` macht aus einer Regel `allow` und aus der Durchreiche zum Sprachmodell `findings::hard_block_decision()` (`Block { Secret }`, Quelle `System`, Satz `findings::HARD_BLOCK_NOTE` an den Agenten); `PassthroughPipeline` ebenso. `HoldQueue::decide_acknowledging`, die eine Tür für jede Entscheidung über einen gehaltenen Flow, weist ein `Allow` mit `NotHeld::SendRefused { id, refusal }` zurück, gleich welche Herkunft und welche Bestätigung, und der Flow wartet weiter. Der Befund liegt dafür in `Pending` und nicht in der Registry: Die Registry darf einen Datensatz vergessen, und eine Prüfung, die dann nichts fände, ließe das Geheimnis hinaus. Als zweite Linie leitet `FlowHandler::carry_out` ein `Allow` mit gesetztem Befund nie weiter (`revise_to_hard_block`). `AllowEdited` kommt durch und wird wie bisher ein zweites Mal gescannt.
- **`decide_one` fragt die Warteschlange, nicht die Registry.** Die Spezifikation nannte `check_allow` mit den Funden aus der Registry. Gebaut ist die Prüfung in der Warteschlange, mit dem Befund, den der Handler mit dem Schalter dieses Flows gebaut hat: Die gRPC-Schicht kennt so weder Konfiguration noch Funde, die Sammelanfrage geht durch dieselbe Tür, und ein vergessener Datensatz öffnet nichts. `not_held` meldet `SendRefused` als den Befund selbst; `grpc_code` ordnet `HOLD_004` `FailedPrecondition` zu.
- **Die Ansage am Flow, in der Zeile und im Strom.** Die Oberfläche liest den Schalter nicht (ADR-018). `FlowRecord.send_refusal` trägt den Befund, `FlowSummary.send_refusal` (Feld 30, `PROTO_MINOR` 16) bringt ihn in jede Zeile eines Flows der laufenden Sitzung: aus der Registry in `record_to_summary`, und in `recorded_page` als Auflage auf die Zeile der Aufzeichnung wie `findings_truncated`, denn die Tabelle `flows` führt die Spalte nicht. Ein Client, der nach der Ansage startet, sieht die Sperre damit beim Nachladen über `ListFlows`. Dazu faltet die Oberfläche `HOLD_004` mit Flusskennung aus dem Strom und die Antwort auf ein zurückgewiesenes `Allow` in `Flow.sendRefusal`.
- **Gruppen.** Über eine Auswahl bleiben gesperrte Anfragen stehen, statt den Batch abzubrechen (`InterceptDecision._unlocked` in `allowMany`, `askAllowAll` und `confirmBatch`); das Modal nennt ihre Zahl (`BatchRequest.locked`, `interceptBatchLocked`) und wird dafür immer gezeigt. Ist jede Anfrage der Auswahl gesperrt, steht die Freigabe als „Cannot send" da.
- **Ohne Frist.** Bei `ask_mode = none` oder `hold.timeout_secs = 0` blockt `AskPipeline` einen gesperrten Flow sofort mit `hard_block_decision()`, damit Agent und Audit `secret` lesen und nicht `timeout`.
- **Oberfläche.** Die Freigabe heißt dann „Cannot send" / „Senden nicht möglich" (`interceptSendRefused`), ist abgeschaltet und trägt den Satz des Daemons als `HoverLabel` und Semantik-Tooltip; `InterceptDecision.allow` kehrt für einen solchen Flow ohne Wirkung zurück, also tun `Enter`, `A` und `S` nichts. `FindingsPause.onSendAnyway` ist nullbar und lässt „Send anyway" weg. Der Fake weist ein `Allow` mit `HOLD_004` zurück, wenn der Flow eine Ansage trägt oder sein Schalter `hardBlockChecksumSecrets` steht und das Detail einen prüfsummen-bestätigten Fund der vier Arten nennt.
- **Satz von `HOLD_004`.** „…, so it cannot leave this machine unchanged; nothing was sent" statt „so it was blocked": Der Satz steht jetzt auch an einer gehaltenen Anfrage, die noch nicht geblockt ist.

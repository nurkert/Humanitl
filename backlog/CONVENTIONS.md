...
- **Nie gelaufen heißt: Der Shim sagt es auf seinem Berichtskanal.** Kommt `execvp` zurück, schreibt das Kind des Shims `EXEC fail errno=<n>` auf `HUMANITL_REPORT_FD` (`humanitl_sandbox::parse_exec_line`, `ReportSnapshot::exec_failed`) und endet mit `127`. Fälschen kann der Agent die Zeile nicht: Der Deskriptor trägt `FD_CLOEXEC`, ein gelungenes `exec` schließt ihn, und vor dem `exec` wartet das Kind an einem Tor (eine Pipe, deren Schreibseite nur der Eltern-Shim hält), das erst aufgeht, wenn der Eltern-Shim nach seiner eigenen Einrichtung auch seine Kopie des Berichts geschlossen hat. Ohne das Tor hielte der Eltern-Shim, der im PID-Namensraum des Agenten lebt, einen Schreiber, den der Agent über `/proc/<pid>/fd` öffnen könnte (Review von Codex). Mit dieser Zeile ist alles im Terminal Sache des Shims, auch eine Zeile, die ein Kommandoname mit Zeilenumbruch in zwei zerlegt. Der Text im Terminal entscheidet nie; bis zum Review von Codex galt eine Zeile, die mit `humanitl-shim: exec failed:` begann, als Zeile des Shims, und ein Agent, der sie nachdruckt, hätte sich einen falschen Befund ausgestellt. `Inner::report_exit` liest den Bericht nach dem Ende bis zum Schluss der Pipe (`SandboxHandle::report_after_exit`, höchstens `STATUS_DRAIN`), denn die Zeile kommt nach den fünf Prüfungen.

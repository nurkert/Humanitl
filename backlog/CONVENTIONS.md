...
- Konfiguration, Nachtrag aus dem Review von HUM-062 (Codex): Das Projekt-Profil `<projekt>/.humanitl/profile.toml` liegt im geklonten Repository und ist damit Angreifer-beeinflusst. Vertrauensrelevante Schlüssel dürfen deshalb nur aus eingebauten Defaults, globaler Config, globalem Profil, Umgebung oder Kommandozeile kommen, nie aus dem Projekt-Profil: `llm.*`, `sandbox.work_dir`, `sandbox.work_mode`, `sandbox.profile`, `sandbox.env`, `agent.adapter`, `agent.command`, `hold.ask_mode`, `hold.hard_block_checksum_secrets` (seit HUM-208: ein Projekt, das die harte Sperre für Geheimnisse mit Prüfsumme abschaltet, lässt IBAN und Kartennummer ungefragt über eine vorhandene Freigabe hinaus), `findings.enabled`, `findings.ignored_hashes`, `findings.email_allow_domains`, `pseudonyms.*`, `resolver.*`, `experimental.*`, `recorder.retention_days`, `audit.*` (seit HUM-050: ein Projekt, das den Abstand der Anker oder der Synchronisation vergrößert, vergrößert das Fenster, in dem ein gekürztes Audit-Log unbemerkt bleibt). Jedes Schema-Feld trägt dafür `x-project-scope: allowed | denied`; der Loader lehnt ein Projekt-Profil mit gesperrtem Schlüssel mit `CONFIG_003` ab und nennt Schlüssel und Ebene. Zusätzlich: `llm.endpoint` nur mit Schema `http` oder `https`; `sandbox.work_dir` muss absolut sein, wird kanonisiert und muss ein existierendes Verzeichnis sein; `..`-Segmente sind `CONFIG_003`.

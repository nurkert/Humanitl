/// Die Pause beim Senden mit offenen Funden (HUM-049, HUM-161).
///
/// Sie steht an der Stelle der Entscheidungsleiste, nicht über ihr: kein
/// Modal, keine Ebene, nichts, was sich wegklicken ließe, ohne die Liste
/// gelesen zu haben (`docs/UX.md` 4.7, 5.4). Oben steht, wie viele Funde die
/// Anfrage trägt, darunter je Fund Art, gekürzter Wert und Ort, und am Fuß die
/// Wege weiter: trotzdem senden, pseudonymisieren, blockieren, zurück.
///
/// Zwei Features zeigen sie: die Karte der Warteschlange über den Funden der
/// gehaltenen Fassung und der Editor über den offenen Funden seines Entwurfs
/// (HUM-161). Deshalb steht sie hier und nicht in einem der beiden; kein
/// Feature importiert ein anderes (`docs/ARCHITECTURE.md` 5).
///
/// Das Widget entscheidet nichts. Jeder Knopf ruft, was der Aufrufer
/// hineinreicht; wer sie zeigt, leitet das an denselben Weg weiter wie jede
/// andere Entscheidung dort (ADR-018).
library;

import 'package:flutter/widgets.dart';

import '../../l10n/l10n.dart';
import '../domain/domain.dart';
import '../text/finding_text.dart';
import 'ui.dart';

/// Wie hoch die Liste der Funde höchstens wird, bevor sie scrollt.
///
/// Die drei Knöpfe am Fuß müssen immer sichtbar bleiben: Eine Anfrage mit
/// zwanzig Funden darf die Entscheidung nicht aus dem Pane schieben
/// (`docs/UX.md` 6). Genug für fünf Zeilen in normaler Schriftgröße.
const double findingsPauseListMaxHeight = 160;

/// Wie die Pause auf- und zugeht: in der Höhe, von oben (HUM-049).
///
/// Für den `transitionBuilder` eines `AnimatedSwitcher`, in der Karte der
/// Warteschlange wie im Editor. Was gerade geht, nimmt keinen Klick mehr an
/// (HUM-161): Eine Pause, die sich in 200 ms schließt, bliebe sonst so lange
/// bedienbar, und ein Klick auf „Trotzdem senden" nach „Zurück" sendete.
Widget findingsPauseTransition(Widget child, Animation<double> animation) =>
    SizeTransition(
      sizeFactor: animation,
      alignment: Alignment.topCenter,
      child: AnimatedBuilder(
        animation: animation,
        builder: (BuildContext context, Widget? child) => IgnorePointer(
          ignoring: animation.status == AnimationStatus.reverse,
          child: child,
        ),
        child: child,
      ),
    );

/// Die Pause mit offenen Funden.
class FindingsPause extends StatelessWidget {
  /// Baut die Pause über [count] offene Funde, von denen [findings]
  /// beschrieben sind.
  const FindingsPause({
    required this.count,
    required this.findings,
    required this.onBack,
    this.onSendAnyway,
    this.onPseudonymize,
    this.onBlock,
    this.pseudonymizeLabel,
    this.letterKeys = true,
    this.enabled = true,
    super.key,
  });

  /// Wie viele Funde offen sind.
  final int count;

  /// Die offenen Funde, soweit der Daemon sie beschrieben hat, in seiner
  /// Reihenfolge. Kürzer als [count], solange ein Detail noch unterwegs ist.
  final List<Finding> findings;

  /// Wahr, sobald jeder offene Fund beschrieben ist.
  bool get _complete => findings.length == count;

  /// Sendet, was gerade vorliegt, mit seinen offenen Funden; oder null, wo
  /// das nicht geht.
  ///
  /// In der Karte der Warteschlange ist das die gehaltene Anfrage, im Editor
  /// der Entwurf. Null lässt „Trotzdem senden" weg, etwa unter der harten
  /// Sperre (`HOLD_004`, HUM-159): Ein Knopf, den der Daemon ohnehin
  /// zurückwiese, wäre ein totes Control.
  final VoidCallback? onSendAnyway;

  /// Öffnet den Editor mit allen Funden ersetzt, oder null ohne Editor.
  ///
  /// Null lässt den Knopf weg, statt ihn tot hinzustellen: Ein Control, das
  /// nichts tun kann, ist schlechter als keines (`backlog/CONVENTIONS.md`
  /// 4.13).
  final VoidCallback? onPseudonymize;

  /// Blockt die Anfrage, oder null, wo es keinen Weg dazu gibt.
  final VoidCallback? onBlock;

  /// Die Aufschrift von [onPseudonymize]; null heißt „Pseudonymisieren".
  ///
  /// Im Editor heißt derselbe Knopf „Alle ersetzen" (HUM-161): Dort ist der
  /// Editor schon offen, und der Knopf ersetzt, statt ihn zu öffnen.
  final String? pseudonymizeLabel;

  /// Falsch, wo die Tasten `S`, `P` und `B` nicht gelten.
  ///
  /// Im Editor schreiben sie in das Feld mit dem Fokus. Eine Taste neben
  /// dem Knopf, die dort nichts auslöst, versprach etwas, das nicht gilt;
  /// `Esc` gilt überall und bleibt stehen.
  final bool letterKeys;

  /// Schließt die Pause, ohne zu entscheiden.
  final VoidCallback onBack;

  /// Falsch, solange nicht entschieden werden kann.
  final bool enabled;

  @override
  Widget build(BuildContext context) {
    final HTokens tokens = HTheme.of(context);
    final AppLocalizations l10n = context.l10n;
    final int undescribed = count - findings.length;
    final String title = l10n.interceptFindingsPauseTitle(count);
    return Semantics(
      container: true,
      label: title,
      child: DecoratedBox(
        key: const Key('intercept-findings-pause'),
        decoration: BoxDecoration(
          color: tokens.colors.bg2,
          border: Border.all(color: tokens.state.held),
          borderRadius: BorderRadius.circular(tokens.radii.card),
        ),
        child: Padding(
          padding: EdgeInsets.all(tokens.spacing.x3),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              Text(
                title,
                key: const Key('intercept-findings-pause-title'),
                style: tokens.typography.ui13.medium.tinted(tokens.colors.fg0),
              ),
              SizedBox(height: tokens.spacing.x2),
              // Die Liste nimmt, was sie braucht, höchstens
              // [findingsPauseListMaxHeight], und scrollt darüber hinaus.
              ConstrainedBox(
                constraints: const BoxConstraints(
                  maxHeight: findingsPauseListMaxHeight,
                ),
                child: SingleChildScrollView(
                  key: const Key('intercept-findings-pause-list'),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: <Widget>[
                      for (int i = 0; i < findings.length; i++)
                        _FindingRow(index: i, finding: findings[i]),
                    ],
                  ),
                ),
              ),
              // Was der Daemon noch nicht beschrieben hat, zählt trotzdem:
              // Eine Liste, die kürzer ist als die Zahl darüber, sähe sonst
              // vollständig aus (`backlog/CONVENTIONS.md` 4.13).
              if (undescribed > 0)
                Text(
                  l10n.interceptFindingsPauseUndescribed(undescribed),
                  key: const Key('intercept-findings-pause-undescribed'),
                  style: tokens.typography.ui12.tinted(tokens.colors.fg1),
                ),
              SizedBox(height: tokens.spacing.x3),
              _footer(tokens, l10n),
            ],
          ),
        ),
      ),
    );
  }

  /// Der Fuß: die Wege weiter, jeder mit seiner Taste, wo sie gilt.
  Widget _footer(HTokens tokens, AppLocalizations l10n) => Wrap(
    spacing: tokens.spacing.x3,
    runSpacing: tokens.spacing.x2,
    crossAxisAlignment: WrapCrossAlignment.center,
    children: <Widget>[
      if (onSendAnyway case final VoidCallback send)
        _PauseButton(
          key: const Key('intercept-findings-pause-send'),
          variant: HButtonVariant.secondary,
          label: l10n.interceptFindingsPauseSendAnyway,
          shortcut: letterKeys ? l10n.interceptFindingsPauseKeySend : null,
          // Erst wenn jeder offene Fund beschrieben ist: Bestätigt
          // wird, was zu sehen war, mit seinem Platz in der Liste
          // des Daemons (HUM-160).
          onPressed: enabled && _complete ? send : null,
        ),
      if (onPseudonymize != null)
        _PauseButton(
          key: const Key('intercept-findings-pause-pseudonymize'),
          variant: HButtonVariant.primary,
          label: pseudonymizeLabel ?? l10n.interceptFindingsPausePseudonymize,
          shortcut: letterKeys
              ? l10n.interceptFindingsPauseKeyPseudonymize
              : null,
          onPressed: enabled ? onPseudonymize : null,
        ),
      if (onBlock != null)
        _PauseButton(
          key: const Key('intercept-findings-pause-block'),
          variant: HButtonVariant.danger,
          label: l10n.interceptFindingsPauseBlock,
          shortcut: letterKeys ? l10n.interceptKeyBlock : null,
          onPressed: enabled ? onBlock : null,
        ),
      _PauseButton(
        key: const Key('intercept-findings-pause-back'),
        variant: HButtonVariant.ghost,
        label: l10n.interceptFindingsPauseBack,
        shortcut: l10n.interceptFindingsPauseKeyBack,
        onPressed: onBack,
      ),
    ],
  );
}

/// Eine Zeile der Liste: Art, gekürzter Wert, Ort.
class _FindingRow extends StatelessWidget {
  const _FindingRow({required this.index, required this.finding});

  final int index;
  final Finding finding;

  @override
  Widget build(BuildContext context) {
    final HTokens tokens = HTheme.of(context);
    final AppLocalizations l10n = context.l10n;
    // `display_prefix` kommt vom Daemon schon in seiner Anzeigeform
    // (`daemon/crates/findings/src/display.rs`): maskiert und, wo gekürzt, mit
    // eigenem `…`, etwa `a***@x.de`, `GB82 …` oder `**** 1111`. IPv4-Adressen
    // und eigene Begriffe stehen dort vollständig. Die Oberfläche zeigt ihn,
    // wie er kommt; ein zweites Auslassungszeichen wäre falsch.
    final String prefix = finding.displayPrefix;
    return Padding(
      key: Key('intercept-findings-pause-row-$index'),
      padding: EdgeInsets.only(bottom: tokens.spacing.x1),
      child: Wrap(
        spacing: tokens.spacing.x2,
        runSpacing: tokens.spacing.x1,
        crossAxisAlignment: WrapCrossAlignment.center,
        children: <Widget>[
          HBadge(text: findingName(finding, l10n), color: tokens.state.held),
          if (prefix.isNotEmpty)
            Text(
              prefix,
              key: Key('intercept-findings-pause-prefix-$index'),
              style: tokens.typography.mono12.tinted(tokens.colors.fg0),
            ),
          Text(
            findingWhere(finding, l10n),
            style: tokens.typography.ui12.tinted(tokens.colors.fg1),
          ),
        ],
      ),
    );
  }
}

/// Ein Knopf am Fuß der Pause, mit seiner Taste daneben.
class _PauseButton extends StatelessWidget {
  const _PauseButton({
    required this.variant,
    required this.label,
    required this.shortcut,
    required this.onPressed,
    super.key,
  });

  final HButtonVariant variant;
  final String label;
  final String? shortcut;
  final VoidCallback? onPressed;

  @override
  Widget build(BuildContext context) {
    final HTokens tokens = HTheme.of(context);
    return HButton(
      variant: variant,
      onPressed: onPressed,
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          Text(label),
          if (shortcut case final String key) ...<Widget>[
            SizedBox(width: tokens.spacing.x2),
            Text(
              key,
              style: tokens.typography.mono11.tinted(tokens.colors.fg1),
            ),
          ],
        ],
      ),
    );
  }
}

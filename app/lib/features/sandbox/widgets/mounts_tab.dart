...
/// Wie viel Höhe der Kopf des Reiters höchstens einnimmt, wenn es eng wird.
const double _headShare = 0.4;
...
class MountsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Der `LayoutBuilder` steht über der Spalte und nicht darin: Eine
    // vertikale Spalte reicht ihren Kindern `maxHeight: double.infinity`, und
    // ein Deckel, der daraus einen Anteil rechnet, wäre wieder unendlich —
    // also keiner. Derselbe Fehler wie eine Ebene höher, aus dem Review von
    // Antigravity (HUM-068).
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) => Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: <Widget>[
          // Der Satz oben ist wichtig, aber er darf den Reiter nicht sprengen:
          // Wenn wenig Platz bleibt, rollt dieser Block in sich, statt Inhalt
          // unsichtbar über den Rand zu schieben.
          //
          // Kein `Flexible`: Das teilte die Höhe mit der Tabelle darunter zur
          // Hälfte, und bei viel Platz bliebe unter dem kurzen Satz eine leere
          // Fläche stehen (Review Antigravity). Der Deckel greift nur, wenn es
          // eng wird.
          ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight: constraints.maxHeight * _headShare,
            ),

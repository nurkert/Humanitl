...
1. Nur additiv, mit der einen Ausnahme in Regel 2. Neue Felder bekommen die nächste freie Nummer, neue Enum-Werte die nächste freie Zahl, neue RPCs kommen ans Ende des Service.
2. Feldnummern werden nie recycelt. Ein entferntes Feld wird `reserved`, nie überschrieben. Dasselbe gilt für Enum-Werte. Entfernt wird ein Feld sonst nur mit einem Major-Sprung (Abschnitt 5), mit einer Ausnahme: ein Feld, das nachweislich kein Handler liest und kein Client setzt. Es zu entfernen bricht niemanden, denn nie hing etwas an ihm; Nummer und Name werden `reserved`, und die Tabelle unten nennt es (HUM-089). `buf breaking` meldet eine solche Löschung trotzdem als `FIELD_NO_DELETE`. Unabhängig davon steht der Schritt in CI noch auf `continue-on-error`; wer die Flag entfernt, entscheidet das mit einem ADR, und derselbe ADR legt fest, wie diese Ausnahme dann in der Regelmenge steht. Weder die Flag noch eine niedrigere Kategorie sind eine Folge dieser Ausnahme.
   ...
   | `DecideRequest` | 6 | `acknowledge_findings` als `bool` | Nie gelesen, nie gesetzt; die Bestaetigung steht seit HUM-160 als `repeated uint32 acknowledged_findings` auf 8 (HUM-089). |
...
9. Ein neuer RPC bringt sein CLI-Subkommando im selben Issue mit (`docs/ARCHITECTURE.md` 3b). Die eine offene Ausnahme, `ProbeLlm` (HUM-039), ist mit HUM-114 geschlossen: `humanitl llm test URL` gibt es, und `humanitl rules test URL` wertet seither über `RulesRequest.Test` aus, statt mit `CLI_003` zu antworten (`backlog/CONVENTIONS.md` 4.21). `GetSessionSummary` (HUM-043) bringt `humanitl sessions summary <id>` mit.

## 5. Versionsregel

`Info.proto_major` und `Info.proto_minor` sagen, was der Daemon spricht; die Konstanten dazu stehen in `humanitl_ipc::PROTO_MAJOR` und `PROTO_MINOR`. Stand 2026-09-24: `1.16` (die harte Sperre in der Zeile: `FlowSummary.send_refusal` traegt den Befund `HOLD_004`, solange ein Flow der laufenden Sitzung unter `hold.hard_block_checksum_secrets` nur bearbeitet hinaus darf; ein fehlendes Feld heisst „keine Sperre angesagt“, nie „Senden erlaubt“, HUM-159; davor `1.15` mit dem Zeitpunkt des Kopfes: `AuditResponse.head_ts` trägt bei `verify` und `head` den Zeitstempel des letzten Records als Text, genau mit den Zeichen seiner Zeile, weil diese Zeichen im Hash stehen, wie bei `AuditEntry.ts`; leer ohne Kopf, HUM-162; davor `1.14` mit den verweigerten Versuchen: `SandboxEvent.refusals` und `SandboxEvent.Status.refusals` tragen, was der Filter dem Agenten an `socket(2)` verweigert hat, je Paar aus Familie und Typ gezählt, mit `RefusalReporting` und `RefusalReason`; neue Stände kommen nur auf dem Strom des Starts, alle anderen Clients lesen die Momentaufnahme, HUM-138; davor `1.13` mit der Spur offener Funde: `DecideRequest.acknowledged_findings` traegt die Funde, die der Mensch mit „Trotzdem senden" bestaetigt, nur mit `allow` und genau einer Flow-Id, sonst `IPC_004`; `FlowEvent.Decided.unresolved_findings` und `FlowSummary.unresolved_findings` sagen als `optional uint32`, wie viele Funde ohne Ersetzung und ohne Bestaetigung hinausgingen, bei `allow_edited` die des zweiten Scans, und ein fehlendes Feld heisst „nichts ging hinaus oder niemand zaehlte", nie null, HUM-160; davor `1.12` mit der `Audit`-RPC im Daemon: `verify` mit Schlüssel und Ankern, `head`, `query` mit Filter nach Art, Sitzung und Zeitraum samt Cursor, `export` als JSONL oder CSV in eine Datei, die der Daemon schreibt und nie überschreibt; `AuditResponse` füllt die Felder 7 bis 14, die HUM-051 angelegt hat, und `anchors_reported` sagt, dass Zahl und Zeitpunkt der Anker gemeldet sind, HUM-156; davor `1.11` mit dem Dienst in der Zeile: `FlowSummary.catalog_id` traegt die Kennung des Katalogeintrags, den der Daemon zum Host gefunden hat, leer wenn er keinen gefunden hat, damit die Warteschlange eine Gruppe nach dem Dienst benennt, ohne den Katalog ein zweites Mal zu fragen, HUM-094; davor `1.10` mit der Notiz einer Entscheidung: `FlowSummary.decision_note` und `FlowDetail.decision_note` tragen den Satz, den der Mensch beim Blocken an den Agenten gerichtet hat, gesaeubert wie der 403-Rumpf und aus der Aufzeichnung gelesen, damit er einen Neustart des Daemons ueberdauert, HUM-117; davor `1.9` mit dem entpackten Body: `BodyRef.content_encoding` nennt die Kodierung der aufgezeichneten Bytes, `BodyRef.decoded` bittet `GetBody` darum, sie abzunehmen, und `BodyChunk.encoding_left` sagt, was danach noch auf den Bytes liegt — leer nach dem Entpacken, sonst der Name der Kodierung, die der Daemon nicht kann, HUM-119; davor `1.8` mit der registrierbaren Domain in der Zeile: `FlowSummary.apex` traegt sie nach der Public Suffix List, leer wenn der Daemon sie nicht kennt, und derselbe Wert steht im Filterterm `apex:`, HUM-091; davor `1.7` mit der Hinweiszeile als eigenem Rahmen: `TerminalOutput.notice` traegt die Zeile zu einem gehaltenen oder entschiedenen Fluss neben den Bytes des Agenten statt darin, damit ein Client mit eigener Anzeige sie nicht in sein Bild bekommt, HUM-042; davor `1.6` mit der Zusammenfassung eines Sandbox-Laufs: `SessionSummary` samt `FileChange`, `SymlinkEscape`, `SummaryFinding` und `FileChangeKind`, der achte Arm `SandboxEvent.summary` und die RPC `GetSessionSummary(SessionSummaryRef)`, HUM-043; davor `1.5` mit `FlowSummary.meta`, dem Vermerk an einer Anfrage, die der Proxy selbst beantwortet hat, HUM-103; davor `1.4` mit der Konfiguration je Sitzung am Start — `SandboxRequest.Start.session_profile`, `.ask_mode` und `.cli_overrides` — samt den beiden Ereignissen, die den Agenten begleiten, `SandboxEvent.output` und `SandboxEvent.exit`, HUM-067; davor `1.3` mit der Sandbox-Momentaufnahme `Mount`, `EnvVar`, `argv_preview` und `agent_running` samt der Operation `SandboxRequest.Plan`, HUM-040; davor `1.2` mit `ProbeLlm`, `FlowEvent.flow_diagnostic`, `Rule.passthrough_llm` und `RuleMatcher.path_prefixes`, HUM-039). Die Nebenversion steigt bei jeder additiven Erweiterung, und `app/lib/core/ipc/proto_version.dart` darf nachziehen; eine aeltere Nebenversion auf einer Seite ist keine Stoerung, sie liest die neueren Felder dann nicht. Sie steht seit HUM-159 auf `16`, weil die Freigabe nach dem Nachladen `FlowSummary.send_refusal` liest; davor stand sie seit HUM-138 auf `14`, weil der Isolations-Reiter die verweigerten Versuche liest; davor stand sie seit HUM-160 auf `13`, weil die Pause „Trotzdem senden" `acknowledged_findings` schickt und das History-Detail `FlowSummary.unresolved_findings` zeigt; davor stand sie seit HUM-156 auf `12`, weil der Audit-Screen `anchors_reported` liest und ohne das Feld „Anker nicht gemeldet" zeigt statt „null Anker"; davor stand sie seit HUM-094 auf `11`, weil der Kopf einer Gruppe den Dienst aus `FlowSummary.catalog_id` benennt und die Katalog-Karte denselben Wert liest; davor stand sie seit HUM-117 auf `10`, weil das History-Detail die Notiz unter der Entscheidung zeigt; davor stand sie seit HUM-119 auf `9`, weil die Rumpf-Ansicht `decoded` verlangt und `encoding_left` liest; davor stand sie seit HUM-091 auf `8`, weil die Warteschlange nach `FlowSummary.apex` gruppiert und der Regel-Scope daraus seine Domaene schreibt; davor stand sie seit HUM-103 auf `5`, weil die Historie `FlowSummary.meta` liest. Die Nebenversionen `4`, `6` und `7` werden auf dieser Seite uebersprungen: die fuenf Felder des Sitzungsstarts (HUM-067), die Zusammenfassung eines Sandbox-Laufs (HUM-043, bis das Sheet dasteht) und die Hinweiszeile neben den Terminal-Bytes (HUM-042) haben in der Oberflaeche weiterhin keinen Leser. Ebenso `15`: `AuditResponse.head_ts` (HUM-162) liest bisher nur die Kommandozeile; die App zieht nach, sobald die Karte des Audit-Screens den Zeitpunkt zeigt.

- Additive Änderung ⇒ Minor steigt. Ältere Clients funktionieren weiter, sie sehen die neuen Felder nicht.
- Ein nachweislich totes Feld zu entfernen (Abschnitt 4, Regel 2) ist kein Bruch: Major und Minor bleiben, weil sich für keinen Client etwas ändert.

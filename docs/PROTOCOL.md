...
`Info.proto_major` und `Info.proto_minor` sagen, was der Daemon spricht; die Konstanten dazu stehen in `humanitl_ipc::PROTO_MAJOR` und `PROTO_MINOR`. Stand 2026-09-05: `1.5` (`FlowSummary.meta`, der Vermerk an einer Anfrage, die der Proxy selbst beantwortet hat, HUM-103; davor `1.4` mit der Konfiguration je Sitzung am Start — `SandboxRequest.Start.session_profile`, `.ask_mode` und `.cli_overrides` — samt den beiden Ereignissen, die den Agenten begleiten, `SandboxEvent.output` und `SandboxEvent.exit`, HUM-067; davor `1.3` mit der Sandbox-Momentaufnahme `Mount`, `EnvVar`, `argv_preview` und `agent_running` samt der Operation `SandboxRequest.Plan`, HUM-040; davor `1.2` mit `ProbeLlm`, `FlowEvent.flow_diagnostic`, `Rule.passthrough_llm` und `RuleMatcher.path_prefixes`, HUM-039). Die Nebenversion steigt bei jeder additiven Erweiterung, und `app/lib/core/ipc/proto_version.dart` darf nachziehen; eine aeltere Nebenversion auf einer Seite ist keine Stoerung, sie liest die neueren Felder dann nicht. Sie steht seit HUM-103 auf `5`, weil die Historie `FlowSummary.meta` liest; die Nebenversion `4` wird auf dieser Seite uebersprungen, denn die fuenf Felder des Sitzungsstarts (HUM-067) haben in der Oberflaeche weiterhin keinen Leser.
